...
测试、构建、基准和长期浏览器进程使用仓库认可的内存上限与墙钟超时入口，覆盖编译和
执行阶段。每条长命令不超过本次剩余时间；候选预算计算按 checkpoint。无法在截止前
完成验证则保存 inconclusive，不让子进程绕过外层限制。
...
完整门禁前先核对候选与基线的 CPU 型号、逻辑核心数、亲和性、进程可用并行度、
构建 features、二进制和测量配置。绑核可能改变 `available_parallelism()`，导致原脚本
跳过比较而仍退出 0；不能改报告元数据、阈值或基线强行兼容。不可比记 inconclusive。
迭代期先做相关路径的配对诊断，候选冻结后执行必要全量门禁；定向结果不能代替全量。
失败后先写下一次测量能排除的假设、改变的条件及预计成本，再决定是否重跑。
同条件基础设施重试仍受前述一次上限约束；没有新证据就定位阶段耗时或保存阻塞，
不重复测量直到偶然全绿。只在测量结束后观察到繁忙，不能推断整个测量窗口都有干扰。
...
进程诊断只采本任务进程及 PID、启动身份、名称、RSS/CPU 等必要字段；不导出全机

...
1. **同步并确定范围**
   ...
   - 读取 `.rally/zeroweb-regression-guard.last-success`。若其 SHA 等于当前 `HEAD`，直接输出 `DONE`，不启动构建、浏览器或额外 Agent 轮次。
...
2. **收集证据**
   ...
   - 按 [guard-policy.md](references/guard-policy.md) 执行核心门禁。测试、构建、reftest 和基准必须走 `docs/rally/run-rules.md` 规定的 test-guard 入口。
...
4. **最小修复**
   ...
   - 只修改与回归直接相关的代码；不得放宽阈值、覆盖较差基线、关闭检查或改测量配置来换取通过。
   - 为根因补最贴近的常驻回归测试。渲染兼容性修复按仓库规则补 WPT/reftest；深入排查得到的经验写入 `docs/learnings/`。
...
5. **验证、提交和通知**
   ...
   - 推送成功后更新本地成功 SHA，并发送飞书结果（根因、修改、验证、commit SHA）。无法安全修复、验证或推送时发送飞书卡点及下一步，不得声称已修复。

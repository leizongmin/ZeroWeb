...
#   2. 超阈值 → 全量删除 target/。不清「最近修改」子集的原因：partial
#      清理（按 mtime 挑目录删）极易破坏 cargo 增量编译一致性，下次构建反而
#      触发更多重编译产出更多中间产物；全量清后从头重建的产物集是已知
#      可控的。磁盘满的代价（连丢多次 rally 轮次）远高于一次全量重编译。
#
...
echo "target-disk-guard: target/ 占用 ${USED_GB}GB 超过阈值 ${LIMIT_GB}GB，执行清理 …"
# 直接删测量到的目录（等价 cargo clean，但自定义 CARGO_TARGET_DIR 时 cargo clean
# 会清错对象；且 cargo clean 在无 Cargo.toml 的上下文里不可用）
rm -rf "$TARGET_DIR"
echo "target-disk-guard: 清理完成（释放前 ${USED_GB}GB → 现应接近 0），继续执行原命令。"

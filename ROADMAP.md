...
- **M11/M12/M13 余面拆分（2026-09-12 新立三 goal）**: [desktop-browser](docs/goal/desktop-browser.md)——桌面浏览器产品化（真窗口主链路 + 导航/标签 + 内容工具 + 数据面）；[security-hardening](docs/goal/security-hardening.md)——CSP 完整实现 + Mixed Content + HSTS + 权限模型（WPT 三 corpus）；[web-api-batch2](docs/goal/web-api-batch2.md)——Clipboard + Fullscreen，✅ 2026-09-24 M4 收口（DC-1~4 ✅，终态 WPT clipboard-apis 83.6% / fullscreen 88.0%；DnD 因宿主拖拽输入管线深度依赖维持排除挂账）

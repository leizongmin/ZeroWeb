...
- **Render compact（恢复主动实施）**: WPT/CSSWG reftest 对齐 Chromium Oracle（`make reftest-oracle` 为诚实度量），上游 WPT reftest corpus **14556/16814 = 86.6%**（R4121 轮口径）；transform-box 属性三 crate 贯通 + svg 参考框换算（R4098–R4110）、Ahem 真实轮廓光栅化（R4115）、省略号单 U+2026（R4116）、background-position 负偏移（R4118）、@supports 裸 general-enclosed（R4120）等系列持续收口（R3936–R4121）；Phase A IFC / R1043 vertical-mode / R2174 border-box 仍等用户点名。详见 [docs/goal/rendering-compat/master.md](docs/goal/rendering-compat/master.md)
...
- **M12/M14 剩余面拆分（2026-09-07 用户决策，新立 6 子 goal）**: [storage-opfs](docs/goal/storage-opfs.md)（OPFS 真实化）、[page-wasm](docs/goal/page-wasm.md)（页面 WASM 深化）、[android-browser](docs/goal/android-browser.md)（Android 可用化）、webdriver（W3C 协议补齐，✅ 2026-09-08 完成，见 [archive/webdriver.md](docs/goal/archive/webdriver.md)）、[web-components](docs/goal/web-components.md)（Web Components 缺口补齐）、[event-loop-spec](docs/goal/event-loop-spec.md)（事件循环 spec 形态），均与 rendering-compat 声明 run-rules §9 碰撞边界
...
| M12 | Render compatibility / render-compact | `🚧 进行中` | 2026-08-04 起降频守成、2026-08-09 字体栈重建获批后恢复主动实施；WPT/CSSWG reftest 对齐 Chromium Oracle；**transform-box 三 crate 贯通 + svg 参考框（R4098–R4110）+ Ahem 真实轮廓光栅化/省略号/background-position 负偏移/@supports general-enclosed 系列（R3936–R4121，corpus 14556/16814 = 86.6%）**，详见「当前重点」 |

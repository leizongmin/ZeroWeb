...
- **Render compact（恢复主动实施）**: WPT/CSSWG reftest 对齐 Chromium Oracle（`make reftest-oracle` 为诚实度量），上游 WPT reftest corpus default **15091/16594 = 90.95%**（R4508 轮口径，首破 91 边）；**ruby-overhang gate 系列**（ZW_MIXED_BARE_TEXT default-on R4408、ruby-position 解析 R4412、rt-float hoist R4421 float-handling 翻绿）、pre-line 强制断行三站贯通（R4398–R4400）、字体栈与垂直度量重建大族（R4365–R4379）、content-height 严格翻绿系列（R4379–R4384）、ruby-overhang slice B 收官（R4358–R4363）、display:run-in 深挖大族（R4324–R4344）、DC-14 strict oracle 基线（R4348）**ruby-overhang 模型 flip default-on**（R4422）与 **vertical writing modes 线开动**（vrl 根盒右缘锚定、sideways-lr 字形取向、正交流 fill 臂、vertical line_height 转置、vertical float、cascade 别名消解等，R4425–R4463）、**multicol-span-all 与 multicol 平衡线开动**（spanner 区域列平衡、nested spanner 扁平化 pre-step、auto-height balance 容器高对称写回——css-multicol 288→299，R4499–R4508）等系列持续收口（R3936–R4508）；布局依赖值两遍样式解析深域已立案（R4189：cq 单位 / stretch 父 % 高 / line-clamp 约束链三族同构，等用户点名）；Phase A IFC / R2174 border-box 仍等用户点名。详见 [docs/goal/rendering-compat/master.md](docs/goal/rendering-compat/master.md)
...
| M12 | Render compatibility / render-compact | `🚧 进行中` | 2026-08-04 起降频守成、2026-08-09 字体栈重建获批后恢复主动实施；WPT/CSSWG reftest 对齐 Chromium Oracle；**ruby-overhang flip default-on/vertical writing modes 线开动/pre-line 强制断行/字体栈垂直度量重建/multicol-span-all 与平衡线系列（R3936–R4508，corpus default 15091/16594 = 90.95%）**，详见「当前重点」 |

...
- **Render compact（恢复主动实施）**: WPT/CSSWG reftest 对齐 Chromium Oracle（`make reftest-oracle` 为诚实度量），上游 WPT reftest corpus default **15175/16594 = 91.45%**（R4762 轮口径，fail 账册 1419 恒值）；**ruby-overhang gate 系列**（ZW_MIXED_BARE_TEXT default-on R4408、ruby-position 解析 R4412、rt-float hoist R4421 float-handling 翻绿）、pre-line 强制断行三站贯通（R4398–R4400）、字体栈与垂直度量重建大族（R4365–R4379）、content-height 严格翻绿系列（R4379–R4384）、ruby-overhang slice B 收官（R4358–R4363）、display:run-in 深挖大族（R4324–R4344）、DC-14 strict oracle 基线（R4348）**ruby-overhang 模型 flip default-on**（R4422）与 **vertical writing modes 线开动**（vrl 根盒右缘锚定、sideways-lr 字形取向、正交流 fill 臂、vertical line_height 转置、vertical float、cascade 别名消解等，R4425–R4463）、**multicol-span-all / multicol 平衡与 region-distribution 线**（spanner 区域列平衡、nested spanner 扁平化 pre-step、auto-height balance 容器高对称写回、region-distribution 分段与收缩兄弟重排收割 004a/b/006 族、溢出语义第三律，R4499–R4524）、**边框/裁剪渲染线**（clip 多边形条带 v2、per-layer clip border 墨迹环带、border-shape slice 2 内容/replaced 内形状裁剪、corner-shape × overflow 内形状裁剪、clip-path 覆盖改写 z 序原位修复——corner-shape 凹角簇解锁，R4525–R4547）、**background-clip:text 恒色染色线**（clip-text 族 12→4 fail，R4548–R4553）、**尺寸关键字与垂直 width 族线**（块轴 content 关键字求解、paint Path B 直系文本度量种子、垂直 width 族块轴镜像臂——vert-1 翻绿 0.00%，R4556–R4568）、**corner-shape 族延伸与圆角裁剪边勘察线**（canvas Path2D 绘制期 CTM 修复、伪元素盒化 gate 切片、斜杠 border-radius 两轴存储、圆角裁剪边协同试作耦合定谳回退专项挂账，R4570–R4579）、R3928 翻负真因修复与 **@container style() 廉价子切片**（R4581/R4582）、**font-size 计算值确定性线**（渲染双相根因修复：预应用 pass 消除 HashMap 迭代序依赖 + em/percentage 归一化以 Px 存储，R4642/R4643）等系列持续收口（R3936–R4762）；布局依赖值两遍样式解析深域已立案（R4189：cq 单位 / stretch 父 % 高 / line-clamp 约束链三族同构，等用户点名）；Phase A IFC / R2174 border-box 仍等用户点名。详见 [docs/goal/rendering-compat/master.md](docs/goal/rendering-compat/master.md)
...
- **HTML 编辑与键盘（三 goal 已完成，2026-09-08 收口归档）**: editing——WPT selection 套件 45P→2994P/5F（99.8%）、execCommand bold/italic/underline/delete/insertHTML/insertParagraph 实应用 + queryCommandState/Supported/Enabled 真实反射、CE 键入/删除/Enter 换行落 DOM（DC-1~5 ✅）；keyboard——keydown 默认动作分发层、隐式提交 default button 三规则（implicit-submission 3/3 全 Pass）、Space=keyup/Enter=keydown 激活全语义、select/radio 导航 + type-ahead、Esc dialog cancel/close（DC-1~5 ✅）；page-scrolling——滚动键全键位双面接通 + scrollIntoView 选项面 + scroll 事件语义（DC-1/3/5 ✅，DC-2/4 跨域 defer 口径收口）。残余跨域项（renderer S3 布局几何 R3298 协调点）由渲染流域主导。详见 [docs/goal/archive/editing-contenteditable/master.md](docs/goal/archive/editing-contenteditable/master.md)、[docs/goal/archive/keyboard-default-actions/master.md](docs/goal/archive/keyboard-default-actions/master.md)、[docs/goal/archive/keyboard-page-scrolling/master.md](docs/goal/archive/keyboard-page-scrolling/master.md)
- **M12/M14 剩余面拆分（2026-09-07 用户决策，新立 6 子 goal）**: [storage-opfs](docs/goal/archive/storage-opfs.md)（OPFS 真实化，✅ 2026-09-09 完成归档——DC-1~4 ✅）、[page-wasm](docs/goal/archive/page-wasm.md)（页面 WASM 深化，✅ 2026-09-12 完成归档——DC-1~4 ✅，WPT jsapi 31 案 99.4%）、[android-browser](docs/goal/android-browser.md)（Android 可用化，✅ 2026-09-13 DONE——DC-1~4 ✅，多标签/视口/IME/下载/通知/双语无障碍/CI 模拟器冒烟 + chaos 断连恢复）、webdriver（W3C 协议补齐，✅ 2026-09-08 完成，见 [docs/goal/archive/webdriver.md](docs/goal/archive/webdriver.md)（控制面 [archive/webdriver/](docs/goal/archive/webdriver/master.md)）；screenshot 续作 webdriver-screenshot goal ✅ 2026-09-09 完成归档，见 [docs/goal/archive/webdriver-screenshot.md](docs/goal/archive/webdriver-screenshot.md)）、[web-components](docs/goal/archive/web-components.md)（Web Components 缺口补齐，✅ 2026-09-11 完成归档——DC-1~5 ✅）、event-loop-spec（事件循环 spec 形态，✅ 2026-09-12 完成，见 [docs/goal/archive/event-loop-spec.md](docs/goal/archive/event-loop-spec.md)（控制面 [archive/event-loop-spec/](docs/goal/archive/event-loop-spec/master.md)）），均与 rendering-compat 声明 run-rules §9 碰撞边界
- **自动化与调试面（2026-09-12 新立两 goal）**: [cdp-protocol](docs/goal/cdp-protocol.md)——CDP 协议兼容 + Playwright 验证矩阵，✅ 2026-09-16 M5 分支 B 定稿收口 DONE（命令账本验收，headless.rs CDP 雏形收敛；S13–S39 落地 FetchObserved Network 观测/keyboard 全选/子帧元探测，Playwright 绿步 6→33，后继静默监测轮维持）；[devtools](docs/goal/devtools.md)——复用 Chrome DevTools frontend 的调试面板，✅ 2026-09-17 DONE 转守成（DC-1~3 ✅：bundle pin + serve 骨架、Elements 活 DOM + Computed 侧栏、Console REPL、Network 请求行演示流 + cookie 面板、GUI 模式 CDP server 可开关；GUI 标签页桥接等碰头协调项挂账）
- **M11/M12/M13 余面拆分（2026-09-12 新立三 goal）**: [desktop-browser](docs/goal/desktop-browser.md)——桌面浏览器产品化（真窗口主链路 + 导航/标签 + 内容工具 + 数据面）；[security-hardening](docs/goal/security-hardening.md)——CSP 完整实现 + Mixed Content + HSTS + 权限模型（WPT 三 corpus），✅ 2026-09-25 R11 终判 DONE（DC-1~4 ✅）——CSP 运行时强制接线 default-on（meta 装配 + script/style/img 元素与运行时/attr/connect/eval 全检查点 + `SecurityPolicyViolationEvent` 事件，csp corpus subtests 16.3%→28.8%、74/445 案全绿零丢失）+ Mixed Content 分级阻止 default-on + HSTS 响应注册 + `navigator.permissions` 语义层 + change 事件；[web-api-batch2](docs/goal/web-api-batch2.md)——Clipboard + Fullscreen，✅ 2026-09-24 M4 收口（DC-1~4 ✅，终态 WPT clipboard-apis 83.6% / fullscreen 88.0%；DnD 因宿主拖拽输入管线深度依赖维持排除挂账）
- **WPT corpus 分域推进（2026-09-12/13 新立九 goal，rendering-compat 协同）**: [svg-compat](docs/goal/svg-compat.md)（SVG 文档面）、[net-api-compat](docs/goal/net-api-compat.md)（fetch/XHR/URL/Streams/EventSource）、[navigation-compat](docs/goal/navigation-compat.md)（history/navigation/iframe）、[workers-compat](docs/goal/workers-compat.md)（Dedicated Worker）、[timing-animation-compat](docs/goal/timing-animation-compat.md)（hr-time/performance-timeline/WAAPI）、[encoding-compat](docs/goal/encoding-compat.md)（TextEncoder/TextDecoder）、[html-syntax-compat](docs/goal/html-syntax-compat.md)（html/syntax + html/dom）、[uievents-compat](docs/goal/uievents-compat.md)（uievents/pointerevents）、[webgl-compat](docs/goal/webgl-compat.md)（远期深度门控）——按 WPT 目录分域立项，与 M12 渲染线声明 run-rules §9 碰撞边界
...
| M11 | 浏览器产品层 | `🚧 进行中` | `browser-shell`、标签页、地址栏、历史、书签、下载、设置等基础逐步落地；[desktop-browser goal](docs/goal/desktop-browser.md)（2026-09-12 立项）推进真窗口主链路与产品验收 |
| M12 | Render compatibility / render-compact | `🚧 进行中` | 2026-08-04 起降频守成、2026-08-09 字体栈重建获批后恢复主动实施；WPT/CSSWG reftest 对齐 Chromium Oracle；**ruby-overhang flip default-on/vertical writing modes 线开动/pre-line 强制断行/字体栈垂直度量重建/multicol-span-all 与 region-distribution/边框裁剪与 clip:text 染色/垂直 width 族/corner-shape 延伸与 @container style()/font-size 计算值确定性系列（R3936–R4762，corpus default 15175/16594 = 91.45%）**，详见「当前重点」 |
...
1. **拆分子 goal 推进**（desktop-browser 与九个 WPT 分域 compat goal 推进中；自动化/调试与安全面已收口——cdp-protocol 2026-09-16 DONE、devtools 2026-09-17 DONE 转守成、web-api-batch2 2026-09-24 M4 收口、security-hardening 2026-09-25 R11 终判 DONE；M12/M14 首批六子 goal 全部收口——webdriver 2026-09-08、webdriver-screenshot 与 storage-opfs 2026-09-09、web-components 2026-09-11、page-wasm 与 event-loop-spec 2026-09-12、android-browser 2026-09-13 均已完成归档或达成 DONE）
...
- WebRTC
- 浏览器扩展系统
- 首期移动端发布（Android 可用化 goal 已完成 2026-09-13：Kotlin chrome + Rust JNI 桥 28 导出面、多标签/IME/下载/通知/双语无障碍、CI 模拟器冒烟 + chaos 断连恢复；真机验收与完整移动端发布仍不在近期主线）

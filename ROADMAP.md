...
- **Render compact（恢复主动实施）**: WPT/CSSWG reftest 对齐 Chromium Oracle（`make reftest-oracle` 为诚实度量），上游 WPT reftest corpus default **15135/16594 = 91.20%**（R4539 轮口径）；**ruby-overhang gate 系列**（ZW_MIXED_BARE_TEXT default-on R4408、ruby-position 解析 R4412、rt-float hoist R4421 float-handling 翻绿）、pre-line 强制断行三站贯通（R4398–R4400）、字体栈与垂直度量重建大族（R4365–R4379）、content-height 严格翻绿系列（R4379–R4384）、ruby-overhang slice B 收官（R4358–R4363）、display:run-in 深挖大族（R4324–R4344）、DC-14 strict oracle 基线（R4348）**ruby-overhang 模型 flip default-on**（R4422）与 **vertical writing modes 线开动**（vrl 根盒右缘锚定、sideways-lr 字形取向、正交流 fill 臂、vertical line_height 转置、vertical float、cascade 别名消解等，R4425–R4463）、**multicol-span-all / multicol 平衡与 region-distribution 线**（spanner 区域列平衡、nested spanner 扁平化 pre-step、auto-height balance 容器高对称写回、region-distribution 分段与收缩兄弟重排收割 004a/b/006 族、溢出语义第三律，R4499–R4524）、**边框/裁剪渲染线**（background-clip:text 实底彩字、per-layer clip border 墨迹环带、border-shape slice、clip-path 覆盖改写 z 序原位修复——corner-shape 凹角簇 10 案翻绿，R4525–R4539）等系列持续收口（R3936–R4539）；布局依赖值两遍样式解析深域已立案（R4189：cq 单位 / stretch 父 % 高 / line-clamp 约束链三族同构，等用户点名）；Phase A IFC / R2174 border-box 仍等用户点名。详见 [docs/goal/rendering-compat/master.md](docs/goal/rendering-compat/master.md)

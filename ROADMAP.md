...
- **Render compact（恢复主动实施）**: WPT/CSSWG reftest 对齐 Chromium Oracle（`make reftest-oracle` 为诚实度量），上游 WPT reftest corpus default **15175/16594 = 91.45%**（R4744 轮口径）；**ruby-overhang gate 系列**（ZW_MIXED_BARE_TEXT default-on R4408、ruby-position 解析 R4412、rt-float hoist R4421 float-handling 翻绿）、pre-line 强制断行三站贯通（R4398–R4400）、字体栈与垂直度量重建大族（R4365–R4379）、content-height 严格翻绿系列（R4379–R4384）、ruby-overhang slice B 收官（R4358–R4363）、display:run-in 深挖大族（R4324–R4344）、DC-14 strict oracle 基线（R4348）**ruby-overhang 模型 flip default-on**（R4422）与 **vertical writing modes 线开动**（vrl 根盒右缘锚定、sideways-lr 字形取向、正交流 fill 臂、vertical line_height 转置、vertical float、cascade 别名消解等，R4425–R4463）、**multicol-span-all / multicol 平衡与 region-distribution 线**（spanner 区域列平衡、nested spanner 扁平化 pre-step、auto-height balance 容器高对称写回、region-distribution 分段与收缩兄弟重排收割 004a/b/006 族、溢出语义第三律，R4499–R4524）、**边框/裁剪渲染线**（clip 多边形条带 v2、per-layer clip border 墨迹环带、border-shape slice 2 内容/replaced 内形状裁剪、corner-shape × overflow 内形状裁剪、clip-path 覆盖改写 z 序原位修复——corner-shape 凹角簇解锁，R4525–R4547）、**background-clip:text 恒色染色线**（clip-text 族 12→4 fail，R4548–R4553）、**尺寸关键字与垂直 width 族线**（块轴 content 关键字求解、paint Path B 直系文本度量种子、垂直 width 族块轴镜像臂——vert-1 翻绿 0.00%，R4556–R4568）、**corner-shape 族延伸与圆角裁剪边勘察线**（canvas Path2D 绘制期 CTM 修复、伪元素盒化 gate 切片、斜杠 border-radius 两轴存储、圆角裁剪边协同试作耦合定谳回退专项挂账，R4570–R4579）、R3928 翻负真因修复与 **@container style() 廉价子切片**（R4581/R4582）、**font-size 计算值确定性线**（渲染双相根因修复：预应用 pass 消除 HashMap 迭代序依赖 + em/percentage 归一化以 Px 存储，R4642/R4643）等系列持续收口（R3936–R4651）；布局依赖值两遍样式解析深域已立案（R4189：cq 单位 / stretch 父 % 高 / line-clamp 约束链三族同构，等用户点名）；Phase A IFC / R2174 border-box 仍等用户点名。详见 [docs/goal/rendering-compat/master.md](docs/goal/rendering-compat/master.md)
...
- **自动化与调试面（2026-09-12 新立两 goal）**: [cdp-protocol](docs/goal/cdp-protocol.md)——CDP 协议兼容 + Playwright 验证矩阵（命令账本验收，headless.rs CDP 雏形收敛；S13–S39 落地 FetchObserved Network 观测/keyboard 全选/子帧元探测，Playwright 绿步 6→33）；[devtools](docs/goal/devtools.md)——复用 Chrome DevTools frontend 的调试面板，✅ 2026-09-17 DONE 转守成（DC-1~3 ✅：bundle pin + serve 骨架、Elements 活 DOM + Computed 侧栏、Console REPL、Network 请求行演示流 + cookie 面板、GUI 模式 CDP server 可开关；GUI 标签页桥接等碰头协调项挂账）
- **M11/M12/M13 余面拆分（2026-09-12 新立三 goal）**: [desktop-browser](docs/goal/desktop-browser.md)——桌面浏览器产品化（真窗口主链路 + 导航/标签 + 内容工具 + 数据面）；[security-hardening](docs/goal/security-hardening.md)——CSP 完整实现 + Mixed Content + HSTS + 权限模型（WPT 三 corpus），M1 导入基线 + M2-s1–s5 CSP 运行时强制接线推进中（script/style/img 检查点 + connect-src 阻止族 + `SecurityPolicyViolationEvent` 事件，csp corpus subtests 16.3%→28.9%）；[web-api-batch2](docs/goal/web-api-batch2.md)——Clipboard + Fullscreen，✅ 2026-09-24 M4 收口（DC-1~4 ✅，终态 WPT clipboard-apis 83.6% / fullscreen 88.0%；DnD 因宿主拖拽输入管线深度依赖维持排除挂账）

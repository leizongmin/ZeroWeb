...
| M12 | Render compatibility / render-compact | `🚧 进行中` | 2026-08-04 起降频守成、2026-08-09 字体栈重建获批后恢复主动实施；WPT/CSSWG reftest 对齐 Chromium Oracle；**line-clamp / SVG intrinsic 系列（R3762-R3770b）：`line-clamp: auto` 约束钳制 + 跨块 clamp（layout/paint 双侧）+ SVG ComputedIntrinsic**，详见「当前重点」 |
| M13 | 完整 JS/DOM API 兼容性 | `🚧 进行中` | 当前主线：P1a DOM/JS Bridge 原生化已主体落地（R30xx 系列），P1b V8 原生 DOM 绑定接续推进（R3095 起，native 替代 polyfill 字符串桥；S5 customElements/Web Components 里程碑已完，续 DOM/CSS 选择器一致化）；**js-dom M3 lit 框架端到端闭环（R97-R99）+ Vue e2e 双引擎 parity（R339，DC-2 QuickJS 同页对齐达成）**，**M4 续作（R100-R339）**：真实 Vue 3 端到端（R100）、M4 events/nodes 大簇收口（R104-R186）、M1 L2 原生查询切片（R163-R171）、iframe 子文档脚本执行（R206）、**Range 全序复刻 + live-range 迁移（R208-R265，mutations 八套件全 100%）**、**Range 深水区（R266-R289：delete/clone/extract 三套件 100%）**、**selector 工厂域 identity 归一（R290-R311：Element-matches 675P/0F、querySelector 全族 2055P/0F）**、L2 查询归并 + 执行路径测绘（R319-R331：adoptNode/template inert/change-document-HTMLNess 全文件转绿、vue_reconciliation 复活）、**MutationObserver 全族清零（R332-R336：childList 38/38 全 Pass，4 个文件级 Timeout 全消除）**；完整 Web API 兼容性仍在此阶段扩展 |

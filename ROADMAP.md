...
| M13 | 完整 JS/DOM API 兼容性 | `🚧 进行中` | 当前主线：P1a DOM/JS Bridge 原生化已主体落地（R30xx 系列），P1b V8 原生 DOM 绑定接续推进（R3095 起，native 替代 polyfill 字符串桥；S5 customElements/Web Components 里程碑已完，续 DOM/CSS 选择器一致化）；**js-dom M3 lit 框架端到端闭环（R97-R99）**，**M4 续作（R100-R294）**：真实 Vue 3 端到端（R100）、M4 events/nodes 大簇收口（R104-R186）、M1 L2 原生查询切片（R163-R171）、iframe 子文档脚本执行（R206）、**Range extract/surround/insertNode 全序复刻（R208-R259，surroundContents 1840P/0F 100%）**、**live-range 边界迁移系列（R260-R265：removeChild/appendChild/replaceChild/insertBefore 全迁移——mutations 域八套件全 100%）**、**Range 深水区全序复刻（R266-R289：deleteContents/cloneContents/extractContents 三套件 100%、compareBoundaryPoints 592F→0F、dom 全量 53733P→54041P）**、nodes/querySelector/insertAdjacent/MutationObserver 收口（R290-R294，**dom 全量 54070P/89F**）；完整 Web API 兼容性仍在此阶段扩展 |

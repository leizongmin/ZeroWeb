...
- **Render compact（恢复主动实施）**: WPT/CSSWG reftest 对齐 Chromium Oracle（`make reftest-oracle` 为诚实度量），上游 WPT reftest corpus **14765/16594 = 89.0%**（R4322 轮口径）；**inline 子树 flatten walk 系列 default-on 翻转**（R4299–R4313）、**counter-style 族**（ethiopic-numeric / CJK 万亿组系 / Hebrew bidi / 隐式值域·additive，R4314–R4321）、**S11 style-key 缓存继承态碰撞修复**（R4322）、filter region 外扩 + backdrop blur 修复（R4291–R4293）、auto 表格列宽压缩（R4298）、margin-trim 通用化族（R4250–R4255）、`::scroll-marker` / `::scroll-button`（R4256–R4264）、CSS filter url() 引用链与 alpha 保真（R4273–R4284）等系列持续收口（R3936–R4322）；布局依赖值两遍样式解析深域已立案（R4189：cq 单位 / stretch 父 % 高 / line-clamp 约束链三族同构，等用户点名）；Phase A IFC / R1043 vertical-mode / R2174 border-box 仍等用户点名。详见 [docs/goal/rendering-compat/master.md](docs/goal/rendering-compat/master.md)
...
| M12 | Render compatibility / render-compact | `🚧 进行中` | 2026-08-04 起降频守成、2026-08-09 字体栈重建获批后恢复主动实施；WPT/CSSWG reftest 对齐 Chromium Oracle；**inline walk default-on/counter-style 族/S11 缓存修复/margin-trim 通用化/corner-shape/scroll-marker·scroll-button/filter url() 引用链/alpha 保真系列（R3936–R4322，corpus 14765/16594 = 89.0%）**，详见「当前重点」 |

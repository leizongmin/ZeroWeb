...
- **Render compact（恢复主动实施）**: WPT/CSSWG reftest 对齐 Chromium Oracle（`make reftest-oracle` 为诚实度量），上游 WPT reftest corpus **14742/16594 = 88.8%**（R4290 轮口径）；margin-trim 通用化族（R4250–R4255）、corner-shape 特性切片（R4248）、`::scroll-marker` / `::scroll-button` 函数伪元素与组盒合成（R4256–R4264）、`revert-layer` / @layer 层序（R4245/R4246）、**CSS filter url(#id) 引用 SVG filter 输出链**（R4273–R4280）、**CPU 光栅 alpha 保真三切片**（R4282–R4284）、SVG presentation CSS 注入（R4280）、svg 原子 inline 尺寸（R4290）等系列持续收口（R3936–R4290）；布局依赖值两遍样式解析深域已立案（R4189：cq 单位 / stretch 父 % 高 / line-clamp 约束链三族同构，等用户点名）；Phase A IFC / R1043 vertical-mode / R2174 border-box 仍等用户点名。详见 [docs/goal/rendering-compat/master.md](docs/goal/rendering-compat/master.md)
...
- **自动化与调试面（2026-09-12 新立两 goal）**: [cdp-protocol](docs/goal/cdp-protocol.md)——CDP 协议兼容 + Playwright 验证矩阵（命令账本验收，headless.rs CDP 雏形收敛）；[devtools](docs/goal/devtools.md)——复用 Chrome DevTools frontend 的调试面板（Network/Console/Elements/Application，启动门控 = cdp-protocol M3）
...
| M12 | Render compatibility / render-compact | `🚧 进行中` | 2026-08-04 起降频守成、2026-08-09 字体栈重建获批后恢复主动实施；WPT/CSSWG reftest 对齐 Chromium Oracle；**margin-trim 通用化/corner-shape/scroll-marker·scroll-button/revert-layer/filter url() 引用链/alpha 保真/svg 原子 inline 尺寸系列（R3936–R4290，corpus 14742/16594 = 88.8%）**，详见「当前重点」 |

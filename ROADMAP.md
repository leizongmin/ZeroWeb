...
| M12 | Render compatibility / render-compact | `🚧 进行中` | 2026-08-04 起降频守成、2026-08-09 字体栈重建获批后恢复主动实施；WPT/CSSWG reftest 对齐 Chromium Oracle；**table 布局 / 3D transform / bidi-override / display:contents 系列（R3809-R3845，corpus 13568→16814，含 R3844 css/display 新 surface 导入 +213）**，详见「当前重点」 |
| M13 | 完整 JS/DOM API 兼容性 | `🚧 进行中` | 当前主线：P1b V8 原生 DOM 绑定（R3095 起）**M1-M5 已达成（R383/R384：V8 `native_dom` default-on land + kill-switch 删除，DC-1 双达成；QuickJS 同款 default-on，M7）**，js-dom goal 已知 Fail 集 10→6；**M3 框架端到端闭环（R97-R99 lit + R100 Vue 3 + R339 DC-2 QuickJS parity）**，**M4 续作（R100-R384）**：events/nodes 大簇收口（R104-R186）、**Range 全序复刻 + live-range 迁移（R208-R265，mutations 八套件全 100%）**、**Range 深水区（R266-R289）**、**selector 工厂域 identity 归一（R290-R311）**、L2 查询归并 + 执行路径测绘（R319-R331）、**MutationObserver 全族清零（R332-R336）**、M4 深收口（R344-R368：Range-mutations data 族 Timeout 根治 +1333P、L2-d3 RFC v0.3、CE registry per-realm 三片——sweep 55493P）、域导入 + pending-apply RFC + pa2 代际令牌（R370-R384）；完整 Web API 兼容性仍在此阶段扩展 |

...
- **Render compact（恢复主动实施）**: WPT/CSSWG reftest 对齐 Chromium Oracle（`make reftest-oracle` 为诚实度量），上游 WPT reftest corpus **14664/16813 = 87.2%**（R4193 轮口径）；object/embed/applet 解码固有尺寸桥接（R4159）、ratio-only SVG × aspect-ratio 优先级族（R4160–R4165）、head 内注入文本节点渲染链（R4169）、cq 单位样式键缓存禁用与渐变解析（R4178/R4179）、float 提升钳制基准改假设流位置（R4176）、line-clamp:auto 家族（R4181–R4184）、单行 flex line cross 钳制（R4185）、margin-trim 自折叠臂补 height:auto 空元素（R4191）、contain:inline-size 内联尺寸抑制（R4193）等系列持续收口（R3936–R4193）；布局依赖值两遍样式解析深域已立案（R4189：cq 单位 / stretch 父 % 高 / line-clamp 约束链三族同构，等用户点名）；Phase A IFC / R1043 vertical-mode / R2174 border-box 仍等用户点名。详见 [docs/goal/rendering-compat/master.md](docs/goal/rendering-compat/master.md)
...
- **M12/M14 剩余面拆分（2026-09-07 用户决策，新立 6 子 goal）**: [storage-opfs](docs/goal/archive/storage-opfs.md)（OPFS 真实化，✅ 2026-09-09 完成归档——DC-1~4 ✅）、[page-wasm](docs/goal/archive/page-wasm.md)（页面 WASM 深化，✅ 2026-09-12 完成归档——DC-1~4 ✅，WPT jsapi 31 案 99.4%）、[android-browser](docs/goal/android-browser.md)（Android 可用化）、webdriver（W3C 协议补齐，✅ 2026-09-08 完成，见 [docs/goal/archive/webdriver.md](docs/goal/archive/webdriver.md)（控制面 [archive/webdriver/](docs/goal/archive/webdriver/master.md)）；screenshot 续作 webdriver-screenshot goal ✅ 2026-09-09 完成归档，见 [docs/goal/archive/webdriver-screenshot.md](docs/goal/archive/webdriver-screenshot.md)）、[web-components](docs/goal/archive/web-components.md)（Web Components 缺口补齐，✅ 2026-09-11 完成归档——DC-1~5 ✅）、event-loop-spec（事件循环 spec 形态，✅ 2026-09-12 完成，见 [docs/goal/archive/event-loop-spec.md](docs/goal/archive/event-loop-spec.md)（控制面 [archive/event-loop-spec/](docs/goal/archive/event-loop-spec/master.md)）），均与 rendering-compat 声明 run-rules §9 碰撞边界
...
| M12 | Render compatibility / render-compact | `🚧 进行中` | 2026-08-04 起降频守成、2026-08-09 字体栈重建获批后恢复主动实施；WPT/CSSWG reftest 对齐 Chromium Oracle；**宽轴关键字/abspos stretch/object 固有尺寸桥/SVG ratio 优先级/cq 单位缓存/line-clamp 家族/单行 flex line 钳制/margin-trim 自折叠/contain:inline-size 系列（R3936–R4193，corpus 14664/16813 = 87.2%）**，详见「当前重点」 |
...
1. **M12/M14 拆分子 goal 推进**（android-browser 推进中；keyboard/editing 三 goal 已于 2026-09-08 收口归档，webdriver 已于 2026-09-08 收口归档，webdriver-screenshot 已于 2026-09-09 收口归档，storage-opfs 已于 2026-09-09 收口归档，web-components 已于 2026-09-11 收口归档，page-wasm 已于 2026-09-12 收口归档——WPT jsapi 31 案 99.4%，event-loop-spec 已于 2026-09-12 收口归档——IO/RO 基线 + MO host 触发 default-on + per-task checkpoint default-on）

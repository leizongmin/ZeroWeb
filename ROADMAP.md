...
- **Render compact（恢复主动实施）**: WPT/CSSWG reftest 对齐 Chromium Oracle（`make reftest-oracle` 为诚实度量），上游 WPT reftest corpus **14916 default / 14919 gate-on per 16594 = 89.9%**（R4421 轮口径）；**ruby-overhang gate 系列**（ZW_MIXED_BARE_TEXT default-on R4408、ruby-position 解析 R4412、rt-float hoist R4421 float-handling 翻绿）、pre-line 强制断行三站贯通（R4398–R4400）、字体栈与垂直度量重建大族（R4365–R4379）、content-height 严格翻绿系列（R4379–R4384）、ruby-overhang slice B 收官（R4358–R4363）、display:run-in 深挖大族（R4324–R4344）、DC-14 strict oracle 基线（R4348）等系列持续收口（R3936–R4421）；布局依赖值两遍样式解析深域已立案（R4189：cq 单位 / stretch 父 % 高 / line-clamp 约束链三族同构，等用户点名）；Phase A IFC / R1043 vertical-mode / R2174 border-box 仍等用户点名。详见 [docs/goal/rendering-compat/master.md](docs/goal/rendering-compat/master.md)
...
| M12 | Render compatibility / render-compact | `🚧 进行中` | 2026-08-04 起降频守成、2026-08-09 字体栈重建获批后恢复主动实施；WPT/CSSWG reftest 对齐 Chromium Oracle；**ruby-overhang gate 系列/pre-line 强制断行/字体栈垂直度量重建/content-height 翻绿系列（R3936–R4421，corpus 14916 default / 14919 gate-on = 89.9%）**，详见「当前重点」 |

...
- **HTML 编辑与键盘（keyboard/editing 三 goal 活跃）**: editing——WPT selection 套件 2993P、execCommand bold/italic/underline/delete/insertHTML/insertParagraph 实应用 + queryCommandState/Supported/Enabled 真实反射、CE 键入/删除/Enter 换行/innerHTML window named access 注册落 DOM（E2 切片 12，2026-09-08）；keyboard——Space/Enter/Esc/方向键/type-ahead 激活与导航、隐式表单提交、Ctrl+Home/End 与滚动键 JS 滚动默认动作（K2–K5 / M2–M3 切片持续落地）

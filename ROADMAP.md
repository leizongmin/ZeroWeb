...
- **媒体播放（解码管线起步）**: `crates/media`（zero-media）已落地——webm/Matroska demux、VP9 纯 Rust 解码、AV1（`decode-av1` feature）、音频解码（mp3/ogg-vorbis/opus/webm 音轨）、`VideoPlayer` 播放驱动、混音总线与 Web Audio 振荡器最小面；media-elements 语义面（状态机/事件/API 行为）WPT 529P/0F。fixture-mounted 播放面切片推进中；H.264 立项 RFC 待用户批复。详见 [docs/goal/media-playback/master.md](docs/goal/media-playback/master.md)

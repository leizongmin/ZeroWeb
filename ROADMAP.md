...
- **媒体播放（解码管线起步）**: `crates/media`（zero-media）已落地——webm/Matroska demux、VP9 纯 Rust 解码、AV1（`decode-av1` feature）、音频解码（mp3/ogg-vorbis/opus/webm 音轨）、`VideoPlayer` 播放驱动、混音总线与 Web Audio 振荡器最小面；media-elements 语义面（状态机/事件/API 行为）WPT 532P/0F。fixture-mounted 播放面切片推进中；H.264 立项 RFC 待用户批复。详见 [docs/goal/media-playback/master.md](docs/goal/media-playback/master.md)
...
| M12 | Render compatibility / render-compact | `🚧 进行中` | 2026-08-04 起降频守成、2026-08-09 字体栈重建获批后恢复主动实施；WPT/CSSWG reftest 对齐 Chromium Oracle；**display:contents 全布局域穿透 / aspect-ratio 语义簇 / ::first-letter / margin-trim 系列（R3846-R3874，wpt-runner mismatch 阈值对齐上游 0.01%）**，详见「当前重点」 |

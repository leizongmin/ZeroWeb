...
- **字体（字体栈重建切片 R3230-F–R3248-F）**：OpenType features 贯通（font-face feature defaults、CSS feature precedence、shaping 携 feature）、generic layout advance 统一（fontdue/rustybuzz 对齐）、ordered fallback faces 全链路（shaping→IFC→layout→paint）、shaped fallback default-on（R3243-F）、**CSS Fonts 4 two-value `font-size-adjust` 全栈贯通（R3245-F，css-fonts Oracle 净改善 14.34pp）**、font-synthesis 属性（R3248-F）、font-size 绝对关键字 + HTML `<font size>` 映射（R3247-F）；`unicode-segmentation`（pin =1.12.0）依赖加入。

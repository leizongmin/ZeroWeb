...
- **浏览器二进制瘦身**：发布版 `zero-browser` 不再编译进程内 `tab_worker`、页面脚本 worker 和页面脚本调度，也不再链接 `zero-webview` / V8；headless 的导航、内联 HTML、脚本执行及截图改由 `zero-renderer` IPC 驱动，browser 只保留宿主、网络代理、绘制快照导入和 chrome 呈现职责。默认 Windows release 从 71.4 MB 降至 19.2 MB；renderer 继续独占 V8 页面脚本执行。

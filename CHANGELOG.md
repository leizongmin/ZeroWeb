...
- **Service Worker 控制语义推进（service-workers M3-4，2026-08-20）**：claim 匹配 document clients、document controller 投影（`navigator.serviceWorker.controller`）、page→worker 消息派发（shim + `ServiceWorkerManager` + webview 测试资产）——M3 控制语义 + page-to-worker message 完成；M2 fetch 拦截仍受 js-dom fetch 改造门控。
...
- 测试规模从约 1,000 增长到 16,000+（2026-08-20 静态统计 16,419 个 `#[test]`，v8 feature 全量全绿）。

...
- **Service Worker 控制语义推进（service-workers M3-4 → M3-22，2026-08-20/21）**：claim 匹配 document clients、document controller 投影（`navigator.serviceWorker.controller`）、page→worker 消息派发（shim + `ServiceWorkerManager` + webview 测试资产）——M3 控制语义 + page-to-worker message 完成；**update cache matrix 完成（M3，真实 update job/script byte comparison/updatefound）+ dynamic import update（M3-22，worker 内动态 import 与 update 流程对齐）**；**SW runtime CacheStorage open/put/match 桥接完成（2026-08-22，SW 侧 `caches` 走真实 Rust 存储）**；**CacheStorage WPT 基线接入（2026-08-23：cache-match WPT baseline + serviceworker CacheStorage wrappers 扩充）**；M2 fetch 拦截仍受 js-dom fetch 改造门控。
...
- 测试规模从约 1,000 增长到 16,000+（2026-08-23 静态统计 16,764 个 `#[test]`，v8 feature 全量全绿）。

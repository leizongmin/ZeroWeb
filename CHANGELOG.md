...
- **CSS 简写语法校验批量（R3470-F–R3482-F）**：scroll edge、grid-template、border-image 组件、text-emphasis、font-variant、columns 宽度、background size/attachment、transition/animation 的 fallback ident、iteration count 与 duration grammar 等无效简写拒绝/硬化（css-parser → style-system 双路径），持续收口 CSS 简写解析与 Chromium 行为差距。
...
- 测试规模从约 1,000 增长到 16,000+（2026-08-19 静态统计 16,282 个 `#[test]`，v8 feature 全量全绿）。

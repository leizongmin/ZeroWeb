...
**日期**：2026-08-07
**相关模块**：`tests/wpt-runner/src/reftest.rs`（`render_to_framebuffer_gpu_with_base`）、`tests/wpt-runner/src/main.rs`（`effective_jobs`）、`crates/render-foundation/src/gpu/renderer/mod.rs`（`GpuRenderer` / `GPU_CREATE_MUTEX`）

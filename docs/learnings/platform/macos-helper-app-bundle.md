...
- 日期：2026-08-08（2026-08-17 更新）
- 相关模块：`scripts/package-macos.sh`、`apps/browser/src/process_backend.rs`、`apps/browser/src/compositor_client.rs`、`crates/webview/src/image_decoder.rs`
...
将 renderer、compositor 和 image-decoder 分别包装为 `ZeroBrowser.app/Contents/Frameworks/ZeroBrowser Helper (<Role>).app`。每个 Helper 都设置独立的 `CFBundleName`、`CFBundleDisplayName`、`CFBundleIdentifier` 和 `CFBundleIconFile`，并复用主应用的 `.icns`。browser 和 renderer 优先查找对应 Helper 内的可执行文件，同时保留同目录裸二进制作为本地开发和旧布局兜底。

Renderer Helper 启动后必须在主线程运行 AppKit event loop，并使用 `NSApplicationActivationPolicyAccessory` 与 `LSUIElement=true` 注册为 UIElement application。原 renderer runtime 移到工作线程，stdin/stdout pipe IPC 保持不变。注册完成后，`lsappinfo` 能返回 Helper 的 bundle id、display name 和 ASN，Activity Monitor 显示图标，同时 Dock 不出现额外图标。compositor 与 image-decoder 是无 UI 后台进程，本次只使用独立 Bundle 承载发布元数据和签名。
...
签名必须由内向外执行：所有 Helper 可执行文件、所有 Helper bundle、主应用 bundle。否则修改嵌套内容会使外层代码签名失效。

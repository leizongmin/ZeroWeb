...
- 门禁：`cargo build --workspace` + clippy `-D warnings` + fmt 全净；foundation/text 27 + ui/render **15** + ui/testing 5(+3) + ui/widgets **33** + browser-ui/chrome **48** + ui/runtime **22**(+8 focusscope/semantics) + ui/adapters/webview **8** + ui/examples **5** + ui/dsl **64**(+8 i18n bridge) + ui/i18n **22**(+9 诊断/locale plural/RTL) + ui/core **38**(+3 focus) tests 全绿；DC-17 聚合下轮统一重测（本轮新增 ~330 行，预期 per-crate 仍 ≥85%）。
...
| DC-14 | 浏览器迁移完成 + 零退化 | 🟡 counter + form 示例已立（M2） | **counter + form 示例可构建/运行、不依赖浏览器 crate**（`ui/examples`：counter retained 闭环；form 受控文本输入 + 焦点/键盘 + 校验）；剩余：browser-shell-demo 示例、apps/browser 逐组件灰度迁移（product-smoke 零退化硬门禁） |
| DC-15 | 移动端运行时 | ⬜ M4 | — |
...
1. **`ui/examples` browser-shell-demo（DC-14）**：组合 browser-ui/chrome 组件 + render 桥 + ScrollBar + WebViewWidget 占位（不接真实浏览器进程），用 `WidgetHost` 跑 retained 闭环 + `semantics()`/`enter_focus_scope` 验证浏览器 chrome 组合力（含 a11y 树/焦点 trap）。SDK-only。
2. **DC-6 phase-5（DSL command·route·overlay·asset 引用）**：DSL 声明式引用 `ui/commands` ActionId、`ui/navigation` route、`ui/overlay` open/close、`ui/assets` 资源路径（接 WidgetSpec 的 actions/bindings 字段，sandbox 内）。SDK-only。
...
6. **DC-6 phase-5（DSL command·route·overlay·asset 引用）**：DSL 声明式引用 `ui/commands` ActionId、`ui/navigation` route、`ui/overlay` open/close、`ui/assets` 资源路径（接 WidgetSpec 的 actions/bindings 字段，sandbox 内）。SDK-only。

...
- 门禁：`cargo build --workspace` + clippy `-D warnings` + fmt 全净；foundation/text 27 + ui/render **15** + ui/testing 5(+3) + ui/widgets **33** + browser-ui/chrome **53**(+5 shell_demo) + ui/runtime **22**(+8 focusscope/semantics) + ui/adapters/webview **8** + ui/examples **5** + ui/dsl **64**(+8 i18n bridge) + ui/i18n **22**(+9 诊断/locale plural/RTL) + ui/core **38**(+3 focus) tests 全绿；DC-17 聚合下轮统一重测（本轮新增 ~430 行，预期 per-crate 仍 ≥85%）。
...
**M3 进行中（DC-6）**：phase-1 表达式引擎 + phase-2 YAML→WidgetSpec loader + phase-3 map/filter（字段投影）+ phase-4 responsive branch（`responsive:` 节点按 AdaptiveBranch 选断点子树）已落地（见上）；剩余 = ①command·route·overlay·asset 引用；②嵌套路径投影 map/filter（follow-up）；③counter/form/browser-shell-demo 示例（counter+form 已立，DC-14）。
...
| DC-6 | YAML DSL + 完整表达式语言 | 🟡 phase-4（M3） | phase-1 表达式引擎（parse/typecheck/eval + sandbox + 资源上限，26 测）+ phase-2 YAML→WidgetSpec loader（22 测）+ phase-3 map/filter 字段投影（无 lambda，2 测）+ **phase-4 responsive branch**（`responsive:` 节点按 AdaptiveBranch 选断点子树，6 测）已落地；剩余：command·route·overlay·asset 引用、嵌套路径投影 map/filter |
...
1. **DC-6 phase-5（DSL command·route·overlay·asset 引用）**：DSL 声明式引用 `ui/commands` ActionId、`ui/navigation` route、`ui/overlay` open/close、`ui/assets` 资源路径（接 WidgetSpec 的 actions/bindings 字段，sandbox 内）。SDK-only。
2. **host flex 布局增强**：当前 Row/Column 按剩余空间顺序分配、无 flex，导致 demo 与 chrome 多填宽子节点争宽（DC-14 demo 已据此编排规避）；加 flex/grow 权重让 chrome toolbar 多子节点共存。SDK-only。
3. **DC-11 完整闭环：render-foundation 字体栈统一到 foundation/text**（使 zero-webview 也走 foundation/text；**触碰渲染后端，需 `make product-smoke`**）。
...
5. **apps/browser 逐组件灰度迁移（DC-14，product-smoke）**：用 browser-ui/chrome 组件 + render 桥替换 apps/browser 手绘 chrome（含 page_scroll.rs → ScrollBar 迁移，DC-14 零退化硬门禁）。

...
- 门禁：`cargo build --workspace` + clippy `-D warnings` + fmt 全净；foundation/text 27 + ui/render **15** + ui/testing 5(+3) + ui/widgets **33** + browser-ui/chrome **54**(+1 flex) + ui/runtime **33**(+6 flex) + ui/adapters/webview **8** + ui/examples **5** + ui/dsl **72** + ui/i18n **22** + ui/core **38** tests 全绿；DC-17 聚合下轮统一重测（本轮新增 ~120 行 host flex 代码，host.rs 92.29%，per-crate 仍 ≥85%）。

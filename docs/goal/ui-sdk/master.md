...
- 门禁：`cargo build --workspace` + clippy `-D warnings` + fmt 全净；foundation/text 27 + ui/render **15**(+ExternalSurface translate) + ui/testing 5(+3) + ui/widgets 29 + browser-ui/chrome **47**(+2 render) + ui/runtime **15**(+host layout-prop) + ui/adapters/webview **7**(+3 Widget) + ui/examples 2 + ui/dsl 48 tests 全绿；DC-17 聚合 **line 93.33% / function 92.90% / region 92.51%**（per-crate 全部 ≥85%；本轮 ExternalSurface/WebViewWidget 新增代码覆盖率预计 ≥85%，下轮统一重测）。
...
1. **DC-4 滚动语义 + ScrollBar**：把 `apps/browser/src/page_scroll.rs` 的滚动语义抽到 `ui/widgets::ScrollBar`（通用几何/视觉）+ `ScrollCommand` → `apply_scroll_command`（已有 scroll_bridge）→ WebViewWidget.scroll（DC-3 已接 scroll 事件）。补 hit-test + command 测试；与 WebViewWidget scroll 度量联动。这是与 DC-3 联动的下一 SDK-only 缺口。
2. **`ui/examples` form/browser-shell-demo（DC-14）**：counter 已立；续做 form（多控件 + 校验 + 提交）与 browser-shell-demo（组合 browser-ui/chrome 组件 + render 桥，不接真实浏览器进程）示例。

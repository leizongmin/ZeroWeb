...
- 门禁：`cargo build --workspace` + clippy `-D warnings` + fmt 全净；foundation/text 27 + ui/render **15** + ui/testing 5(+3) + ui/widgets **33** + browser-ui/chrome **48**(+1 themed) + ui/runtime **16**(+1 theme flow) + ui/adapters/webview **8** + ui/examples 2 + ui/dsl 48 + ui/core **35**(+4 theme: color_for/contrast/wcag/baseline) tests 全绿；DC-17 聚合 **line 93.33% / function 92.90% / region 92.51%**（per-crate 全部 ≥85%；本轮 theme 新增代码覆盖率预计 ≥85%，下轮统一重测）。
...
1. **DC-8 焦点/键盘路由**（⬜→🟡）：当前 WidgetHost 只路由带位置事件（指针/滚动），非位置事件（键盘/焦点/IME）未路由。补焦点遍历（Tab 按声明顺序/traversal policy）+ 键盘事件路由到 focused widget + IME rect（TextInput caret）。**这是解锁 form 示例（文本输入）与浏览器迁移（快捷键/command palette）的关键缺口**，SDK-only。
2. **`ui/examples` form/browser-shell-demo（DC-14）**：counter 已立；form 需 DC-8 键盘路由（文本输入）；browser-shell-demo 组合 browser-ui/chrome 组件 + render 桥 + ScrollBar + WebViewWidget 占位（不接真实浏览器进程），验证组件组合力。

...
- 门禁：`cargo build --workspace` + clippy `-D warnings` + fmt 全净；foundation/text 27 + ui/render **15** + ui/testing 5(+3) + ui/widgets **33** + browser-ui/chrome **53**(+5 shell_demo) + ui/runtime **22**(+8 focusscope/semantics) + ui/adapters/webview **8** + ui/examples **5** + ui/dsl **72**(+8 phase-5 action/asset) + ui/i18n **22**(+9 诊断/locale plural/RTL) + ui/core **38**(+3 focus) tests 全绿；DC-17 聚合下轮统一重测（本轮新增 ~250 行，预期 per-crate 仍 ≥85%）。
...
1. **host flex 布局增强**：当前 Row/Column 按剩余空间顺序分配、无 flex，导致 demo 与 chrome 多填宽子节点争宽（DC-14 demo 已据此编排规避，真实 chrome toolbar 仍受影响）；加 flex/grow 权重（opt-in prop，默认 0=现行行为，向后兼容）让 chrome toolbar 多子节点共存。SDK-only。
2. **DC-11 完整闭环：render-foundation 字体栈统一到 foundation/text**（使 zero-webview 也走 foundation/text；**触碰渲染后端，需 `make product-smoke`**）。
3. **DC-3 phase-2：WebViewWidget 真实纹理合成**（render-foundation 后端实现 `RenderBackend::draw_external_surface`，按 surface_id 取 zero-webview `RenderPrimitives`/texture 合成；**触碰渲染后端，需 `make product-smoke`**）。
4. **apps/browser 逐组件灰度迁移（DC-14，product-smoke）**：用 browser-ui/chrome 组件 + render 桥替换 apps/browser 手绘 chrome（含 page_scroll.rs → ScrollBar 迁移，DC-14 零退化硬门禁）。
5. **DC-13 13 域 flesh-out**：当前 animation/gestures/navigation/overlay/collections/commands/forms/assets/platform/restoration/testing/devtools/design-system 多为 skeleton + 最小单测；逐域推进到接口可用 + 浏览器所需最小子集（§8.4.1B 矩阵）。SDK-only 为主。

...
- 门禁：`cargo build --workspace` + clippy `-D warnings` + fmt 全净；foundation/text 27 + ui/render **15** + ui/testing 5(+3) + ui/widgets **33** + browser-ui/chrome **48** + ui/runtime **22**(+8 focusscope/semantics) + ui/adapters/webview **8** + ui/examples **5** + ui/dsl **56**(+6 responsive) + ui/i18n **22**(+9 诊断/locale plural/RTL) + ui/core **38**(+3 focus) tests 全绿；DC-17 聚合下轮统一重测（本轮新增 ~250 行，预期 per-crate 仍 ≥85%）。
...
| DC-10 | 国际化资源与 message id | 🟡 flesh-out（M2） | ui/i18n：locale/catalog/fallback/参数替换/plural（多语种 CLDR：英/阿/俄/乌/白俄/波）/RTL direction/diagnostic（MissingKey/FallbackUsed/PluralFallback）可用；locale 切换→layout+paint+semantics 失效（ui/runtime I18nRuntime）。**本轮**：locale 感知 plural + resolve 诊断链 + RTL/Arabic plural 集成快照（22 测）。剩余：DSL `i18n:` message id 端到端解析桥（loader 已保留 i18n 为 Value::Object） |
...
6. **DC-10 i18n DSL 桥**：DSL loader 已把 `i18n:` 保留为 `Value::Object`；补 resolve 侧桥接（i18n object `{id, params}` → `LocalizedText::Message` / `MessageRef`），闭合 DSL `i18n:` message id 端到端引用（DC-10 唯一剩余）。SDK-only。

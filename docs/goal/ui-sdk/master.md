...
- 门禁：`cargo build --workspace` + clippy `-D warnings` + fmt 全净；foundation/text 27 + ui/render **15** + ui/testing 5(+3) + ui/widgets **33** + browser-ui/chrome **48** + ui/runtime **20**(+4 focus/key) + ui/adapters/webview **8** + ui/examples 2 + ui/dsl 48 + ui/core **35** tests 全绿；DC-17 聚合 **line 93.33% / function 92.90% / region 92.51%**（per-crate 全部 ≥85%；本轮 DC-5/DC-8 新增代码覆盖率预计 ≥85%，下轮统一重测）。
...
1. **`ui/examples` form 示例（DC-14）**：counter 已立；DC-8 phase-1 键盘路由已通 → form 示例可做（focusable TextInput 接收键盘、校验、提交 action），验证 SDK 多控件组合 + 焦点/键盘闭环。续做 browser-shell-demo（组合 chrome + render 桥 + ScrollBar + WebViewWidget 占位）。
2. **DC-8 phase-2**：click-to-focus（点击 focusable widget 聚焦）、IME rect（TextInput caret → 屏幕 rect，供平台 IME 定位）、FocusScope（modal/popup focus trap，复用 ui/core::focus FocusScope）、SemanticsNode a11y tree 接 host paint。

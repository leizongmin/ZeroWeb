...
- 门禁：`cargo build --workspace` + clippy `-D warnings` + fmt 全净；foundation/text 27 + ui/render **15** + ui/testing 5(+3) + ui/widgets **33** + browser-ui/chrome **48** + ui/runtime **22**(+8 focusscope/semantics) + ui/adapters/webview **8** + ui/examples **5** + ui/dsl **56**(+6 responsive) + ui/core **38**(+3 focus) tests 全绿；DC-17 聚合下轮统一重测（本轮新增 ~150 行，预期 per-crate 仍 ≥85%）。
...
6. **DC-10 i18n flesh-out**：locale/catalog/fallback/plural/RTL 从 skeleton 推进到可解析 + DSL `i18n:` message id 引用闭环（DC-10 当前仅 skeleton）。SDK-only。

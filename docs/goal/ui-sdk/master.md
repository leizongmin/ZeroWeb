...
- 门禁：`cargo build --workspace` + clippy `-D warnings` + fmt 全净；foundation/text 27 + ui/render **15** + ui/testing 5(+3) + ui/widgets **33** + browser-ui/chrome **48** + ui/runtime **20** + ui/adapters/webview **8** + ui/examples **4**(+2 form) + ui/dsl 48 + ui/core **35** tests 全绿；DC-17 聚合 **line 93.33% / function 92.90% / region 92.51%**（per-crate 全部 ≥85%；本轮 form 新增代码覆盖率预计 ≥85%，下轮统一重测）。
...
| DC-8 | 无障碍/焦点/IME | 🟡 焦点遍历+键盘路由 已通（M2 phase-1） | ui/core::focus(FocusDirection/TraversalPolicy/FocusScope) + semantics + ui/runtime::ime/accessibility；**本轮**：`Widget::focusable()` + `WidgetHost` focused 状态 + `focus_next`(Tab 声明顺序 wrap) + 键盘事件路由到 focused widget（Enter→emit 验证）。剩余：click-to-focus、IME rect（TextInput caret 屏幕 rect）、FocusScope modal/popup trap、SemanticsNode a11y tree 接 host |
...
1. **DC-8 phase-2**：click-to-focus（点击 focusable widget 聚焦）、IME rect（TextField caret → 屏幕 rect，供平台 IME 定位）、FocusScope（modal/popup focus trap，复用 ui/core::focus FocusScope）、SemanticsNode a11y tree 接 host paint。SDK-only。
2. **`ui/examples` browser-shell-demo（DC-14）**：组合 browser-ui/chrome 组件 + render 桥 + ScrollBar + WebViewWidget 占位（不接真实浏览器进程），验证浏览器 chrome 组合力（counter + form 已验证通用控件组合）。

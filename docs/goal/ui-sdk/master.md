...
- 门禁：`cargo build --workspace` + clippy `-D warnings` + fmt 全净；foundation/text 27 + ui/render 14(+render_node translate+paint_ctx draw_text trait) + ui/testing 5(+3) + ui/widgets 29 + browser-ui/chrome 45 + ui/runtime 8(+6 host) + ui/examples 2 + ui/dsl 48(+22) tests 全绿；DC-17 聚合 **line 93.33% / function 92.90% / region 92.51%**（per-crate 全部 ≥85%；host.rs 91.85%、render_node.rs 96%、scene.rs 95%；聚合较上轮 -0.25pp 因新增 ~440 行 host 运行时代码 91.85% 覆盖，非既有代码退化）。
...
1. **`ui/examples` form/browser-shell-demo（DC-14）**：counter 已立；续做 form（多控件 + 校验 + 提交）与 browser-shell-demo（组合 browser-ui/chrome 组件，但仍只依赖 SDK，不接真实浏览器进程）示例，进一步验证 SDK 外部可复用与组件组合力。
2. **chrome 组件 paint → scene snapshot（DC-7 完整验收）**：`WidgetHost` 已能驱动 widget paint→Scene，现可把 browser-ui/chrome 的 §8.4.1A 组件接 host 跑出 scene snapshot（断言 paint 输出进统一 Scene，不绕过 ui/render）。需先让 chrome 组件从「props+build+on_activate 产出 WidgetSpec」接出可被 host registry 注册的 widget 工厂（或一个把 WidgetSpec→Box<dyn Widget> 的通用 bridge）。

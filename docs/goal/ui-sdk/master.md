...
- 门禁：`cargo build --workspace` + clippy `-D warnings` + fmt 全净；foundation/text 27 + ui/render **15** + ui/testing 5(+3) + ui/widgets **33** + browser-ui/chrome **48** + ui/runtime **22** + ui/adapters/webview **8** + ui/examples **5** + ui/dsl **50**(+2 map/filter) + ui/core **35** tests 全绿；DC-17 聚合 **line 93.33% / function 92.90% / region 92.51%**（per-crate 全部 ≥85%；本轮 DC-6 phase-3 新增代码覆盖率预计 ≥85%，下轮统一重测）。
...
**M3 进行中（DC-6）**：phase-1 表达式引擎 + phase-2 YAML→WidgetSpec loader + phase-3 map/filter（字段投影）已落地（见上）；剩余 = ①DSL responsive branch（`Adaptive`/`branches`）/ command·route·overlay 引用；②嵌套路径投影 map/filter（follow-up）；③counter/form/browser-shell-demo 示例（counter+form 已立，DC-14）。
...
| DC-6 | YAML DSL + 完整表达式语言 | 🟡 phase-3（M3） | phase-1 表达式引擎（parse/typecheck/eval + sandbox + 资源上限，26 测）+ phase-2 YAML→WidgetSpec loader（22 测）+ **phase-3 map/filter 字段投影**（`map($items,"field")`/`filter($items,"field")`，无 lambda，2 测）已落地；剩余：DSL responsive branch（`Adaptive`/`branches`）/ command·route·overlay 引用、嵌套路径投影 |
...
| DC-8 | 无障碍/焦点/IME | 🟡 焦点遍历+键盘+click-focus+IME rect 已通（M2 phase-2） | ui/core::focus(FocusDirection/TraversalPolicy/FocusScope) + semantics + ui/runtime::ime/accessibility；phase-1：Widget::focusable + host focused + focus_next(Tab wrap) + 键盘路由 focused widget；**phase-2**：click-to-focus（Pointer Pressed→最深 focusable）+ `Widget::ime_rect` + `WidgetHost::ime_rect`(绝对 caret rect) + build_node 初始 props 同步修正。剩余：FocusScope(modal/popup trap)、SemanticsNode a11y tree 接 host |
...
1. **DC-6 phase-4（DSL responsive branch）**：`Adaptive`/`branches` 声明式响应分支（按 ViewportClass/PlatformClass/InputClass 选布局，接 ui/core::layout AdaptiveBranch）+ command·route·overlay·asset 引用。SDK-only。
2. **DC-8 phase-3**：FocusScope（modal/popup focus trap，复用 ui/core::focus FocusScope）+ SemanticsNode a11y tree 接 host paint（accessibility tree 输出）。SDK-only。
3. **`ui/examples` browser-shell-demo（DC-14）**：组合 browser-ui/chrome 组件 + render 桥 + ScrollBar + WebViewWidget 占位（不接真实浏览器进程），验证浏览器 chrome 组合力。

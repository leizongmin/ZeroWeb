...
- 门禁：`cargo build --workspace` + clippy `-D warnings` + fmt 全净；foundation/text 27 + ui/render 14 + ui/testing 5(+3) + ui/widgets 29 + browser-ui/chrome **47**(+2 render) + ui/runtime **15**(+host layout-prop) + ui/examples 2 + ui/dsl 48 tests 全绿；DC-17 聚合 **line 93.33% / function 92.90% / region 92.51%**（per-crate 全部 ≥85%；上轮覆盖率口径未重测，新增 render.rs/~30 行 chrome 叶子绘制代码覆盖率预计 ≥85%）。
...
1. **DC-3 WebViewWidget 接入**（DC-3，⬜→🟡）：`ui/adapters/webview::WebViewWidget` 真实接入 —— 接收 viewport/clip/scale/theme/输入，把 `zero-webview` 输出合成进 UI scene。当前 PageViewportFrame 仅画白底占位；接入后视口节点承载真实网页 texture/scene。这是 chrome→真实浏览器宿主的下一关键缺口。
2. **DC-4 滚动语义**：把 `apps/browser/src/page_scroll.rs` 的滚动语义抽到 `ui/widgets::ScrollBar` + `ScrollCommand`（页面尺寸/scroll offset 由 WebView 管；通用 ScrollBar 几何/视觉由 ui/widgets），与 DC-3 WebViewWidget 联动。
3. **`ui/examples` form/browser-shell-demo（DC-14）**：counter 已立；续做 form（多控件 + 校验 + 提交）与 browser-shell-demo（组合 browser-ui/chrome 组件 + chrome_scene 桥，但仍只依赖 SDK，不接真实浏览器进程）示例。
4. **DC-6 phase-3**：①map/filter lambda —— 需 spec 决策（`Expression` 枚举无 Lambda 变体；候选：字段投影 `map($items, "field")` / 取注册 fn id；走 `lei-spec-rfc` 回写 spec §8.4.7）；②DSL `Adaptive`/`branches` responsive branch（按 ViewportClass/PlatformClass/InputClass 选布局，接 ui/core::layout）+ command·route·overlay·asset 引用。
5. **render-foundation 字体栈统一到 foundation/text**（DC-11 完整闭环；**触碰渲染后端，需 `make product-smoke`**）。
6. **apps/browser 逐组件灰度迁移（DC-14，product-smoke）**：用 browser-ui/chrome 组件 + render 桥替换 apps/browser 手绘 chrome（DC-14 零退化硬门禁）。

...
**最后更新**: 2026-09-16（S848：**树变化刷新轮**（R4385 代
码提交入树，S245→S829→S835→S843 先例链，门 + make test
双腿刷新）——pull 拉入 2 提交：9d16f1636 fix(layout)
R4385 advance 源第四站点同源化（**crates/engine/src/
text_metrics.rs +11/-5，engine 共享面兄弟流经 main 落地，
本流不碰 engine 口径不变**）+ f9df17909 docs(rendering-
compat) R4385 记账 + layout-golden content-height-004 资
产更新；兄弟流侧自带验证在案（make test 19,307P/0F、
reftest 687/687、product-smoke 15.40% + legacy 0 struct
FAIL、bench-gate 定向 zero-engine GATE PASS）。**门腿**
make cdp-e2e ZW_IPC_VALIDATE=1 在位 04:28:21 启动 04:28:58
落盘（wall ~37s）**净窗首调即 PASS 33 绿 deterministic 双
跑 YES EXIT=0**（单次触发即收口，S790 先例勿二调）含
R4385 recompile 链（zero-engine→zero-page-runtime→
zero-browser 3 编译单元），expected_green 33 对称差 none
（绿步集与基线机械 diff 逐项一致零漂移）、regressions 空、
无 fatal，run_details 双 run ok 集各 33 逐项一致（差异仅
run 序号字段；failed 面=frames.click+evaluate 既有挂起项
维持），ZW_IPC_VALIDATE 捕获网在位静默（#0 复现监测零命
中，capture jsonl 面最近落盘为往轮 09-12/09-13 时点本轮
零写入）；首调红形态**连续第二百六十二次零再现**（累计两
例非聚集维持）。**dead_code warning 本轮为全新编译相位确
定性再现**（R4385 链重编译 3 单元，两相位口径记账维持，
warning 面维持 js_dom_bridge.rs:3420 match_media_to_json），
clippy -p zero-engine --lib -- -D warnings 维持 PASS（本
轮实测零 warning）DC-4 不可退让面零暴露，归因 engine
crate zero-web 流域，本流不碰 engine 记账不修。**make
test 腿 nohup 后台轮询一调收口** 04:29:07 启动 04:38:48
落盘（wall ~9min41s）EXIT=0 **19,307P/0F 与参考基线精确
一致零回归**（R4385 组合态首次全量覆盖，67 组 result 全
ok、零 make Error、零 FAILED）。本轮属**净窗亚型活跑**（
门腿窗内零>50% CPU 外来进程 负载 0.41→门后 3.11 为本门
make test 腿自然衰减 非端口竞争面 9222/45029/34293/19222
四端口族窗前窗后全程 free 门后零本门残留）；机器卫生照
S842 瞬时 zombie 先例口径 zombie 双复测（t0 与 t+5s）均
零。三层锚点——自有面对 765429dda 维持 **4 files +137/-17**
精确一致（零新增漂移）；**全树排除本流 docs 对 18d462de6
刷新为 91 files +9538/-613**（+2 文件 = R4385 双提交归
因：text_metrics.rs +11/-5 新入集 + layout-golden
content-height-004 +13/-13 新入集，rendering-compat.md 已
在集内累计 diff 增长；S848 实测逐文件记账刷新基线）；
**crates/ 观察面 raw 53→54**（git log 8fb39cd46..HEAD --
crates/ 口径，+1 = 9d16f1636 R4385 crates/engine 提交）。
子帧关键词非测试代码零命中维持（7 测试文件既有形态维
持），实质判定不变 frames.click+evaluate 维持挂起。机器
卫生：启停零 zombie（双复测口径）、端口族全 free、零树污
染（tracked clean，out/ 报告为忽略产物，门后零本门残
留）。控制面零外来提交（R4385 双提交未触
docs/goal/cdp-protocol/）。解冻条件实质判定不变：① 观察
面 crates/ raw 54（R4385 递增实测），子帧三件套
iframe.contentDocument null 现状不变，frames.click+
evaluate 维持挂起；② 本流控制面 docs/goal/cdp-protocol/
零外来提交，DC-2 口径无新拍板记录。goal 自有面零新缺口、
无扩展面（S40-S847 重审结论延续）。**make test 全量参考
基线 19,307P/0F 维持（S848 双腿刷新 R4385 组合态实测与
S843/S835 基线精确一致）——引用计数以 S848 双腿为新周期
锚点 0/10（门腿 04:28:58 落盘 + make test 腿 04:38:48 收
口），下轮 S849=1/10（至迟 S855=8/10 次轮即期限轮窗、

...
**最后更新**: 2026-09-14（S168：活跑最后期限轮——引用计数 10/10 触发，负载窗口内
落窗活跑。**首调红（门禁史 S78 后首红）**：run 1 瞬态 11 步失败 / run 2 即基线态 →
deterministic NO；四点机械归因定性单次瞬态环境事件（零代码/二进制漂移，复跑即
**PASS 33 绿 deterministic YES** 零漂移，负载下第六样本）；引用计数归零（下次活跑
至迟 S178）；绿步维持 33；并行流红绿两轮间换代 3258264→3259456、负载窗口连续）
...
3. **持续推进**：每轮 pull → cdp-e2e 门防回归（基线 **33** 步；**免复跑条件**：pull 后
   ...
   S158/S168 已执行——S168 为期限轮负载窗口内活跑（第六个负载下样本）且首次出现
   「首调红（run 1 瞬态 11 步失败 → deterministic NO）/ 复跑即 PASS」形态，四点机械
   归因定性单次瞬态环境事件（见 S168 记录）；后续轮次若再现该形态，同口径归因并留意
   复现频率——单轮偶发记账、多轮聚集升级 #0 排查；下次活跑至迟 S178；若活跑时逢
   并行流负载窗口则优先窗口内执行，负载下样本对 #0 更有价值）。
   余项按窗口逐个解冻。

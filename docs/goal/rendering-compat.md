...
>
> **▶ 字体栈当前进展（R3243-F·2026-08-11）**：resolved-face fallback 与 `font-size-adjust` 已从 computed style 贯通到 layout advance、engine callback/cache、paint glyph size 和最终 raster；新增 `from-font` 解析/序列化，Path B 持久化 per-run adjustment，并修复 ordered-ID map 错误重键到 parent 导致 run-node lookup miss。`ZW_SHAPED_FALLBACK` 现 **default-on**，`=0` 回滚。css-fonts Chromium Oracle：sum `862.56→859.17`（净改善 `3.39pp`），pass/credible/strict `84/74/54` 全持平；`size-adjust-02 -2.34pp`、`font-size-adjust-order-001 -1.45pp`、013 `-0.17pp`，012 `+0.26pp` 与 `size-adjust-03 +0.29pp` 微退。默认 reftest `687/687`、product-smoke 全 PASS、welcome `15.90%`；font/engine/layout/style/WPT 定向包全绿。trace 证明 adjustment 已到 callback，但 012/013 带 tall inline-block 的 adjusted `xA` 未生成 paint fragment，下一步转修 nested atomic-inline/IFC fragment 内容丢失，再复测 012/013。详细执行态见 [`rendering-compat/master.md`](rendering-compat/master.md)。
>

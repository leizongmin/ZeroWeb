...
>
> **▶ 字体栈当前进展（R3235-F·2026-08-11）**：generic layout advance 与逐 glyph paint advance 均已统一为 `FreeType/fallback paint base + (shaped - unshaped)` 并 default-on；回滚开关为 `ZW_SHAPED_LAYOUT=0` 与 `ZW_SHAPED_GENERIC_PAINT=0`。R3234-F 先用 trace 证伪 absolute fontdue shaped width，并把同源 source/font-id override 接入 layout 与 paint IFC；R3235-F 再把同一 contextual delta 逐 glyph 分配，58 个 welcome eligible fragment 的 `paint_consumed - fragment_width` 均精确为 `0`。css-fonts Oracle 在 R3234 基线上为 `89` 改善/`8` 微退/`185` 持平，sum `862.91→861.81`（净 `-1.10pp`），pass/credible/strict 保持 `84/74/54`；self-source `280/287` 持平，welcome `76310→76305/480000`，完整产品 viewport 门通过。下一轴为缺字 fallback 与 complex cluster 的 layout/paint advance 契约；继续 fail-closed，不复用 absolute `.notdef` 宽。详细执行态见 [`rendering-compat/master.md`](rendering-compat/master.md)。
>

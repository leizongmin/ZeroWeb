...
>
> **▶ 字体栈当前进展（R3248-F font-synthesis·2026-08-11）**：resolved-face fallback 与 `font-size-adjust` 已从 computed style 贯通到 layout advance、engine callback/cache、paint glyph size 和最终 raster；`ZW_SHAPED_FALLBACK` **default-on**，`=0` 回滚。**R3245-F** two-value `font-size-adjust` 全栈贯通。**R3246-F** `letter-spacing: 0em` ligature 抑制修复。**R3247-F** CSS `font-size` 绝对关键字 + HTML `<font size>` 映射。**R3248-F** CSS `font-synthesis` shorthand + 4 sub-properties（`font-synthesis-weight`/`-style`/`-small-caps`/`-position`），synthetic italic 受 `font-synthesis-style: none` 抑制。Oracle A/B：css-fonts `84/282 (30%)`，css-writing-modes `2/81 (2%)`；all 23 font-synthesis WPT self-source pass；reftest `687/687`。当前 Oracle 差异瓶颈：固有字体渲染差异（~1-3%）+ complex shaping（ligatures/BiDi/variable fonts）。详细执行态见 [`rendering-compat/master.md`](rendering-compat/master.md)。
>

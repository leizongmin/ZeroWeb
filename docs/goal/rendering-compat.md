...
>
> **▶ 字体栈当前进展（R3240-F·2026-08-11）**：R3239 已让 FontLoader/cache/shaper/painter消费 ordered face list，但 css-fonts A/B 净 `+0.11pp`，fallback gate保持 off。R3240-F 开始布局侧同源：`AdvanceSource` 新增 list-aware `measure_text_with_fonts` 默认方法，旧实现自动退化到首 face；IFC 新增 `NodeId→Vec<font_id>` override 与统一 `advance_run_width`，水平/垂直整串测量两调用点消费 ordered list；engine `ShapedAdvanceSource` 把 slice送入同一 callback。paint Path B 在 gate-on 时从已保存的 `text_node_font_families` 注入 ordered IDs，gate-off map为空、默认行为不变。机制测试守 `[7,9]` 原样到达度量源并决定 fragment width；默认门禁 reftest `687/687`，product-smoke 800/375/320px 全 PASS，welcome Oracle 保持 `15.90%`。stored/layout IFC 尚未生成该 map，下一阶段需在 `inline_finalization` 各构造点从 styles+resolver 注入；完成前 `ZW_SHAPED_FALLBACK` 继续 default-off。详细执行态见 [`rendering-compat/master.md`](rendering-compat/master.md)。
>

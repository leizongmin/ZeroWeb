...
>
> **▶ 字体栈当前进展（R3237-F·2026-08-11）**：R3236-F 已 dormant 贯通 fallback resolved face、grapheme run、绝对 UTF-8 cluster 与图元 font id，`ZW_SHAPED_FALLBACK=1` 显式实验、默认关闭。R3237-F 把上游 `font-size-adjust-012` 及两份 AhemEx 字体纳入常驻资产：importer 支持可重复 `--resource`，tracked `imported-resources.txt` 记录资源，`make fetch-wpt-data` 自动恢复 gitignored 字体，fresh-sync 逐字节验证通过。targeted reftest off/on 均 `0.94%` near-pass；这证明下一缺口不在 shaper face 选择，而在 style/layout 只保留单一 `font_id`，CSS `font-family: primary, secondary` 的 secondary face 从未进入 run fallback list。后续须把解析后的 family-id list 贯通 `TextRun`/shape callback/cache，再用该固定 fixture 验证；在此之前 fallback gate继续 off。详细执行态见 [`rendering-compat/master.md`](rendering-compat/master.md)。
>

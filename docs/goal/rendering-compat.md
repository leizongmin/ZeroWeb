...
>
> **▶ 字体栈当前进展（R3239-F·2026-08-11）**：R3238 已把 ordered `&[font_id]` callback 贯通三宿主。R3239-F 让 FontLoader/TextShaper 真正消费有序列表：shape cache key 首项改为 ordered font-instance ID vector；CSS face list 优先、系统 fallback 后置，按 grapheme coverage 分 run；旧 singleton API 委托 list API。Painter 新增 ordered family resolver，Path A 读 owner style，Path B 复用 `LayoutBox.text_node_font_families` 预解析列表；`ZW_SHAPED_FALLBACK=1` 才传多 face，默认仍 singleton。固定 AhemEx 测试证明 `xA` 分别使用 primary/secondary，反转列表首 glyph 随之变为 secondary，证明 cache list 顺序隔离。css-fonts Oracle A/B 为 `1` 改善/`3` 回归/`278` 持平，sum `862.56→862.67`（净 `+0.11pp`），pass/credible/strict `84/74/54` 不变；012 `+0.26pp`、013 `-0.17pp`，表明 Ahem/size-adjust 度量尚未与 fallback shaping 协调。因此 `ZW_SHAPED_FALLBACK` 继续 default-off；下一步转向 IFC per-run list + size-adjust metric contract，不直接放宽。详细执行态见 [`rendering-compat/master.md`](rendering-compat/master.md)。
>

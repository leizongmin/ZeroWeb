...
>
> **▶ 字体栈当前进展（R3233-F·2026-08-11）**：indexed glyph、custom-face shaped advance、LTR shared-cluster offset、纯 RTL logical shaping、UBA fragment direction 与 complex ligature shaping 均已 default-on；回滚开关为 `ZW_SHAPED_TEXT=0`、`ZW_SHAPED_ADVANCE=0`、`ZW_SHAPED_OFFSETS=0`、`ZW_SHAPED_RTL=0`、`ZW_SHAPED_UBA_RTL=0`、`ZW_SHAPED_COMPLEX=0`。R3230-F 贯通 feature shaper/cache/三宿主，R3231-F 补齐 CSS 两属性计算链，R3232-F 注册 `@font-face` face defaults，R3233-F 完成 `font-variant < letter-spacing < element font-feature-settings` caller precedence，并正确区分 letter-spacing normal 与显式零。全 css-fonts Oracle 283 可比案为 11 改善/0 回归/272 持平，sum `872.00→870.22`，welcome `15.96%` 持平，故 complex 默认启用。下一轴为 generic advance 与 layout fragment width 同源；既有 absolute layout width 强回归路径不得直接启用。详细执行态见 [`rendering-compat/master.md`](rendering-compat/master.md)。
>

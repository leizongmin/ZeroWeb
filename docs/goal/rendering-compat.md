...
>
> **▶ 字体栈当前进展（R3236-F·2026-08-11）**：R3234/R3235 已让 generic layout fragment 与逐 glyph paint advance 统一为 `FreeType/fallback paint base + (shaped - unshaped)` 并 default-on。R3236-F 建立 dormant fallback shaping 契约：FontLoader 按 extended grapheme cluster 解析 fallback face，连续同 face run 独立 rustybuzz shaping，cluster 加回原 UTF-8 偏移；`ShapedGlyph.font_id` 贯通到 `GlyphPrimitive`，不再被 fragment primary face 覆盖。ZWNJ/ZWJ/variation selector 保持在 base grapheme face；fallback chain 变化清空 shape cache。该路径由 `ZW_SHAPED_FALLBACK=1` 显式开启，默认关闭：标量切 face 曾使 css-fonts `0` 改善/`4` 回归；grapheme 修正后目录仅剩 `0` 改善/`2` 个 `+0.01pp` 噪声，当前主机无 CJK fallback，welcome off/on 精确 `76305/480000` 且无真实 fallback trace，证据不足以 default-on。下一步需在具备 CJK 字体或仓内固定 fallback fixture 的环境完成真实 CJK/emoji Oracle A/B。详细执行态见 [`rendering-compat/master.md`](rendering-compat/master.md)。
>

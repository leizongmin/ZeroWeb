...
>
> **▶ 字体栈当前进展（R3238-F·2026-08-11）**：R3236 已 dormant 贯通 fallback resolved face/grapheme/cluster，R3237 已常驻导入 `font-size-adjust-012` 与两份 AhemEx 资源并锁定 secondary family-id 丢失。R3238-F 先扩稳定宿主边界：`TextShapeFn` / `shape_text_for_paint` 从单 `u32` 改为有序 `&[u32]`，browser、renderer、WPT runner 三宿主同步接线；所有现有生产 caller 暂传 singleton，因此像素语义不变。WPT 真实 Lato callback 测试覆盖无 context、空列表 fail-closed、LTR/RTL、feature 开关；browser/renderer/engine 全目标编译通过。下一步让 FontLoader cache key 与 TextShaper 消费完整 ID list，再由 painter 从 owner `font-family` + resolver 生成 ordered list；IFC 仍后续单独贯通。`ZW_SHAPED_FALLBACK` 继续 default-off。详细执行态见 [`rendering-compat/master.md`](rendering-compat/master.md)。
>

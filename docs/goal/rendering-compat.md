...
>
> **▶ 字体栈当前进展（R3242-F·2026-08-11）**：R3241 已把 ordered face list 接入 layout；R3242-F 建立 resolved-face `font-size-adjust` metric/scale 契约。FontLoader 直接经 rustybuzz re-export 的 ttf-parser 读取 OS/2 `sxHeight / unitsPerEm`，新增 `FontSizeAdjustment::{None,ExHeight,FromFont}`、per-face adjusted size 与 cache-key 隔离；TextShaper 按 fallback run 的实际 face size shaping，`ShapedGlyph` 保存实际字号供后续 raster 贯通。固定 AhemEx 500/250 测试证明 aspect `0.5/0.25`，未调整 `xA` 字号 `[100,100]`，`ExHeight(0.5)` / `FromFont` 均为 `[100,200]`。旧 API全部委托 `None`，生产尚未消费新契约；默认 reftest `687/687`、product-smoke 全 PASS、welcome `15.90%`。`loader.rs` shaping API 已抽到子模块并降至 2000 行内。`ZW_SHAPED_FALLBACK` 继续 default-off；下一步把 computed `font-size-adjust` 贯通 engine callback、IFC 与 glyph raster，再跑 012/013 + css-fonts Oracle A/B。详细执行态见 [`rendering-compat/master.md`](rendering-compat/master.md)。
>

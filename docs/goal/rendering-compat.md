...
>
> **▶ 字体栈当前进展（R3245-F two-value font-size-adjust·2026-08-11）**：resolved-face fallback 与 `font-size-adjust` 已从 computed style 贯通到 layout advance、engine callback/cache、paint glyph size 和最终 raster；`ZW_SHAPED_FALLBACK` **default-on**，`=0` 回滚。**R3245-F** 实现 CSS Fonts 4 完整 two-value `font-size-adjust` 语法（`<metric>? [<number> | from-font]`），支持 5 种 metric（ex-height/cap-height/ch-width/ic-width/ic-height）+ `calc()` 表达式 + `from-font`。FontLoader 新增统一 `font_metric_aspect()` provider（ttf-parser 直查 x-height/capital-height/glyph advance）。Paint eligibility guard 放宽（adjustment 活跃时允许 background-color shaped path）；Path B primary override 扩展到 non-generic face；pure-text float shrink-to-fit 接入 resolved max-content（可扩宽）。Oracle A/B：css-fonts 全目录 `859.17→844.83`（净改善 **14.34pp**）；009/010/011 各 `8.03%→3.09%`（`-4.94pp`）；唯一微退 `size-adjust-02 +0.48pp`。reftest `687/687`、integration 735 passed、clippy clean。R3244 负实验（nested atomic-inline ownership net `+3.34pp`）完整回退仅保留文档证据。详细执行态与 A/B 证据见 [`rendering-compat/master.md`](rendering-compat/master.md)。
>

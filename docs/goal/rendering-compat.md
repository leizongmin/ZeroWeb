...
>
> **▶ 字体栈当前进展（R3234-F·2026-08-11）**：indexed glyph、custom-face shaped advance、LTR shared-cluster offset、纯 RTL logical shaping、UBA fragment direction、complex ligature shaping 与 generic layout advance 均已 default-on；回滚开关新增 `ZW_SHAPED_LAYOUT=0`。R3234-F 以 `ZW_SHAPED_ADVANCE_TRACE=1` 对账 layout estimate、unshaped/shaped advance、fragment width 与 paint consumed width，确认 absolute shaped width 会把 fontdue 浮点 metrics / `.notdef` 宽带入 FreeType grid-fit paint。现改为 `paint base + (shaped - unshaped)`，并把同一 source/font-id override 接入非 stored paint IFC；仅显式 generic family、水平 LTR、零 spacing、非 Ahem/ruby run 生效。css-fonts Oracle 282 可比案 `8` 改善/`7` 微退/`267` 持平，sum `870.22→862.91`（净 `-7.31pp`），pass/credible/strict 保持 `84/74/54`；self-source `280/287` 精确持平，welcome `15.96%→15.90%`，完整产品 viewport 门通过。下一轴为让 generic paint glyph 的逐字推进与已同源的 fragment contextual width 精确闭合。详细执行态见 [`rendering-compat/master.md`](rendering-compat/master.md)。
>

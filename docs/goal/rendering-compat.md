...
>
> **▶ 字体栈当前进展（R3243-F + R3244 负实验·2026-08-11）**：resolved-face fallback 与 `font-size-adjust` 已从 computed style 贯通到 layout advance、engine callback/cache、paint glyph size 和最终 raster；`ZW_SHAPED_FALLBACK` **default-on**，`=0` 回滚。R3243 css-fonts Oracle sum `862.56→859.17`（净改善 `3.39pp`），pass/credible/strict `84/74/54` 全持平；默认 reftest `687/687`、product-smoke 全 PASS、welcome `15.90%`。R3244 尝试恢复 012/013 nested atomic-inline 的 adjusted `xA`：机制与 trace 最终闭合到 `font_size=100`、resolved sizes `[100,200]`、fragment/paint `300=300`，但全目录 sum `866.56→869.90`（净退 `3.34pp`），012 `+0.53pp`、013仅 `-0.08pp`，且 font-family/variant 广泛微退，故代码完整回退。该 ownership 修复只能随更广的 IFC geometry unification 重投；下一独立字体方向转 CSS Fonts 4 两值 `font-size-adjust` metric surface。详细执行态与 A/B 证据见 [`rendering-compat/master.md`](rendering-compat/master.md)。
>

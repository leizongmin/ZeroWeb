...
> - **WPT 面**：已接入上游 `cache-storage` window 面基线（22 case / 291 subtest，
>   291 Pass / 0 Fail），扩大覆盖与剩余语义修复继续以该基线为回归锚点。
...
- ✅ **缺口 3 — WPT 基线**：上游 `cache-storage` window 面已导入 22 case / 291 subtest 绿线；扩大覆盖与提升通过率继续推进

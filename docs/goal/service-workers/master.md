...
**最后更新**: 2026-08-23（SW CacheStorage serviceworker cache-abort WPT baseline；broader SW fetch/cache baseline 继续）
...
`cache-storage-match`、`cache-match`、`cache-put`、`cache-add` 与 `cache-abort`
10 case / 154 subtest / 154 Pass 确定性 baseline，覆盖 worker-global
...
重复检测、AbortError rejection 和 DOMString code-unit cache name 保真。broader fetch/cache 基线仍待
...
- WPT service-workers 面：当前 core runner 34 case / 156 subtest 全绿，fetch runner
  6 case / 10 subtest 全绿，CacheStorage serviceworker runner 10 case / 154 subtest 全绿；
...
- CacheStorage serviceworker-wave 资产恢复/审计：30 assets / 154 subtest；
...
- M2 fetch/interception WPT baseline：`request-end-to-end.https.html` +
  ...
  `iso-latin1-header.https.html` 独立 runner、资产清单与 6/10 deterministic baseline
  见
  [Service Worker Fetch WPT Baseline](evidence/2026-08-22-m2-fetch-async-listener-baseline.md)
...
- M2 Service Worker CacheStorage WPT baseline：10 个 `serviceworker/cache-*.https.html`
  wrapper 独立 runner、资产清单与 10/154 deterministic baseline 见

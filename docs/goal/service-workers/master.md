...
**最后更新**: 2026-09-03（Service Worker ExtendableEvent waitUntil WPT promotion；broader SW fetch/cache/message baseline 继续）
...
M3 worker-global registration 上游 WPT core baseline 已扩展到 63 case / 234 subtest /
234 Pass，覆盖 `registration.scope`、worker global `registration.{installing,waiting,active}`
...
等待全部 lifetime promises settle、首个 rejection 诊断和 activate rejection 不阻断激活；
首个
...
| S5 | WPT 覆盖为零 | ✅ core 63/63 case、234/234 Pass、0 Fail/Timeout/Unsupported |
...
- WPT service-workers 面：当前 core runner 63 case / 234 subtest 全绿，fetch/message runner

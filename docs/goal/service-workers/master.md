...
`.any.js` window 面 WPT baseline（9 case / 136 subtest / 136 Pass / 0 Fail），其中
...
已落地。该 sibling 的 page/WebView `StorageManager` owner 现已支持
...
- storage-cache-api CacheStorage window WPT 扩面：9 case / 136 subtest 全绿，并校正
  ...
  `Cache.addAll()` undefined entry 拒绝与 Vary-aware duplicate 判定的页面侧语义，见
  [M2 CacheStorage Window WPT Expansion](../storage-cache-api/evidence/2026-08-22-m2-cache-window-expansion.md)
  和 [M2 Cache.add WPT Expansion](../storage-cache-api/evidence/2026-08-22-m2-cache-add-wpt-expansion.md)

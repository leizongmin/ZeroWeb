...
**最后更新**: 2026-09-02（Service Worker ReadableStream chunk WPT promotion；broader SW fetch/cache/message baseline 继续）
...
27 case / 71 subtest / 71 Pass 确定性
...
1. **M2 fetch/cache WPT 扩面**：在首个 fetch/interception baseline、SW CacheStorage
   ...
   `fetch('pass.txt').body` 转发、invalid chunk reader rejection、partial stream reader
   delivery 和页面 cancel/abort 反传应作为 streaming/cancel 专项处理
...
| M1 — 脚本真实执行 + 生命周期真事件 | ✅ current core WPT 194/194 Pass |
...
- WPT service-workers 面：当前 core runner 50 case / 194 subtest 全绿，fetch/message runner
  27 case / 71 subtest 全绿，CacheStorage runner 25 case / 318 subtest 全绿；

...
**最后更新**: 2026-08-31（SW same-document navigation fetch WPT baseline；broader SW fetch/cache baseline 继续）
...
`historical.https.any.js`、`fetch-event-handled.https.html` 与
`fetch-event-after-navigation-within-page.https.html` 已形成独立 runner 与
19 case / 48 subtest / 48 Pass 确定性
...
- WPT service-workers 面：当前 core runner 37 case / 162 subtest 全绿，fetch runner
  19 case / 48 subtest 全绿，CacheStorage serviceworker runner 12 case / 157 subtest 全绿；
...
- Fetch-wave 资产恢复/审计：50 assets / 48 subtest；
...
- M2 fetch/interception WPT baseline：`request-end-to-end.https.html` +
  ...
  `historical.https.any.js` 独立 runner、资产清单与 19/48
  deterministic baseline 见
  [Service Worker Fetch WPT Baseline](evidence/2026-08-31-m2-fetch-same-document-navigation-baseline.md)

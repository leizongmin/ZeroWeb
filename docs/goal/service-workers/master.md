...
**最后更新**: 2026-08-31（SW `FetchEvent.handled` WPT baseline；broader SW fetch/cache baseline 继续）
...
`historical.https.any.js` 与 `fetch-event-handled.https.html` 已形成独立 runner 与
18 case / 46 subtest / 46 Pass 确定性
...
- WPT service-workers 面：当前 core runner 37 case / 162 subtest 全绿，fetch runner
  18 case / 46 subtest 全绿，CacheStorage serviceworker runner 12 case / 157 subtest 全绿；
...
- Fetch-wave 资产恢复/审计：48 assets / 46 subtest；
...
- M2 fetch/interception WPT baseline：`request-end-to-end.https.html` +
  ...
  [Service Worker Fetch WPT Baseline](evidence/2026-08-31-m2-fetch-handled-baseline.md)

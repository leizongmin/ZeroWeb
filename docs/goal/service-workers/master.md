...
**最后更新**: 2026-09-03（Service Worker oninstall script error WPT promotion；broader SW fetch/cache/message baseline 继续）
...
M3 worker-global registration 上游 WPT core baseline 已扩展到 61 case / 223 subtest /
223 Pass，覆盖 `registration.scope`、worker global `registration.{installing,waiting,active}`
...
rejection 仍失败 install 的 lifecycle error 边界；
首个
...
| S5 | WPT 覆盖为零 | ✅ core 61/61 case、223/223 Pass、0 Fail/Timeout/Unsupported |
...
| M1 — 脚本真实执行 + 生命周期真事件 | ✅ current core WPT 223/223 Pass |
...
- WPT service-workers 面：当前 core runner 61 case / 223 subtest 全绿，fetch/message runner

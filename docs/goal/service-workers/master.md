...
M3 worker-global registration 上游 WPT core baseline 已扩展到 55 case / 203 subtest /
203 Pass，覆盖 `registration.scope`、worker global `registration.{installing,waiting,active}`
...
load 时 `controller` 投影与 iframe realm registration worker identity；
首个
...
| S5 | WPT 覆盖为零 | ✅ core 55/55 case、203/203 Pass、0 Fail/Timeout/Unsupported |
...
| M1 — 脚本真实执行 + 生命周期真事件 | ✅ current core WPT 203/203 Pass |
...
- WPT service-workers 面：当前 core runner 55 case / 203 subtest 全绿，fetch/message runner

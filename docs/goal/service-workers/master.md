...
**最后更新**: 2026-08-23（SW CacheStorage serviceworker cache-match WPT baseline；broader SW fetch/cache baseline 继续）
...
`cache-storage-match` 与 `cache-match` 7 case / 94 subtest / 94 Pass 确定性 baseline，覆盖 worker-global
...
- WPT service-workers 面：当前 core runner 34 case / 156 subtest 全绿，fetch runner
  6 case / 10 subtest 全绿，CacheStorage serviceworker runner 7 case / 94 subtest 全绿；
...
- CacheStorage serviceworker-wave 资产恢复/审计：22 assets / 94 subtest；
...
- M2 Service Worker CacheStorage WPT baseline：7 个 `serviceworker/cache-*.https.html`
  wrapper 独立 runner、资产清单与 7/94 deterministic baseline 见

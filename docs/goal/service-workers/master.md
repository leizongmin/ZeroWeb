...
| M2 — fetch 拦截 + Cache 集成 | ✅ 完成于 2026-09-06 收口（M2-44 streaming/cancel 全案落地后，fetch/message 31/85 + CacheStorage 25/318 双跑 deterministic 全绿；production 页面 fetch respondWith/pass-through、caches 全 API 面、worker fetch、registration-local CacheStorage 持久化均已接入生产链路）。过程切片 M2-1..M2-45 见「实测基线」与 archive/INDEX.md；broader fetch/cache 上游扩面转「DONE 后按需」（defer/gated 面依赖多客户端/https 服务环境） |
| M3 — 控制语义 + 消息 + 收尾 | ✅ 完成于 2026-09-06 收口（core WPT 65/249 三跑稳定：skipWaiting/controller/clients.claim/postMessage 双向/update/persistence/updateViaCache/importScripts/module graph/client registry/iframe client lifecycle/生命周期 error 边界/waitUntil settle 语义/controller-on-load-reload-disconnect 全部收敛；skip-waiting flake 根因修复 82fcb379f） |

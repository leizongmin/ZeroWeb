...
**最后更新**: 2026-08-22（SW fetch respondWith timing baseline；broader SW fetch/cache baseline 继续）
...
`request-end-to-end.https.html` 与 `fetch-event-async-respond-with.https.html` 已形成独立
runner 与 2 case / 5 subtest / 5 Pass 确定性 baseline，broader fetch/cache 基线仍待后续切片，
M3 控制语义继续推进。兄弟目标
...
- ✅ M2-10：Service Worker fetch/interception WPT baseline 扩展到
  `service-workers/service-worker/fetch-event-async-respond-with.https.html`；worker global
  ...
  `InvalidStateError` 并走网络 fallback。WebView iframe nested client 观测桥、iframe
  `contentWindow.fetch()` 的 iframe URL 相对解析与 client id/referrer 透传、manager 按受控
  client registration 优先派发 fetch 均有定向回归；fetch runner 当前 2 case / 5 subtest /
  5 Pass / 0 Fail / 0 Timeout / deterministic true。
...
- WPT service-workers 面：当前 core runner 34 case / 156 subtest 全绿，fetch runner
  2 case / 5 subtest 全绿；上游完整分母 294 个 testharness 源 / 331 URL，
...
- M2 fetch/interception WPT baseline：`request-end-to-end.https.html` +
  `fetch-event-async-respond-with.https.html` 独立 runner、资产清单与 2/5 deterministic
  baseline 见

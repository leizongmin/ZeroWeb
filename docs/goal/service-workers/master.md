...
**最后更新**: 2026-09-03（Service Worker onactivate script error WPT promotion；broader SW fetch/cache/message baseline 继续）
...
M3 worker-global registration 上游 WPT core baseline 已扩展到 62 case / 228 subtest /
228 Pass，覆盖 `registration.scope`、worker global `registration.{installing,waiting,active}`
...
worker-global `error` 报告且不失败 activate；
首个
...
| S5 | WPT 覆盖为零 | ✅ core 62/62 case、228/228 Pass、0 Fail/Timeout/Unsupported |
...
| M1 — 脚本真实执行 + 生命周期真事件 | ✅ current core WPT 228/228 Pass |
...
- WPT service-workers 面：当前 core runner 62 case / 228 subtest 全绿，fetch/message runner

...
This pinned Service Worker M2 fetch/interception baseline covers three cases. `request-end-to-end.https.html` registers a real service worker, loads a controlled iframe, dispatches a FetchEvent, and validates the Request projection returned via `respondWith(new Response(...))`. `fetch-event-async-respond-with.https.html` fixes the FetchEvent `respondWith()` timing boundary: calls from the dispatch microtask checkpoint are accepted, while later task calls throw `InvalidStateError`. `fetch-event-respond-with-argument.https.html` covers Response, Promise<Response>, and invalid non-Response arguments producing a network error.

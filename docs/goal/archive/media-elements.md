...
> 目标 media-playback / media-audio）。以 WPT
> `html/semantics/embedded-content/media-elements` 真实用例通过率为验证标准
> （2026-08-31 修正：原记 `media-0` 目录在上游 WPT 不存在——pinned rev 实测 404；
> media 语义用例实际分布在 `media-elements/` 主目录 + `the-video-element/` +
> `the-audio-element/` + `track/track-element/` 等子目录）。本文定义 Mission、边界、
> Done Criteria、执行协议和文档治理规则，供后续 `rally run` 会话作为稳定输入。日常
...
> media-elements 目录大量用例只断言 JS 可观察行为（canPlayType 返回值、readyState 推进、
> 事件序、异常），不依赖真播放；② 属性反射底座已有（R3040），渐进自然；③ 与解码器选型
...
> - **WPT 面**：`html/semantics/embedded-content/media-elements` 未导入（canvas 流已在
>   CANVAS_TEST_SUBDIRS 注释中提及 video 媒体面不在其范围；仅 testharness.rs
>   HTML_INTERACTION_CASES 里 3 个探针用例），无基线。

---

## Mission

以 **WPT `html/semantics/embedded-content/media-elements`（含 the-video-element /
the-audio-element / track/track-element 语义子集）真实用例通过率为验证标准**，把
HTMLMediaElement 的非解码语义面（状态机/事件序列/API 行为/异常）对齐到 Chromium 水平。
分阶段里程碑校准执行预期：

| 阶段 | 目标 | 说明 |
|---|---|---|
| 第一阶段 | **基线建立** | 导入 media-elements 范围内用例 + 通过率基线（当前无基线） |
...
| WPT 基础设施 | media-elements 用例导入、testharness 执行、通过率报告 | 复用 tests/wpt-runner + `make import-wpt` / `make testharness-media` |
...
- ⚠️ **缺口 4 — track 面缺失**
- ⚠️ **缺口 5 — WPT 覆盖为零**：media-elements 未导入，无基线
...
2. **自主导入** WPT media-elements 用例，扩大覆盖范围

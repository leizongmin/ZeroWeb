...
日期：2026-08-14
状态：P0/P1/P2 已完成（2026-08-14）；P2 仅覆盖 HTTP/1.1/2
...
- P2 已完成：`HttpClient::send_async_stream` 提供不聚合响应体的异步 chunk 回调及与普通 async 请求一致的重定向/安全头语义；in-process 和默认 renderer→browser IPC 页面图片均按块转发、逐块累积后交给现有完整格式解码器。缓存加载器及其他资源仍使用全量体路径以保证缓存写入原子性。
...
- P2 已完成：调度器将本地 `FetchPriority` 映射为 RFC 9218 `Priority` header（调用方显式值优先）；仅在 `ZERO_HTTP2` 启用时发送，关闭时 async client 强制 HTTP/1.1 且不发送该头。响应实际协商协议由 transport 回填匿名 `ResourceLoadEvent` telemetry，不以配置开关冒充协商结果。
- 流式边界：图片流式路径不读写 HTTP cache，半途失败会丢弃页面侧缓冲；缓存加载器仍保留全量体路径，避免半途失败写入缓存。
...
- 每 origin 上限在 HTTP/1.x 保留 6；HTTP/2 的逻辑请求并发策略必须同时受全局上限控制，且不得假设一个连接能无限承载 body。HTTP/3 当前不支持，不参与该策略。

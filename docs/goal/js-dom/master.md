...
**本轮**: R21 — 扩展 DOM_TEST_SUBDIRS 导入 dom/events（81 .html + 16 .js，纯资产零碰撞）；建立 polyfill dom/events 基线 **31.61%（98P/212F/9timeout，319 subtest）**，暴露 54 个 0-pass 事件分发缺口（Event 对象属性 / 三阶段分发语义 / EventListener handleEvent / returnValue）；**发现 native dom/events dispatchEvent 路径 hang**（native_dom=1 下 dispatchEvent 用例卡死，简单用例正常，根因疑 native listener 存储/reentrant 重渲染/get_or_create，R22 首要目标）；dom/nodes 双路径零回归（native classlist 1420P/0F）
...
| WPT dom 上游基线 | ✅ **dom/nodes polyfill 55.63% / native 54.98% 双基线对等**（178 用例 / 4502 subtest；WPT 标准口径 pass/(pass+fail)；Element-classlist.html 100%）+ **dom/events polyfill 31.61%（98P/212F/9timeout，81 用例 / 319 subtest）** | `testharness-dom`（polyfill）+ `testharness-dom-native`（ZW_NATIVE_DOM=1）双入口；R21 导入 dom/events 扩面（polyfill 基线建，native hang 见未解决问题）。失败聚类：dom/nodes iframe.contentDocument（深结构 html-compat）/querySelector-mixed-case（selector 域）；dom/events Event 对象属性/三阶段分发/EventListener（54 个 0-pass 用例） |
...
- R21 已做：① 扩展 `DOM_TEST_SUBDIRS` + `fetch-dom-subset.sh` SUBDIRS 加 `dom/events`（81 .html + 16 .js，jsdelivr CDN 拉取 0 失败，零生产逻辑改动）② 建立 polyfill dom/events 基线 **31.61%（98P/212F/9timeout，319 subtest，81 cases）**③ 失败聚类分析（Event 对象缺属性[30]/事件分发断言[25+12]/EventListener handleEvent[12]/returnValue[4]/incumbent-global 测试设施[5]；54 个 0-pass 用例）④ 核实 dom/nodes 双路径零回归（native classlist 1420P/0F）。**发现 native dom/events dispatchEvent 路径 hang**（native_dom=1 下 dispatchEvent 用例卡死，简单用例正常，根因疑 native listener 存储/reentrant 重渲染/get_or_create，R22 首要目标）。wpt-runner 双矩阵 clippy 干净
- 剩余聚类（按 ROI，R21 后重排）：① **native dom/events dispatchEvent hang**（R22 首要——native_dom=1 下 dispatchEvent 卡死，根因待定位）② **polyfill dom/events 54 个 0-pass**（Event 对象属性 + 三阶段分发语义 + EventListener，R23+ 聚类驱动）③ iframe.contentDocument（深结构 html-compat 域）④ querySelector-mixed-case（selector 域）⑤ canvas proxy instanceof（canvas 流）⑥ polyfill appendChild 闭环（M1 L2）⑦ native namespaceURI getter 独立化（双路径差 0.65pp）⑧ 扩 DOM_TEST_SUBDIRS（dom/collections 等）
...
| 2026-08-14 | R21 | 扩展 DOM_TEST_SUBDIRS + fetch-dom-subset.sh 加 dom/events（81 .html + 16 .js，jsdelivr CDN 0 失败，零生产逻辑）+ polyfill dom/events 基线 31.61%（98P/212F/9timeout）+ 失败聚类 + dom/nodes 双路径零回归核验；wpt-runner 双矩阵 clippy 干净 | **polyfill dom/events 基线 31.61%**（54 个 0-pass 事件分发缺口：Event 属性/三阶段/EventListener/returnValue）。**发现 native dom/events dispatchEvent hang**（native_dom=1 下卡死，简单用例正常，R22 首要目标）。不阻 CI（make test 不含 testharness-dom-native）。完整 JSON 快照入 evidence |
...
1. **R21（本轮，已完成）**：扩展 DOM_TEST_SUBDIRS 导入 dom/events（纯资产）+ polyfill dom/events 基线 31.61% + 失败聚类 + 发现 native events dispatchEvent hang → land（dom/nodes 双路径零回归）
2. **下轮候选（按剩余 ROI，R21 后重排）**：
   - **(a) R22 native dom/events dispatchEvent hang 根因定位修复**（native_dom=1 下 dispatchEvent 卡死，建立 native events 基线；根因线索见未解决问题 15）。
   - **(b) polyfill dom/events 54 个 0-pass 聚类驱动修复**（Event 对象属性 timeStamp/NONE/composedPath/returnValue + 三阶段分发 capture/bubble/stopPropagation 语义 + EventListener handleEvent）。
   - **(c) native namespaceURI getter 独立化**（dom/nodes 双路径差 0.65pp，dom_bindings element.rs 直接读 namespace）。
   - **(d) querySelector-mixed-case**（selector 域 case/namespace 匹配）。
   - **(e) iframe.contentDocument**（深结构 html-compat 域）。
...
15. **native dom/events dispatchEvent hang（R21 发现，R22 首要）**：`testharness-dom-native`（ZW_NATIVE_DOM=1）跑 dom/events 整体卡死（>500s）。诊断：简单用例（Event-type 3P / CustomEvent 有输出）native 正常；dispatchEvent 用例（Event-dispatch-click / event-global）native 下 20s 无输出。根因方向：native_dom=1 时 element prototype 上 `dispatchEvent`（html_element.rs:288 `native_dispatch_event_invoke`）+ `addEventListener`（:281）覆盖 polyfill，native `dispatch_event_impl`（event_target.rs:121）三阶段派发，listener 存 gc.rs LISTENERS（thread-local）——疑 reentrant 重渲染（listener 回调改 DOM 触发 `sync_render_after_native_dom` 循环）/ native listener 存储与 polyfill `_listenerStore` 割裂 / `get_or_create_native_element`（:221）每节点 create。**不阻 CI**（`make test` 不含 testharness-dom-native）。dom/nodes native 不受影响（classlist 1420P/0F）。R22 定位修复后建 native events 基线。

...
**本轮**: R27 — polyfill EventListener handleEvent（spec：addEventListener 的 listener 若是对象非函数，dispatch 时 Get 其 handleEvent 属性再调用，this=对象本身；每次派发都 Get 支持 getter）。`_dispatchToListeners` 的 fire 按类型分支（函数直接 call / 对象 Get handleEvent.call(fn)）；EventListener-handleEvent.html 双路径 0P→3P/5；dom/events polyfill 45.48%→46.75% / native 39.03%→40.26%（双路径各 +3 pass，对等差 6.49pp 基本不变）
...
| WPT dom 上游基线 | ✅ **dom/nodes polyfill 55.63% / native 54.98% 双基线对等**（178 用例 / 4502 subtest；Element-classlist.html 100%）+ **dom/events polyfill 46.75% / native 40.26%**（81 用例 / 318 subtest，R27 EventListener handleEvent 后；双路径差 6.49pp） | `testharness-dom`（polyfill）+ `testharness-dom-native`（ZW_NATIVE_DOM=1）双入口；R27 EventListener handleEvent（EventListener-handleEvent 双路径 3P/5）。失败聚类：dom/nodes iframe.contentDocument（深结构 html-compat）/querySelector-mixed-case（selector 域）；dom/events Event-dispatch 系列（深结构 document/window listener 独立）/Event-returnValue/~38 个 0-pass 用例 |
...
1. **R27（本轮，已完成）**：polyfill EventListener handleEvent（对象 listener 调 .handleEvent）→ land（EventListener-handleEvent 双路径 3P/5，dom/events polyfill 46.75% / native 40.26%，各 +3 pass）
2. **下轮候选（按剩余 ROI，R27 后重排）**：
   - **(a) Event-returnValue**（独立小切片，spec Event.returnValue = !cancelBubble 后续态镜像）。
   - **(b) cancelBubble setter dispatch 止上溯副作用**（Event-cancelBubble 剩 4 test）。
   - **(c) Event-dispatch 系列**（深结构：document/window listener 独立存储 + cloneNode/new Document/new Text 基础设施 + capture/bubble 含 document/window 链，~38 个 0-pass 主力但需深改，按需评估切片化）。
   - **(d) 双路径差 6.49pp 收口**（WheelEvent 子类链/SubclassedEvent，分散低 ROI）。

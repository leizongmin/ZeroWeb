...
**最后更新**: 2026-08-18（M2 request core event model）
...
- 🟨 M2 页面接线：factory/store/index/query/cursor stepping 已走 Rust；task-bound deactivation 待实现
...
| # | 缺口 | 状态 |
|---|------|------|
| I1 | WPT IndexedDB 用例覆盖为零 | 🟨 M1/M2 已导入 32 文件 |
| I2 | 页面→Rust 引擎零接线 | ✅ factory/store/index/query/cursor stepping 已接 |
| I3 | 无持久化（重启即失） | ⬜ M3 |
| I4 | IDBRequest 事件模型（success/error/readyState/auto-commit）非 spec | 🟨 core events 完成；task deactivation 待实现 |

## 下一步计划

1. **M2/M3**：实现 transaction active flag 与 task/microtask deactivation 时序
...
| 里程碑 | 状态 |
|--------|------|
| M1 — WPT IndexedDB 基线建立 | 🟨 imported 192/192 |
| M2 — JS↔Rust 接线（核心通路） | 🟨 request core complete；task deactivation pending |
| M3 — 索引 + 事件模型 + 持久化 | ⬜ |
...
- imported 合计：32 文件 / 192 subtest / 192 Pass / 0 Fail / 100.00%
- 当前 100% 仅覆盖 imported 32 文件，不代表上游 IndexedDB 目录整体通过率

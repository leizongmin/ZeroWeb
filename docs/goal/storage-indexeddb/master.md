...
**最后更新**: 2026-08-18（M2 Rust-backed cursor stepping）
...
- 🟨 M2 页面接线：factory/store/index/query/cursor stepping 已走 Rust；完整 request event model 待迁移
...
| # | 缺口 | 状态 |
|---|------|------|
| I1 | WPT IndexedDB 用例覆盖为零 | 🟨 M1/M2 已导入 24 文件 |
...
1. **M2/M3**：完成 request success/error/readyState/auto-commit 事件时序
...
| 里程碑 | 状态 |
|--------|------|
| M1 — WPT IndexedDB 基线建立 | 🟨 imported 182/182 |
| M2 — JS↔Rust 接线（核心通路） | 🟨 cursor stepping complete；request event model pending |
| M3 — 索引 + 事件模型 + 持久化 | ⬜ |
...
- imported 合计：24 文件 / 182 subtest / 182 Pass / 0 Fail / 100.00%
- 当前 100% 仅覆盖 imported 24 文件，不代表上游 IndexedDB 目录整体通过率

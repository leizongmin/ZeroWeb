...
**状态**：⚠️ **R1512 实施 Stage1+2 → PIL 证结构正确 → oracle NET-NEGATIVE → REVERTED。font-wall-entangled，非 clean lever**。**R4522 修订（2026-09-19）**：本 RFC 域内分出一个**非 font-wall 的窄 slice 已落地**——definite-height **balance** 容器对嵌套 multicol 子的溢出行**剪裁**（children-height-007 翻绿 2.51→0.00；溢出语义分叉律：平铺子→R1075 内联溢出列 vs 嵌套 multicol 子→剪裁；`assign_children_to_columns_multirow` cap_nested 标志，kill-switch `ZW_NESTED_MC_CLIP`）。breaking-001..006 主线维持本结论不变。见 [`evidence/r4522-nested-multicol-overflow-clip-2026-09-19.md`](./evidence/r4522-nested-multicol-overflow-clip-2026-09-19.md)。结构修复（flow-thread 2 子列 + clone border）PIL 实测正确（2 子列匹配 CHR、border y=108-132 完全对），但 oracle 004 9.58→12.61 全 background 案恶化——因 font-wall 文本宽（ZW sans-serif "AAAAA"=44 vs CHR 56px）+ 缺 column-rule（blue/fuchsia）主导 diff，结构正确无法克服。**结论：须等 font-wall C-dep 解锁或与 column-rule paint 合并修才可能 net-positive；勿再单试结构修复**。详见 master.md R1512。

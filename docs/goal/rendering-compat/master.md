...
**基线（R417 css-text-decor 扩到全量后——4/9 目录全量完成·首个文字类，DC-14 Phase 2）**：
- self-source 全量目录：**css-grid 30/48=62.5%** + **css-position 61/95=64.2%** + **css-tables 75/112=67.0%** + **css-text-decor 244/246=99.2%**（文字类容差口径，与布局类不直接可比；@0.5% strict 185/242=76.4%）（R401 grid / R404 position / R408 tables / R417 text-decor；子集原 ~100%/~90%+/~90%+/~78%，self 高估）。**4 目录聚合 self-source 410/501=81.8%**（文字容差抬升；strict 口径更低）；**混合口径（4 全量 + 5 仍 60-子集），非全量真通过率**
- **4 目录全量 chromium-Oracle 真一致 35.0%**：css-grid 19/48=39.6% / css-position 36/95=37.9% / **css-tables 49/112=43.8%** / **css-text-decor 70/242=28.9%（最低）**（chr<1%，聚合 174/497=35.0%，从 3-dir 40.8% 下拽）。text-decor **首证「文字类 oracle 显著低于布局类」**（已实现 decoration chr<1% 仅 23% 像素偏薄 < 未实现 emphasis 34%）；子集 78 文件既高估 self 又结构偏斜
...
**基线（R417 css-text-decor 扩到全量后——4/9 目录全量完成·首个文字类；strict/oracle broad 仍 pre-grid-expand 子集口径）**：

- self-source 全量目录：css-grid **30/48=62.5%** + css-position **61/95=64.2%** + **css-tables 75/112=67.0%** + **css-text-decor 244/246=99.2%**（文字容差，strict 76.4%）。**4 目录聚合 self-source 410/501=81.8%**（文字容差抬升）；**混合口径（4 全量 + 5 子集），非全量真通过率**
- **4 目录全量 chromium-Oracle 真一致 35.0%**：grid 19/48=39.6% / position 36/95=37.9% / tables 49/112=43.8% / **text-decor 70/242=28.9%（最低）**（chr<1%，聚合 174/497=35.0%，从 3-dir 40.8% 下拽）——text-decor 首证文字类 oracle 低于布局类；子集 42.1% 广义口径对 self-source 严重高估（下条 200/475 为 pre-grid-expand 子集口径）
...
1. **【优先·4 目录已完成】DC-14 分母去子集化** — gating DC-14 硬门禁（goal line 317/843）：达标判定前必须用上游每目录**全量** reftest。最可操作的多会话增量 = Phase 2 目录全量导入（grid ✅ / position ✅ / tables ✅ / **text-decor ✅**）→ 再扩中目录（fonts 391）→ 大目录（flexbox 586 / writing-modes 855 / multicol 537）→ 最后 CSS2（~5000-7000）。每批 reftest + chromium-Oracle 复测，监控真通过率。**状态**：**4/9 目录全量完成（首个文字类）**——R401 grid（62.5%/39.6%）+ R404 position（64.2%/37.9%）+ R408 tables（67.0%/43.8%）+ **R417 text-decor（99.2% self 文字容差 / 28.9% oracle）**；4-目录全量 oracle **35.0% 真一致**（聚合 174/497，text-decor 最低、从 3-dir 40.8% 下拽，首证文字类低于布局类）。工具链（`discover-reftests-authoritative.py` + `capture-oracle-per-dir.mjs`）可复用于余 5 目录。下批：css-fonts(391)（R410 pre-confirm 预测 ~33-42%）。详见 [`evidence/r412-csstextdecor-full-2026-06-22.txt`](./evidence/r412-csstextdecor-full-2026-06-22.txt) + [`evidence/r405-dc14-three-dirs-complete-2026-06-22.txt`](./evidence/r405-dc14-three-dirs-complete-2026-06-22.txt) + [`evidence/r401-dc14-grid-full-authoritative-2026-06-22.txt`](./evidence/r401-dc14-grid-full-authoritative-2026-06-22.txt) + [`evidence/r394-dc14-denominator-gap-2026-06-22.txt`](./evidence/r394-dc14-denominator-gap-2026-06-22.txt)。
...
| M10 — 上游 WPT 真实 Reftest 导入 | ⏸ plateau + DC-14 Phase 2（4/9 目录全量·首个文字类） | 基础设施 ✅；**grid+position+tables+text-decor 已扩到全量**（48/95/112/246）+ 5 目录仍 60-子集；全量目录 self grid 62.5%/position 64.2%/tables 67.0%/**text-decor 99.2%（文字容差，strict 76.4%）**，**chromium-Oracle 真一致 35.0%**（grid 39.6% / position 37.9% / tables 43.8% / **text-decor 28.9% 最低**，聚合 174/497=35.0%，从 3-dir 40.8% 下拽；首证文字类低于布局类）/ strict 295/490 (60.2%, pre-grid-expand)；R305–R400 单会话杠杆穷尽（R351/R355/R358/R362 四次 plateau 突破），达标需 DC-14 全量去子集化（余 5 目录）+ 多会话架构（见「综合裁决」） |
...
**当前基线（R408 css-tables 扩到全量后——4/9 目录全量·首个文字类；strict post-R326 复验仍零漂移，见 [`evidence/strict-baseline-reverify-post-r326-2026-06-19.txt`](./evidence/strict-baseline-reverify-post-r326-2026-06-19.txt)；4-目录 evidence [`r412-csstextdecor-full-2026-06-22.txt`](./evidence/r412-csstextdecor-full-2026-06-22.txt) + [`r405-dc14-three-dirs-complete-2026-06-22.txt`](./evidence/r405-dc14-three-dirs-complete-2026-06-22.txt) + R404 [`r404-dc14-position-full-2026-06-22.txt`](./evidence/r404-dc14-position-full-2026-06-22.txt) + R401 [`r401-dc14-grid-full-authoritative-2026-06-22.txt`](./evidence/r401-dc14-grid-full-authoritative-2026-06-22.txt)）**：

- self-source 全量目录：css-grid **30/48=62.5%** + css-position **61/95=64.2%** + **css-tables 75/112=67.0%** + **css-text-decor 244/246=99.2%**（文字容差）；**4 目录聚合 self 410/501=81.8%**；聚合（4 全量 + 5 子集）较 R401 456/518=88.0% 再下修；**混合口径，非全量真通过率**
- self-source strict **295/490 (60.2%)** @ 锁定 0.1%/0.5%（DC-14 真通过口径）
- chromium-Oracle 真一致率 **~42.1% (broad, 200/475) / 37.3% (strict self-pass&chr<1%, 177/475)**（self-source 含 46.5% 假通过；R391 锁定诚实基线，pre-R388 ~35.8% 被 108 损坏 Ahem oracle 压低已修；⚠️ 全部基于 ~5-6% 子集分母 503/上游~8000-10000，非全量真通过率，不构成 DC-14 达标证据）。**4-目录全量口径更诚实**：oracle 174/497=35.0%（grid 39.6% / position 37.9% / tables 43.8% / **text-decor 28.9% 最低**，从 3-dir 40.8% 下拽）
...
- [ ] **多会话架构承诺 vs 接受 plateau**：4-目录全量 self 410/501=81.8% / oracle 174/497=35.0%（grid/position/tables/text-decor）+ 456/518 loose（含子集）/ 295/490 strict（pre-grid-expand）/ ~42% Oracle（broad, pre-grid-expand）是诚实基线。剩余提升需 DC-14 全量去子集化（R401/R404/R408/R417 已完成 grid/position/tables/text-decor 4 目录，5 目录待扩：fonts/flexbox/writing-modes/multicol/CSS2）+ Phase A IFC 统一 / Phase 2 嵌套 multicol / baseline 合成 或 taffy 升级，均为多会话工程。R314 已飞书通知。

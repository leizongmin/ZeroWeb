...
**R424（read-only 状态复核，2026-06-21）**：css-flexbox **第 6 目录在途**（oracle-capture 阶段，镜像 R420 css-fonts）。并行 agent 导入基本完成——743 html = 118 既有 smoke 子集 + **600 新未追踪**；权威 `link rel=match` 对 **~496–502**（grep 文件上界 502 / 上轮 discover 脚本 ~496，处合理区间，非 fonts/text-decor 式文件计数高估）。`capture-oracle-per-dir.mjs --category css/css-flexbox` 已跑 **~7.4 min**（etimes 稳增、进程存活、oracle PNG 持续落盘，**非停滞**）+ `REFTEST_DUMP=1 reftest-upstream css-flexbox`（496 例 dump，供 cross-validate）已跑 ~1.9 min。HEAD 仍 **9d9b069**（R423 header hygiene），**无 feat/evidence 提交**。本轮不改代码、不抢跑测量（reftest 进程在跑，抢跑会冲突）；待并行 agent 完成 oracle+dump+cross-validate 并 commit 后，下轮折回第 6/9 目录真数据、更新聚合 oracle 图景（5→6 目录）。**非卡点**。

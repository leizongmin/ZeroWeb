...
**最后更新**: 2026-06-22（**R412** doc-maintenance：R411 单快照 rebalance 复验修正（只读）——246 test-side（69%，R411 时 191/54%）实测 text-emphasis **47.6%→55.7%**（191→246 增 55 文件中 84% 是 emphasis，分批导入使簇占比摆动 ±8pp），R411「样本足够大」假设不成立；emphasis 全失败（R392）硬封顶，**R411 上修 ~38-46% 修正为 ~32-39%**（straddle position 37.9%，居中 ~35%，非高于）。方法学教训=在途单快照预测不可靠，后续一律标「待全量+oracle 复核」；R410 fonts 单快照预测同受警示。text-decor 仍被未实现 emphasis 簇封顶，唯一解锁=Phase A line-box（多会话），de-subsetting 只揭示更诚实数字非提升。详见 [`evidence/r412-csstextdecor-rebalance-volatility-2026-06-22.txt`](./evidence/r412-csstextdecor-rebalance-volatility-2026-06-22.txt)。｜ **R416 停滞第 4 轮（只读，二次通知带解锁命令）**：css-text-decor 仍 246/356（69%），**连续第 4 轮零增长（R413–R416，自 c3616c5 起零提交）**。R415 已归因会话级（非工具 bug）。本轮二次飞书通知 rally owner 并附**确切解锁命令**（重跑 `python3 tests/wpt-runner/scripts/discover-reftests-authoritative.py --category css/css-text-decor --import` 幂等续传 246→356）。**策略：后续不再就同一停滞重复通知（除非状态变化），避免 spam**。doc-maintenance lane 饱和，待并行 agent 恢复提交后折回真 self+oracle。｜ **R415 停滞归因（只读工具核查，可操作）**：读 `tests/wpt-runner/scripts/discover-reftests-authoritative.py` 确认导入工具**无 cap/halt 于 246**——`--max` 默认 None（遍历 `gh_api` 全列表）、`fetch_raw` 失败仅 `continue` 不中断、导入循环对已存在文件 `os.path.exists` 跳过=**幂等**。故连续 3 轮停滞是**会话级**（并行 agent 未运行/中断），非工具 bug。**解锁路径**：并行 agent 重跑 `python3 discover-reftests-authoritative.py --category css/css-text-decor --import` 即从 246 幂等续传至全量 356（属并行 agent 代码 lane，doc-maintenance 不代执行）。｜ **R414 停滞升级（只读，已飞书通知）**：并行 agent 自 R413（c411f68）起**再次零提交**，css-text-decor 仍 246/356 test-side（69%）——**连续 R413→R414 两轮零增长**（最高 ROI 去子集化轨道、第 4/9 目录）。数据仍完整（非损坏，R413 已核查）。已达 run-rules「长时间阻塞」阈值，本轮飞书通知 rally owner 核查并行 agent 会话（信息性、不阻塞）。预测与 R412 修正（~32-39%）不变，doc-maintenance 前向准备仍饱和（R409–R412 覆盖 text-decor+fonts 基线），待并行 agent 恢复提交后折回真 self+oracle。｜ **R413 状态复核（只读，无代码变更）**：并行 agent 自 R412（c3616c5）起**零提交**，css-text-decor 导入停滞在 **246/356 test-side（69%，零增长）**；数据完整性核查通过（无截断/空文件、无子目录缺口、扁平结构、字母序代表性切片）= 非损坏，系**批次间停顿/会话暂停**。预测与 R412 修正（~32-39%）不变，仍待并行 agent 提交后折回真 oracle 对照。｜ **R411** doc-maintenance：css-text-decor 在途量化验证 R409「再平衡」预测（只读）——在途 191 test-side 实测 text-emphasis 子集 **82%→全量 47.6%**、text-decoration **10%→42.4%**，子集 emphasis 偏斜在全量消失（平衡集）；text-decoration 已实现占 42.4% 是真实大通过项 → R409 预测**上修至 ~38-46%**（大概率触及 41% 基线）。**本轮是最后一轮前向准备**（R409+R410+R411 已覆盖下两个文字类目录全部基线），后续等并行 agent 提交 text-decor 折回 4/9 对照。｜ **R410** doc-maintenance：css-fonts 全量前 pre-confirm 簇映射（只读）——子集 ~47% 依赖 OpenType feature/color font（rustybuzz 生产未接 text.rs:405 `ch as u32` + COLR/CPAL 全无），结构偏斜镜像 text-decor；预测全量 oracle 落 ~33-42%（OpenTech-feature/color-font 簇封顶），0 新单会话 lever。｜ **R409** doc-maintenance：css-text-decor 全量前 pre-confirm 簇映射（只读）——子集 78 文件 **82% text-emphasis**（生产全无，R392 完整回退）结构偏斜**不可外推全量**；实现锚定 text-decoration ✅/text-shadow ✅/text-emphasis ❌；预测全量 oracle 落 ~35-43%（text-emphasis 簇封顶），0 新单会话 lever。｜ **R408** doc-maintenance：css-tables 全量数据折回（并行 agent commit 7c08d74「R405 css-tables 全量」带测量，明确「master.md left to doc-maintenance agent」）。**css-tables 全量**：self-source 75/112=67.0% / chromium-Oracle 49/112=43.8%；**3/9 目录全量完成**（grid+position+tables），全量 oracle 真一致 **~41%**（grid 39.6% / position 37.9% / tables 43.8%）——子集 42.1% 广义口径对 oracle 接近、对 self-source 严重高估（3 目录聚合 self 65.1% vs 子集 ~90%+）。本轮把 3-目录全量图景折回 header / 综合裁决基线+R408 行 / M10 / 轨道 1 / 上游通过率节；**tables 簇 pre-confirm**：tables 比 grid/position 偏高 +3-6pp = 真实历史修复累积（colspan R177b 5 部件 / border-collapse R49+R342c / auto-cell-column-width R117 / height-as-minimum R168 / explicit-width 列冻结 R364），**非异常**；残余 tables 失败落 writing-mode 轴 / baseline-export / border-conflict 已知结构簇，0 新单会话 lever。零代码变更。｜**R401–R408 摘要**：R401 grid（62.5%/39.6%）+ R404 position（64.2%/37.9%）+ R408 tables（67.0%/43.8%）完成 3 目录 DC-14 Phase 2（聚合 self 166/255=65.1% / oracle 104/255=40.8%）；分母方法论（discover 权威 `link rel=match`，tables 203 上游→119 对→112 运行）3 目录一致验证。详见 [`evidence/r405-dc14-three-dirs-complete-2026-06-22.txt`](./evidence/r405-dc14-three-dirs-complete-2026-06-22.txt)。结构化结论见下方「综合裁决」表，R381–R393 verbose 详记见 [`archive/rounds-r381-r393-header.md`](./archive/rounds-r381-r393-header.md)。）
...
**基线（R408 css-tables 扩到全量后——3/9 目录全量完成，DC-14 Phase 2）**：
- self-source 全量目录：**css-grid 30/48=62.5%** + **css-position 61/95=64.2%** + **css-tables 75/112=67.0%**（R401 grid / commit d593f9c + R404 position / commit 903144a + R408 tables / commit 7c08d74；子集原 ~100% / ~90%+ / ~90%+，**子集高估 ~37/26/23pp**）。**3 目录聚合 self-source 166/255=65.1%**（vs 子集 ~90%+）；聚合（3 全量 + 6 仍 60-子集）较 R401 456/518=88.0% **再下修**；**混合口径，非全量真通过率**
- **3 目录全量 chromium-Oracle 真一致 ~41%**：css-grid 19/48=39.6% / css-position 36/95=37.9% / **css-tables 49/112=43.8%**（chr<1%，b55d46a + R404 + 7c08d74）——子集口径 42.1% 广义一致对 oracle **接近**（R388 oracle 修复后子集污染已较诚实），但对 self-source **严重高估**；tables 偏高 +3-6pp = 真实历史修复累积（colspan/border-collapse/height-as-min/auto-cell-width），非异常
...
**基线（R408 css-tables 扩到全量后——3/9 目录全量完成；strict/oracle broad 仍 pre-grid-expand 子集口径）**：

- self-source 全量目录：css-grid **30/48=62.5%** + css-position **61/95=64.2%** + **css-tables 75/112=67.0%**（子集原 ~100%/~90%+/~90%+，高估 ~37/26/23pp）。**3 目录聚合 self-source 166/255=65.1%**；聚合（3 全量 + 6 子集）较 R401 456/518=88.0% 再下修；**混合口径，非全量真通过率**
- **3 目录全量 chromium-Oracle 真一致 ~41%**：grid 19/48=39.6% / position 36/95=37.9% / **tables 49/112=43.8%**（chr<1%）——子集 42.1% 广义口径对 oracle 接近、对 self-source 严重高估（下条 200/475 为 pre-grid-expand 子集口径）
...
1. **【优先·3 目录已完成】DC-14 分母去子集化** — gating DC-14 硬门禁（goal line 317/843）：达标判定前必须用上游每目录**全量** reftest。最可操作的多会话增量 = Phase 2 小目录全量导入（grid ✅ / position ✅ / tables ✅）→ 再扩中目录（text-decor 356 / fonts 391）→ 大目录（flexbox 586 / writing-modes 855 / multicol 537）→ 最后 CSS2（~5000-7000）。每批 reftest + chromium-Oracle 复测，监控真通过率。**状态**：**3/9 目录全量完成**——R401 grid（30/48=62.5% self / 19/48=39.6% oracle）+ R404 position（61/95=64.2% self / 36/95=37.9% oracle）+ R408 tables（75/112=67.0% self / 49/112=43.8% oracle）；3-目录全量 oracle **~41% 真一致**（聚合 104/255=40.8%），子集对 self-source 严重高估、对 oracle 接近。工具链（`discover-reftests-authoritative.py` + `capture-oracle-per-dir.mjs`）可复用于余 6 目录。下批：css-text-decor(356) / css-fonts(391)。详见 [`evidence/r405-dc14-three-dirs-complete-2026-06-22.txt`](./evidence/r405-dc14-three-dirs-complete-2026-06-22.txt) + [`evidence/r404-dc14-position-full-2026-06-22.txt`](./evidence/r404-dc14-position-full-2026-06-22.txt) + [`evidence/r401-dc14-grid-full-authoritative-2026-06-22.txt`](./evidence/r401-dc14-grid-full-authoritative-2026-06-22.txt) + [`evidence/r394-dc14-denominator-gap-2026-06-22.txt`](./evidence/r394-dc14-denominator-gap-2026-06-22.txt)。
...
| M10 — 上游 WPT 真实 Reftest 导入 | ⏸ plateau + DC-14 Phase 2（3/9 目录全量） | 基础设施 ✅；**grid+position+tables 已扩到全量**（grid 48 / position 95 / tables 112）+ 6 目录仍 60-子集；全量目录 self-source grid 62.5% / position 64.2% / tables 67.0%，**chromium-Oracle 真一致 ~41%**（grid 39.6% / position 37.9% / tables 43.8%，聚合 104/255=40.8%；子集 42.1% 对 oracle 接近、对 self 严重高估）/ strict 295/490 (60.2%, pre-grid-expand)；R305–R400 单会话杠杆穷尽（R351/R355/R358/R362 四次 plateau 突破），达标需 DC-14 全量去子集化（余 6 目录）+ 多会话架构（见「综合裁决」） |
...
**当前基线（R408 css-tables 扩到全量后——3/9 目录全量；strict post-R326 复验仍零漂移，见 [`evidence/strict-baseline-reverify-post-r326-2026-06-19.txt`](./evidence/strict-baseline-reverify-post-r326-2026-06-19.txt)；3-目录 evidence [`r405-dc14-three-dirs-complete-2026-06-22.txt`](./evidence/r405-dc14-three-dirs-complete-2026-06-22.txt) + R404 [`r404-dc14-position-full-2026-06-22.txt`](./evidence/r404-dc14-position-full-2026-06-22.txt) + R401 [`r401-dc14-grid-full-authoritative-2026-06-22.txt`](./evidence/r401-dc14-grid-full-authoritative-2026-06-22.txt)）**：

- self-source 全量目录：css-grid **30/48=62.5%** + css-position **61/95=64.2%** + **css-tables 75/112=67.0%**（子集原 ~100%/~90%+/~90%+，高估 ~37/26/23pp）；**3 目录聚合 self 166/255=65.1%**；聚合（3 全量 + 6 子集）较 R401 456/518=88.0% 再下修；**混合口径，非全量真通过率**
- self-source strict **295/490 (60.2%)** @ 锁定 0.1%/0.5%（DC-14 真通过口径）
- chromium-Oracle 真一致率 **~42.1% (broad, 200/475) / 37.3% (strict self-pass&chr<1%, 177/475)**（self-source 含 46.5% 假通过；R391 锁定诚实基线，pre-R388 ~35.8% 被 108 损坏 Ahem oracle 压低已修；⚠️ 全部基于 ~5-6% 子集分母 503/上游~8000-10000，非全量真通过率，不构成 DC-14 达标证据）。**3-目录全量口径更诚实**：oracle 104/255=40.8%（grid 39.6% / position 37.9% / tables 43.8%）
...
- [ ] **多会话架构承诺 vs 接受 plateau**：3-目录全量 self 166/255=65.1% / oracle 104/255=40.8%（grid/position/tables）+ 456/518 loose（含子集）/ 295/490 strict（pre-grid-expand）/ ~42% Oracle（broad, pre-grid-expand）是诚实基线。剩余提升需 DC-14 全量去子集化（R401/R404/R408 已完成 grid/position/tables 3 小目录，6 目录待扩：text-decor/fonts/flexbox/writing-modes/multicol/CSS2）+ Phase A IFC 统一 / Phase 2 嵌套 multicol / baseline 合成 或 taffy 升级，均为多会话工程。R314 已飞书通知。

...
**🎯 当前最高优先级（2026-06-17，R197 plateau 全面确认后）**：**结构性 plateau**——DC-13 welcome 卡 Phase A font-size 死锁（R125/R158 硬阻塞）、DC-12 基本完成、reftest clean win 穷尽。剩余均为**多轮硬架构**：① **Phase A IFC font_size 统一**（welcome 文本真因 + large-font reftest 5 失败；R125 三路死锁，需突破 multicol-fill-auto 反向依赖；影响 DC-13 + DC-2~5）；② multicol 碎片化（17 失败，R113/R131 column-aware IFC）；③ DC-14 chromium Oracle 默认接线（anti-false-pass 门禁，infra 已就绪待默认化）；④ DC-13 DONE#11「5 真实网站 ≤1%」（0/5，受 Phase A 阻塞）；⑤ clip-path circle/ellipse/polygon 真裁剪（DC-12 唯一真缺口）。⚠️ line-height/font/advance-width **均证伪非 welcome 主因**。下轮须选定一个硬架构目标多轮 commit。

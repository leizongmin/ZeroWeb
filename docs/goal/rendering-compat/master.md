...
| M10 — 上游 WPT 真实 Reftest 导入 | 🔧 进行中 | 基础设施 ✅(PNG 解码+ImageCache+base_dir 路径解析+discover 脚本)；render_full_scene 全量渲染 ✅(13 种图元)；skip_indicators 模式 ✅；UA 默认样式 ✅；493 个上游 reftest 已导入（9 个目录）；总体通过率 76.3% (376/493)；css-text-decor 100.0% ✅；css-fonts 98.3% ✅(≥95%)；css-writing-modes 94.9%；css-grid 85.0%；css-multicol 70.2%；css-tables 67.9%；CSS2 66.2%；css-flexbox 56.4%；css-position 58.8%；**本轮改进**：gap 简写 handler 修复 ✅；table 行组位置更新 ✅(out-of-order 通过)；CSS 绝对长度单位支持 ✅(in/pt/pc/cm/mm/Q→px 转换)；floats-clear 通过率从 35%→57%(+5 通过)；CSS2/floats 通过率从 93%→100% ✅；**注意**：in 单位修复暴露了 CSS2/borders 中的布局差异(1in=96px 大边框)，borders 通过率从 93%→86%，后续需修复边框布局精度；剩余：border 布局精度、border-collapse、writing-mode、column breaking、flexbox baseline |

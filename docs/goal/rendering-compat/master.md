...
**基线（R425 css-flexbox 扩到全量后——6/9 目录全量完成·4 布局类+2 文字类，DC-14 Phase 2）**：
- self-source 全量目录：**css-grid 30/48=62.5%** + **css-position 61/95=64.2%** + **css-tables 75/112=67.0%** + **css-flexbox 246/496=49.6%**（布局类，runner 容差可直接比；flexbox 最低=真实缺口最多）+ **css-text-decor 244/246=99.2%**（@0.5% strict 185/242=76.4%）+ **css-fonts 282/284=99.3%**（@0.5% strict 249/282=88.3%）（文字类容差口径，与布局类不直接可比）（R401 grid / R404 position / R408 tables / R425 flexbox / R417 text-decor / R422 fonts；子集 self 高估）。**6 目录聚合 self-source 938/1581=59.3%**（文字容差抬升；strict 口径更低）；**混合口径（6 全量 + 3 仍 60-子集），非全量真通过率**
- **6 目录全量 chromium-Oracle 真一致 40.5%**：css-grid 19/48=39.6% / css-position 36/95=37.9% / **css-tables 49/112=43.8%** / **css-flexbox 244/496=49.2%（最高）** / **css-text-decor 70/242=28.9%（最低）** / **css-fonts 98/282=34.8%**（chr<1%，聚合 516/1275=40.5%，flexbox 上拉自 5-dir 34.9%）。**布局类 4 目录聚合 348/751=46.3% vs 文字类 2 目录 168/524=32.1%**——分化清晰确立（文字类主拖=decoration 像素/emphasis 未实现/font-features 未接）；flexbox 高=布局类确定性高+污染仅 13.3%（6 目录最低）
...
1. **【优先·6 目录已完成】DC-14 分母去子集化** — gating DC-14 硬门禁（goal line 317/843）：达标判定前必须用上游每目录**全量** reftest。最可操作的多会话增量 = Phase 2 目录全量导入（grid ✅ / position ✅ / tables ✅ / **flexbox ✅** / text-decor ✅ / fonts ✅）→ 再扩中/大目录（multicol 537 / writing-modes 855 / css-text）→ 最后 CSS2（~5000-7000）。每批 reftest + chromium-Oracle 复测，监控真通过率。**状态**：**6/9 目录全量完成（4 布局类 + 2 文字类）**——R401 grid（62.5%/39.6%）+ R404 position（64.2%/37.9%）+ R408 tables（67.0%/43.8%）+ **R425 flexbox（49.6% self / 49.2% oracle·最高）** + R417 text-decor（99.2% self / 28.9% oracle）+ R422 fonts（99.3% self / 34.8% oracle）；6-目录全量 oracle **40.5% 真一致**（聚合 516/1275，布局类 4 目录 46.3% vs 文字类 2 目录 32.1% 分化清晰）。工具链（`discover-reftests-authoritative.py` + `capture-oracle-per-dir.mjs`）可复用于余 3 目录。**下批：css-multicol（第 7 目录，导入 in-flight·权威 331 对，R430 pre-confirm 已备基线，预测 oracle ~25-33%）** → 余 css-writing-modes(855)/css-text。详见 [`evidence/r425-cssflexbox-full-2026-06-22.txt`](./evidence/r425-cssflexbox-full-2026-06-22.txt) + [`evidence/r421-cssfonts-full-2026-06-22.txt`](./evidence/r421-cssfonts-full-2026-06-22.txt) + [`evidence/r412-csstextdecor-full-2026-06-22.txt`](./evidence/r412-csstextdecor-full-2026-06-22.txt) + [`evidence/r405-dc14-three-dirs-complete-2026-06-22.txt`](./evidence/r405-dc14-three-dirs-complete-2026-06-22.txt) + [`evidence/r401-dc14-grid-full-authoritative-2026-06-22.txt`](./evidence/r401-dc14-grid-full-authoritative-2026-06-22.txt) + [`evidence/r394-dc14-denominator-gap-2026-06-22.txt`](./evidence/r394-dc14-denominator-gap-2026-06-22.txt)。
...
| M10 — 上游 WPT 真实 Reftest 导入 | ⏸ plateau + DC-14 Phase 2（6/9 目录全量·4 布局类+2 文字类） | 基础设施 ✅；**grid+position+tables+flexbox+text-decor+fonts 已扩到全量**（48/95/112/496/246/284）+ 3 目录仍 60-子集；全量目录 self grid 62.5%/position 64.2%/tables 67.0%/**flexbox 49.6%（布局类最低，真实缺口最多）**/text-decor 99.2%/fonts 99.3%（文字容差，strict 76.4%/88.3%），**chromium-Oracle 真一致 40.5%**（grid 39.6% / position 37.9% / tables 43.8% / **flexbox 49.2% 最高** / text-decor 28.9% / fonts 34.8%，聚合 516/1275=40.5%；布局类 4 目录 46.3% vs 文字类 2 目录 32.1% 分化）/ strict 295/490 (60.2%, pre-grid-expand)；R305–R400 单会话杠杆穷尽（R351/R355/R358/R362 四次 plateau 突破），**flexbox 暴露潜在 clean-win 面**（aspect-ratio transferred/min-size:auto 高密度簇，待 probe），达标需 DC-14 全量去子集化（余 3 目录）+ 多会话架构（见「综合裁决」） |

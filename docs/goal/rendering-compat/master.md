...
**🎯 当前最高优先级（2026-06-17，R227 welcome padding 双计修复后）**：**范式转移——剩余 chromium-diff 大头是「可定位的盒模型/布局坐标约定 bug」，非字体噪声、非 Phase A 死锁**。R225 证伪 advance-width、R226/R227 证伪「welcome 卡 Phase A font_size」（welcome 28% 真因=taffy `Layout::location` border-box 相对 vs painter content-box 相对的 **padding 双计**，engine.rs:777-793 已修，welcome 28.34%→17.06%）。R221 DC-14 诚实口径：严格真通过（z_vs_chr<1%）= **188/475=39.6%**，183 case 集中在 1-3% chromium-diff=最大杠杆且 advance-width 已排除→真因指向**更多同类坐标约定不一致**（painter 对 inline/IFC 子节点 padding 处理正确、对 block/taffy 子节点双计，约定 MIXED）。**新可复用杠杆 = R226 方法学（region-diff 扫描定位偏移带 → dump 盒几何 → 定位根因 → 修）+ R227 `LAYOUT_DUMP=1` env 诊断（reftest.rs:951 dump_layout_tree，打印 abs_y/height/margin-top/padding-top）**。下一步（R228）= ① welcome **剩余 17%**：R226 已测底部 cards/shortcuts/footer 区 diff 密度最高（41-43%），顶部 36px 仅修了垂直级联偏移，底部高 diff 是**独立的定位 bug**（疑似另一处双计或 grid/flex gap），用 LAYOUT_DUMP+region-diff 精确定位；② 同方法学攻 wintertc 25% / morning.work；③ 自源中性（不刷 438 reftest），但直接攻 DC-14 chromium 一致率（唯一达标口径）。⚠️ **并行 agent 正在 `crates/layout-engine/src/table.rs` 改 colspan/`<col>` border-collapse 列宽（R177/R178 谱系）——本方向勿碰 table.rs 避免冲突**。⚠️ 持续 ruled out（勿再追）：advance-width（R225 死路）/ multicol paint 侧切片（R203 net-negative）/ multicol balance 两趟（R200 证伪已正确）/ chromium 高 diff 候选 abspos-stretch·backdrop·fixed-bg（R202 特性缺口）/ DC-12（R197 全实现）。⚠️ 仍开放的多轮硬架构（非单会话，非当前杠杆）：multicol-breaking 需 **layout 侧 column-aware IFC**（R131，IFC 按列高预算碎片化行盒）；DC-9 GPU 收尾仅 transform/filter/blend 三项（R220 clip=空谈，需 ping-pong 双纹理后处理，reftest 低频非 load-bearing）；DC-14 chromium-oracle 严格容差默认接线（infra 就绪待默认化）。

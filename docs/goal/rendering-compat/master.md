...
**🎯 当前最高优先级（2026-06-17，字体排除后统一定向，全部布局/度量）**：① **R109 fragment border**（R192 生产端接线已落地 env-gated 且 +1 验证；唯一阻塞默认启用 = inline 级 fragment border——首/末片段画 inline 开放边 border 而非匿名块全宽 block border，否则 inline-box-002 3.20→4.67% 恶化。需 painter inline 级片段 border 子系统，≈R182 §3 多轮）。影响 block-in-inline-001/002、inline-box-001/002、clear-inline-001、morning.work blue-nav；② **line-height/字体度量对齐**（行高/ascent/descent 致卡片高度差，welcome/wintertc 文本块主因）。两者均用 chromium Oracle（`cross-validate.py`）验证，非同源通过率。

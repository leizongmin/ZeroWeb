...
**🎯 当前最高优先级（2026-06-17，R196 证伪 line-height/font 后）**：① **advance-width 对齐**（welcome 28% 真因：layout IFC 用 estimate_char_width 0.55×fs 粗估 vs chromium 真实 advance → 文本宽度/换行/位置偏差累积；R188 架构阻塞 layout IFC 不持 FontLoader；**自源中性仅影响 DC-13 不推 438**。plumbing = FontLoader measure_advance → engine 预建 font→advance → layout IFC 替代 estimate）；② DC-13 DONE#11「5 真实网站截图 ≤1%」+ wintertc fixture（0/5，DC 门禁）；③ clear-inline-001（R109 唯一残余）。⚠️ line-height:normal/font 不匹配**已证伪非 welcome 主因**（welcome 用显式 line-height；font override negligible）。均用 chromium Oracle（`cross-validate.py`）验证。

...
**最后更新**: 2026-06-20（R345/R346：2000 行规则向测试文件延伸——R345 `crates/engine/src/paint/tests/visual.rs` 2056→1790（抽出尾部 CSS resize + scroll-container + overflow-hidden 渲染集成测试 6 个到 `visual_resize_scroll.rs`）；R346 `crates/layout-engine/src/inline/tests/advanced.rs` 2281→1948（抽出尾部 CSS float 排除 + tab-size 行内布局测试 12 个到 `advanced_float_tab.rs`）。**纯测试代码移动零行为变更**：均带各自 section header 的连续尾部块，新模块复用 `make_box`/`super::super::*` 等既有共享项；make test **12265/0/72**（测试总数不变，迁移测试在新路径下全过）、clippy/fmt 干净。**剩余 >2000 仅 `tests/wpt-runner/src/reftest.rs`（2379，DC-1 关键 reftest 引擎）**——其函数高度互联（render→build_image_cache→load_png_file→convert_png_buffer 链），大块抽取对「度量所有进度」的 harness 风险 > 测试基建可维护性收益，**有意 defer**（需专门谨慎处理或作为内聚引擎模块接受）。承接 R344。**非 pass-rate lever**——plateau 不变（self-source 438/490 / strict 295/490 / Oracle ~35.6%，本轮 fresh 复跑 upstream 438/490 确认 52 失败全为已知结构性用例）。**结构性结论**（不变）：rendering-compat 全面 plateau（R305–R346，39+ 轮）——全 9 reftest 目录 + WM-1×3 + multicol + welcome 垂直/精度 bounded lever 穷尽；column-aware-IFC Phase 1 经 R319 REFUTED；唯一 forward path = 多会话 Phase A IFC 统一（墙② multicol + 换行精度同时解）+ 渲染器精度。需用户对「投入多会话 Phase A」vs「接受 plateau」决策）

**前轮 R343/R344**：R343 `app_render.rs`（browser）2156→1820（`app_render_geometry.rs`，include! 引入，附修 R342c fmt 遗留）；R344 `gpu/renderer/mod.rs`（render-foundation）2359→1927（`gpu/renderer/filters.rs`，split-impl + pub(super)，DC-9 headless 后处理三件套）。至此**所有非 vendored 生产源码均 <2000**。详见 R344 段（下）。

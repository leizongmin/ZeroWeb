...
> - **★ R2028（2026-07-25）P1b S1 incr1+incr2 LANDED**（轨 B 启动）：**incr1** Sandbox trait 加 `resolve_async_callback(id, result)`（**默认 no-op**，QuickJS 降级继承，RFC v0.3 V8-first）+ V8 实现（防御性 execute `__zwResolveCallback(id,result)`，复用 `perform_microtask_checkpoint` drain `.then`）+ `js_string_literal` 转义器（防注入）+ 4 单测。**incr2** 生产 DOM shim（`js_dom_shim.js`）注入 `__zwResolveCallback` + `__zw_pending` 表（idempotent 初始化跨脚本存活）+ e2e 单测（证生产 shim 与 resolve 机制端到端互通：注入 generate_js_dom_shim() → 宿主回调返 ID → JS 建 pending Promise → Rust resolve_async_callback → `.then` 触发）。方案 A：宿主回调同步返「回调 ID」，JS 建 pending Promise，Rust 异步完成后 `resolve_async_callback` 触发 resolve。**向后兼容**（`register_callback` 不变；默认 impl 零下游影响）。script-sandbox 143/0（+4），engine lib 1246/0（+1 e2e），clippy v8 干净，quickjs lib 编译。⏳ **S1 剩余**：tab_js_worker marshal channel（跨线程 net/timer 完成 → JS worker 线程 → `resolve_async_callback`）+ 接通真实 fetch（S3）/setTimeout 真实延迟（S5）。S2 节点身份（v8::External）独立轨道。

...
**🎯 当前最高优先级（2026-06-17，R109 彻底完成后）**：① **line-height/字体度量对齐**（行高/ascent/descent 致卡片高度差，welcome/wintertc 文本块主因——R109 已彻底完成 +2 轮 +3，字体噪声已证伪，剩余产品 smoke 主因 = 行高度量）；② DC-13 DONE#11「5 真实网站截图」+ wintertc fixture 图片密集验收（0/5，DC 门禁）；③ clear-inline-001（R109 唯一残余，inline img+span→block 堆叠，独立子问题）。均用 chromium Oracle（`cross-validate.py`）验证，非同源通过率。

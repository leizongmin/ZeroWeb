...
**最后更新**: 2026-06-22（**R406** doc-maintenance：css-position 全量数据折回（并行 agent commit 903144a 明确「master.md left to doc-maintenance agent」）。**css-position 全量**：self-source 61/95=64.2% / chromium-Oracle 36/95=37.9%；**2/9 目录全量完成**（grid+position），全量 oracle 真一致 **~38-40%**（grid 39.6% / position 37.9%）——子集 42.1% 广义口径系统性高估 ~25-37pp/目录。本轮把 2-目录全量图景折回 header 基线 / 综合裁决基线+表 R404 行 / M10 / 轨道 1（2 目录完成，下批 tables）/ 上游通过率节；R405 pre-confirm 簇映射（in-inline=Phase A / absolute-center=taffy-definite-size）已被数据 confirm。零代码变更。｜**R401–R405 摘要**：R401 grid 全量（62.5%/39.6%）+ R404 position 全量（64.2%/37.9%）完成 2 目录 DC-14 Phase 2；R396/R400/R405 簇映射→0 单会话 lever（position 失败落 Phase A abspos-text / taffy-definite-size / table-internal-abspos-CB / JS-dynamic 已知簇）；R397–R399 归档 4 块 R68 stale 节。详见 [`evidence/r404-dc14-position-full-2026-06-22.txt`](./evidence/r404-dc14-position-full-2026-06-22.txt)。结构化结论见下方「综合裁决」表，R381–R393 verbose 详记见 [`archive/rounds-r381-r393-header.md`](./archive/rounds-r381-r393-header.md)。）
...
**基线（R404 css-position 扩到全量后——2/9 目录全量完成，DC-14 Phase 2）**：
- self-source 全量目录：**css-grid 30/48=62.5%** + **css-position 61/95=64.2%**（R401 grid / commit d593f9c + R404 position / commit 903144a；子集原 ~100% / ~90%+，**子集高估 ~37/26pp**）。聚合 self-source（grid+position 全量 + 7 目录仍 60-子集）较 R401 的 456/518=88.0% **再下修**（position 真 64.2% 替代子集 ~90%+）；**混合口径，非全量真通过率**
- **2 目录全量 chromium-Oracle 真一致 ~38-40%**：css-grid 19/48=39.6% / css-position 36/95=37.9%（chr<1%，b55d46a + R404）——子集口径 42.1% 广义一致**系统性高估** ~25-37pp/目录；真 chromium 一致率远低于 self-source headline
...
**基线（R404 css-position 扩到全量后——2/9 目录全量完成；strict/oracle 仍 pre-grid-expand 子集口径）**：

- self-source 全量目录：css-grid **30/48=62.5%** + css-position **61/95=64.2%**（子集原 ~100%/~90%+，高估 ~37/26pp）。聚合 self-source（grid+position 全量 + 7 子集）较 R401 456/518=88.0% 再下修；**混合口径，非全量真通过率**
- **2 目录全量 chromium-Oracle 真一致 ~38-40%**：grid 19/48=39.6% / position 36/95=37.9%（chr<1%）——子集 42.1% 广义口径系统性高估（下条 200/475 为 pre-grid-expand 子集口径，全量真值更低）
...
1. **【优先·2 目录已完成】DC-14 分母去子集化** — gating DC-14 硬门禁（goal line 317/843）：达标判定前必须用上游每目录**全量** reftest。最可操作的多会话增量 = Phase 2 小目录全量导入（grid ✅ / position ✅ / tables 203）→ 再扩大目录（flexbox 586 / writing-modes 855 / multicol 537 / text-decor 356 / fonts 391）→ 最后 CSS2（~5000-7000）。每批 reftest + chromium-Oracle 复测，监控真通过率。**状态**：**2/9 目录全量完成**——R401 grid（30/48=62.5% self / 19/48=39.6% oracle）+ R404 position（61/95=64.2% self / 36/95=37.9% oracle）；2-目录全量 oracle **~38-40% 真一致**，子集系统性高估 ~25-37pp/目录。工具链（`discover-reftests-authoritative.py` + R404 `capture-oracle-per-dir.mjs`）可复用于余 7 目录。下批：css-tables(203)。详见 [`evidence/r404-dc14-position-full-2026-06-22.txt`](./evidence/r404-dc14-position-full-2026-06-22.txt) + [`evidence/r401-dc14-grid-full-authoritative-2026-06-22.txt`](./evidence/r401-dc14-grid-full-authoritative-2026-06-22.txt) + [`evidence/r394-dc14-denominator-gap-2026-06-22.txt`](./evidence/r394-dc14-denominator-gap-2026-06-22.txt)。
...
| M10 — 上游 WPT 真实 Reftest 导入 | ⏸ plateau + DC-14 Phase 2（2/9 目录全量） | 基础设施 ✅；**grid+position 已扩到全量**（grid 48 / position 95）+ 7 目录仍 60-子集；全量目录 self-source grid 62.5% / position 64.2%，**chromium-Oracle 真一致 ~38-40%**（grid 39.6% / position 37.9%，子集 42.1% 系统性高估）/ strict 295/490 (60.2%, pre-grid-expand)；R305–R400 单会话杠杆穷尽（R351/R355/R358/R362 四次 plateau 突破），达标需 DC-14 全量去子集化（余 7 目录）+ 多会话架构（见「综合裁决」） |
...
**当前基线（R404 css-position 扩到全量后——2/9 目录全量；strict post-R326 复验仍零漂移，见 [`evidence/strict-baseline-reverify-post-r326-2026-06-19.txt`](./evidence/strict-baseline-reverify-post-r326-2026-06-19.txt)；R404 evidence [`r404-dc14-position-full-2026-06-22.txt`](./evidence/r404-dc14-position-full-2026-06-22.txt) + R401 [`r401-dc14-grid-full-authoritative-2026-06-22.txt`](./evidence/r401-dc14-grid-full-authoritative-2026-06-22.txt)）**：

- self-source 全量目录：css-grid **30/48=62.5%** + css-position **61/95=64.2%**（子集原 ~100%/~90%+，高估 ~37/26pp）；聚合（grid+position 全量 + 7 子集）较 R401 456/518=88.0% 再下修；**混合口径，非全量真通过率**
- self-source strict **295/490 (60.2%)** @ 锁定 0.1%/0.5%（DC-14 真通过口径）

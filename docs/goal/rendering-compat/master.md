...
**最后更新**: 2026-06-19（R343：2000 行规则跨 crate 收尾——`app_render.rs`（browser）2156→1820 行拆分，抽出 `app_render_geometry.rs`（335，9 个圆角矩形/圆形/几何裁剪图元构造私有 helper：rect_fill / rounded_rect_x_span_at_y / push_fill_clipped_to_rounded_rect / axis_rect_intersects_rounded_rect / push_rounded_rect_outside_corner_masks / push_rounded_rect_border / push_rounded_rect_fill / draw_hollow_square / push_circle_fill），经 app.rs `include!` 引入（与 app_input/app_render/app_platform 同约定，同模块作用域）。**纯文本移动零行为变更**：同可见性（私有 fn）/ 零 call-site 改动 / Color·FillPrimitive·Rect 解析不变（共享作用域）。验证：make test **12265 passed / 0 failed / 72 ignored**、make reftest 686/686 inline smoke 100%、**upstream reftest 438/490 字节级一致零回归**（52 失败 = 已知结构性 plateau 用例，与 R342c 同集合）、clippy `--workspace --all-targets -D warnings` 干净。**附带 cargo fmt 修 R342c 遗留**：table.rs/table_borders.rs 手工对齐 `pub(crate)     field` → 规范化（非语义空白），恢复 DC-7 fmt 门绿（R342c 提交时漏跑 fmt 致 main fmt-red）。**至此非 vendored 生产源码仅剩 gpu/renderer/mod.rs（2359，render-foundation）>2000**（taffy-local/flexbox.rs 2313 为 vendored 不动；engine.rs 1717 / inline/mod.rs 1916 / table.rs 1973 / app_render.rs 1820 均 <2000）。承接 R342c。**非 pass-rate lever**——plateau 本身不变（self-source 438/490 / strict 295/490 / Oracle ~35.6%）。**结构性结论**（不变）：rendering-compat 全面 plateau（R305–R343，37+ 轮）——全 9 reftest 目录 + WM-1×3 + multicol + welcome 垂直/精度 bounded lever 穷尽；column-aware-IFC Phase 1 经 R319 REFUTED；唯一 forward path = 多会话 Phase A IFC 统一（墙② multicol + 换行精度同时解）+ 渲染器精度。需用户对「投入多会话 Phase A」vs「接受 plateau」决策）

...
### R149 — PNG 加载修复独立实测 + 绘制顺序(DC-10) bundle 精化（诊断，持平，已回退，工作区清洁）

**背景**：R135 记录「reftest harness `load_png_file` 不处理非 RGBA PNG → 图退化透明」单独修 net -5（432 基准）。本轮在 **436/490 基准**上重测，刷新数据点并精化 bundle。

**实验**：`load_png_file` 增加 `Transformations::EXPAND | STRIP_16`，按 `output_color_type().0.samples()` 分配缓冲，对 RGB(3)/grayscale(1)/grayscale+alpha(2) 补 alpha=255 规范化为 RGBA。实测 **436 → 427/490（net -9，比 R135 的 -5 更差）**。新增/暴露的失败（set-diff vs 436 基准）：

- **类型分桶绘制顺序破坏（DC-10 核心）**：4 个原通过 `abs-pos-non-replaced-vrl-006/012/122/130.xht`（6.33%）转失败——其 `#containing-block` 用 `background: red url("support/bg-red-3col-3row-320x320.png")`（非 RGBA PNG）。修前 PNG 退化透明（仅红色底），绿 span 子元素可见→过；修后 PNG 网格正确渲染，但 **`render_full_scene`（cpu/mod.rs:53-152）按类型分桶（step5 images 画在所有 step2 fills 之后）**，父背景图把 abspos 绿 span 子元素覆盖。这是真正的 DC-10 缺陷：父背景图必须画在子内容**之下**，分桶渲染违反 CSS painting order。
- **隐藏的 clearance 精度 bug 暴露**：`clear-clearance-calculation-001/002/003`（1.25/1.67/1.62%）+ `clearance-calculations-vrl-004/008`（7.09/6.42%）修前 PNG 退化=假通过，修后暴露真实 clearance 像素误差。

**结论（修正 R135）**：PNG-only 修复在 436 基准上 **net -9**（非 -5）。完整 bundle = **(A) PNG EXPAND 修复 + (B) 按插入顺序的 stacking-aware 绘制列表 + (C) clearance 精度**。其中 (B) 是关键且最难——R135b 已验证纯 DrawRecord（DOM 顺序）让 clearance-calc-001/002/003 真实通过(0.00/0.00/0.38%) 但 regress `abs-pos-non-replaced-vrl-002`(4→5.33%)：**纯插入顺序对 abspos 不正确**（abspos 应按 z-index/stacking context 而非 DOM 顺序），需补 abspos/z-index 处理。当前架构障碍：`RenderPrimitives` 用 13 个 typed Vec + `PrimitiveCounts` 快照驱动 clip/opacity 后处理（helpers.rs:374-514），insertion-order 改动需并行 draw_order 记录且不破坏 clip/opacity 基础设施。**属多会话架构项目，单会话预期 +0**。

**本轮实证复核（独立验证 R148）**：对全部 54 失败按 ref 类型分类 = **41 CSS-REF（真实信号）+ 12 IMG-REF（退化区，含上述 8 个非 RGBA support PNG）+ 1 firefox-bug 路径缺失**。逐个 PIL+BBox 分析 10+ 失败（css-flexbox-row/flexbox-column-row-gap-004/fixed-table-layout/flex-abspos-inset-nested-001(border:none)/border-001/inline-box-001/baseline-007/clear-float-003/min-max-size-table/table-cell-width-0/multicol-breaking-006/multicol-fill-000），全部印证 R148 结构性结论：clean single-session win 确已穷尽。CSS-REF 失败按聚类=IFC 双路径/inline-block ownership(block-in-inline×3+css-flexbox-row+large-font×4) + multicol 碎片化(~16) + 垂直书写模式轴(×5) + flex 基线合成(×4) + intrinsic sizing(R97 ×4) + 表格深层(×3)；IMG-REF 失败全部受 PNG bundle 阻塞（修前假通过/退化）。

**下一步明确指引（多会话）**：启动 **DC-10 drawing-order 架构项目**（== R135b DrawRecord + stacking-aware）作为 PNG bundle 的 (B) 组件。第一步安全子步骤=为 `RenderPrimitives` 增 `draw_order: Vec<DrawOp>` 记录插入顺序（不改 typed Vec、不动 clip/opacity），env-gated `ZERO_DRAW_ORDER=1` 让 `render_full_scene` 按 draw_order 渲染，零默认行为变更、纯插桩诊断 abspos stacking 回归源（R135b 报告的 vrl-002 5.33%）。**不要**单独提交 PNG fix（net -9）；PNG fix 必须与 (B) 同提交。
...
**为何 net -5 必须回退（432→427）**：很多 reftest 的 **test 侧**用背景 PNG 作「期望图案」（绿块应覆盖），ref 侧纯 CSS 绿块。修前 PNG 不渲染（透明）→ test 未覆盖区也透明 → 与 ref 凑巧容差内通过（**假通过**）。修后 PNG 正确渲染 → 暴露第二个 bug：`render_full_scene`（cpu/mod.rs）按**类型分桶**渲染（step2 fills → step5 images），父元素背景 PNG 画在子绿块之后覆盖子内容（违反 CSS painting order，DC-10）。回归 5：CSS2 `clear-clearance-calculation-001/002/003` + writing-modes `clearance-calculations-vrl-004/008`。**0 个新通过**。

**R135b painting-order 诊断（DC-10，纠正「精度误差」误判）**：REFTEST_DEBUG 确认 clear-clearance-calc-001 的 3 个绿块 fill **生成在正确位置**（y=51/91/151，与 ref 一致）——**layout 完全正确**，无精度误差。实现了 `DrawRecord` 顺序记录（primitive/mod.rs draw_order 字段 + 9 个 add_* 记录 + render_full_scene 按 draw_order 渲染），image+painting 让 CSS2 clearance-calc-001/002/003 真实通过（0.00/0.00/0.38%）。但**仍 net -3**（已全回退）：纯 DOM 顺序对 abspos 不正确（需 stacking-context/z-order，类型分桶更接近）→ regress abs-pos-non-replaced-vrl-002（4.00→5.33%）；vrl-004/008 仍 fail（需 bug 2 vertical float）。

**完整 bundle = 3 部分**：(1) image 加载修复；(2) **stacking-aware** painting-order（DrawRecord 已验证可用，仅需补 abspos/z-index 处理，非纯 DOM 顺序）；(3) bug 2 vertical float 定位。三者同提交才 net ≥0。已验证部分组合净负：image-only -5、painting-only -1、image+painting -3。**当前 432 含多个退化-ref 假通过**；此 bundle 是「真实正确性」工作（让通过用例在真实 ref 下也过），不改当前 pass 数。要涨 pass 数须修当前就失败的非退化用例（multicol/flex-baseline/large-font/intrinsic，均结构性，clean win 穷尽）。
...
| 2026-06-15 | R149 PNG bundle 实测 + DC-10 绘制顺序精化（诊断，持平，已回退，工作区清洁） | 436/490 持平。重测 R135 PNG-only 修复（`load_png_file` 加 `EXPAND\|STRIP_16` + 按实际 samples 分配 + RGB/grayscale 补 alpha）：**436→427/490 net -9**（比 R135 记录的 -5 更差，基准不同）。暴露 9 个失败：(a) DC-10 类型分桶绘制顺序——4 个原通过 `abs-pos-non-replaced-vrl-006/012/122/130`（父 `background: url(bg-red-3col-3row-320x320.png)` 非 RGBA PNG）转失败：修前 PNG 退化透明=假通过，修后 `render_full_scene`（cpu/mod.rs:53）按类型分桶(step5 images 画在所有 step2 fills 后)使父背景图覆盖 abspos 绿 span 子元素；(b) 隐藏 clearance 精度 bug 暴露——clear-clearance-calc-001/002/003 + clearance-calculations-vrl-004/008 修前假通过。**修正 R135**：PNG-only 在 436 基准 net -9；完整 bundle=(A)PNG EXPAND + (B)按插入顺序 stacking-aware 绘制列表 + (C)clearance 精度。(B)最难——R135b 已证纯 DrawRecord(DOM 顺序)让 clearance-calc 真实通过但 regress abs-pos-non-replaced-vrl-002(4→5.33%)：纯插入顺序对 abspos 不正确（应按 z-index/stacking 非默认顺序）。架构障碍=`RenderPrimitives` 13 typed Vec + `PrimitiveCounts` 快照驱动 clip/opacity(helpers.rs:374-514)，insertion-order 改动需并行 draw_order 记录不破坏 clip/opacity。**实证复核 R148**：54 失败=41 CSS-REF + 12 IMG-REF(退化区,含 8 非 RGBA support PNG) + 1 路径缺失；逐个 PIL+BBox 分析 10+ 失败全部印证 R148 结构性结论。**下一步=DC-10 drawing-order 多会话架构项目**，第一步安全子步骤=给 `RenderPrimitives` 增 env-gated `draw_order: Vec<DrawOp>`（不改 typed Vec/clip/opacity），`ZERO_DRAW_ORDER=1` 让 render_full_scene 按序渲染，纯插桩诊断 abspos stacking 回归源；PNG fix 必须与 (B) 同提交。 |

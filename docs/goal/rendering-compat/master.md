...
**🎯 当前最高优先级（2026-06-17，R198 Phase A 死锁关闭后）**：**结构性 plateau 硬化**——DC-13 welcome 卡 Phase A font_size 死锁（R198 新变体再证，**方向关闭**）、DC-12 基本完成、reftest clean win 穷尽。剩余可操作最大聚类 = **multicol 碎片化（17 失败，css-multicol 40/57=70.2%）**：① multicol column-aware IFC（R113/R131，把行内流碎片化到各列，最大同源聚类，bounded multicol 子系统，多轮）；② DC-14 chromium Oracle 默认接线（anti-false-pass 门禁，infra 就绪待默认化）；③ DC-13 DONE#11（受 Phase A 阻塞，welcome 文本需架构性 IFC 统一非单点）；④ clip-path 真裁剪（DC-12 唯一真缺口）。⚠️ Phase A font_size/line-height/font/advance-width **全部证伪或死锁**。下轮 commit multicol 碎片化多轮攻坚。

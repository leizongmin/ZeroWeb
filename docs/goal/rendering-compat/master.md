...
**🎯 当前最高优先级（2026-06-17，R201 multicol-breaking 定性后）**：**结构性 plateau 硬化**——multicol-breaking 经 R201 dump 实测定性为**inline 内容碎片化 wiring 缺失**（算法已存在，缺口在 paint 接线，多轮子系统）；DC-13 welcome 卡 Phase A font_size 死锁（R198 关闭）、DC-12 基本完成、reftest clean win 穷尽。剩余可操作方向：① **multicol-breaking Round 4' wiring**（放宽 text.rs:711 门控 + `column_span_offsets` paint 路径重绘碎片化 IFC 内容到各列+列裁剪，解锁 004/006/nobackground-004；前置须守 multicol-fill-auto-001 不回归=R198 font_size 死锁交互）；② DC-14 chromium Oracle 默认接线（anti-false-pass 门禁，infra 就绪待默认化）；③ DC-13 DONE#11（受 Phase A 阻塞）。⚠️ R204 纠正：**DC-12 clip-path 已全实现**（R197「仅画指示线」过时），DC-12 无真缺口。⚠️ Phase A font_size/line-height/font/advance-width + multicol balance/碎片化算法**全部证伪或已存在**，勿再建 measure-first 工具。⚠️ R202 实证排查后，**chromium Oracle 高 diff 候选（abspos-semi-replaced-stretch / backdrop-inherit / background-attachment:fixed）全部 ruled out**（特性缺口或非布局 bug），勿再追。⚠️ R203 实证后，**multicol-breaking paint 侧修复（单点 D / 协调 A'+D）全部 net-negative ruled out**（40/57→36/57），勿再做 paint 侧切片。真实剩余路径 = **layout 侧 column-aware IFC**（R131：IFC 生成行盒时按列高预算碎片化到各列，每列独立正确行盒分布），multi-round layout 子系统，非单会话。

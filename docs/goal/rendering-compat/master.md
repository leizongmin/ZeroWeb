...
**最后更新**: 2026-06-19（R342c：2000 行规则收尾——`table.rs` 2694→1973 行拆分，抽出 `table_borders.rs`（740，resolve_collapsed_borders + BorderSource/resolve_border/边框颜色读取，CSS §17.6.2 collapsed 边框冲突解析集群）。**纯移动零行为变更**：reftest-upstream 438/490 字节级一致、**css-tables 51/55 不变**（border-collapse/collapsed-border/subpixel 全部边框用例 byte-identical）、888 layout 测试过、clippy `-D warnings`（全 workspace --all-targets）干净。共享类型 TableCell/TableRow/TableGrid（含字段）+ 访问器 get_row_box(_mut)/get_cell_box 改 pub(crate)；table.rs 经具体 import 调用 resolve_collapsed_borders。承接 R342（engine.rs 3371→1717）/ R342b（inline/mod.rs 2127→1916）。**至此 layout-engine 全部主要生产源码均 <2000**（engine.rs 1717 / inline/mod.rs 1916 / table.rs 1973）；剩余 >2000 跨 crate：gpu/renderer/mod.rs（2359，render-foundation）、app_render.rs（2156，browser）+ 若干 test 文件，独立项。**非 pass-rate lever**——plateau 本身不变（self-source 438/490 / strict 295/490 / Oracle ~35.6%）。详见 [`evidence/r342c-table-borders-split-2026-06-19.txt`](./evidence/r342c-table-borders-split-2026-06-19.txt)。**结构性结论**：rendering-compat 全面 plateau（R305–R342，36+ 轮）——全 9 reftest 目录 + WM-1×3 + multicol + welcome 垂直/精度 bounded lever 穷尽；column-aware-IFC Phase 1 经 R319 REFUTED；border-001 / flex-abspos-inset-nested = self-false-fail（ZeroWeb 离 chromium 0.73-0.75%，DC-14）。唯一 forward path = 多会话 Phase A IFC 统一（墙② multicol + 换行精度同时解）+ 渲染器精度。需用户对「投入多会话 Phase A」vs「接受 plateau」决策）

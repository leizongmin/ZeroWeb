...
**最后更新**: 2026-06-19（R339：welcome 17% 分布式 diff 实证 = Phase A 非-Ahem font ascent——line-height 计算（resolve_font_metrics）已核验正确（welcome 用显式 1.08/1.5/1.45/1.25），残余是非-Ahem font ascent 近似（paint v_offset=is_ahem?0:font_size 把 font_size 当 ascent，sans-serif 实际 ascent≠font_size）= Phase A offset 语义（R306 load-bearing）。无单点修。R334-R339 全 bounded lever + 产品局部 bug 穷尽，唯一 forward motion = 多会话 Phase A。另：code-health 抽出 engine.rs/tree.rs/table.rs 测试模块（tree 2233→863<2000）。详见 evidence/r339。前置 R338：text-emphasis self-source 回归陷阱——32 测试当前 loose 32/32 / strict 25/32 全过（因 test+ref 两侧都缺 emphasis 标记→同错匹配，DC-14 false-pass）；单方面实施 text-emphasis 会**回归 25-32 当前通过用例**（打破 test==ref 同缺的虚假匹配，除非完整到两侧像素对齐 = 多会话）。**self-source 指标膨胀洞察**：438/490 含未实现 feature 的虚假通过（text-emphasis 贡献 32），强化 DC-14 chromium-Oracle（~35.6%）才是诚实指标。**结构性结论**：rally 单会话无安全 forward path——修渲染 bug 受 Phase A IFC 死锁阻塞（bounded 切片全 net-negative）、实施 feature 触 self-source 回归陷阱。R305-R338（33 轮）收敛，需用户决策。详见 [`evidence/r338-text-emphasis-selfsource-regression-insight-2026-06-19.txt`](./evidence/r338-text-emphasis-selfsource-regression-insight-2026-06-19.txt)。零代码变更，基线持平）

...
**最后更新**: 2026-06-20（R344：2000 行规则收尾完成——`gpu/renderer/mod.rs`（render-foundation）2359→1927 行拆分，抽出子模块 `gpu/renderer/filters.rs`（449，DC-9 headless 后处理三件套 apply_color_filters_headless / apply_transform_filters_headless / apply_blur_filters_headless——ping-pong A/B 双纹理 颜色矩阵 / 2D 变换逆采样 / 可分离模糊 pass，R285/R286/R287 工作）。**split-impl 纯移动**：子模块 `impl super::GpuRenderer` 可访问父私有字段零字段可见性变更；三方法改 `pub(super)`（父 mod.rs 的 render_full_scene_gpu 仍调用，唯一可见性变更，同 R342c `pub(crate)` 谱系）；`use super::*` 复用 mod.rs 导入 + 局部项（Rect/TransformPost/create_headless_texture/run_blur_pass）。验证：make test **12265 passed / 0 failed / 72 ignored**（含 tests.rs **48 个 GPU 单测**直接覆盖 render_full_scene_gpu + 三方法）、make reftest 686/686 inline smoke 100%、**upstream reftest 438/490 字节级零回归**（52 失败 = 已知结构性 plateau 用例）、clippy `--workspace --all-targets -D warnings` 干净、fmt 干净。**🎉 至此 2000 行规则全量达标**：所有非 vendored 生产源码均 <2000（engine.rs 1717 / inline/mod.rs 1916 / table.rs 1973 / app_render.rs 1820 / gpu/renderer/mod.rs 1927；taffy-local/flexbox.rs 2313 为 vendored upstream 副本不动）。承接 R343。**非 pass-rate lever**——plateau 本身不变（self-source 438/490 / strict 295/490 / Oracle ~35.6%）。**结构性结论**（不变）：rendering-compat 全面 plateau（R305–R344，38+ 轮）——全 9 reftest 目录 + WM-1×3 + multicol + welcome 垂直/精度 bounded lever 穷尽；column-aware-IFC Phase 1 经 R319 REFUTED；唯一 forward path = 多会话 Phase A IFC 统一（墙② multicol + 换行精度同时解）+ 渲染器精度。需用户对「投入多会话 Phase A」vs「接受 plateau」决策）

**前轮 R343**：`app_render.rs`（browser）2156→1820 行拆分，抽出 `app_render_geometry.rs`（335，9 个圆角矩形/圆形/几何裁剪图元构造私有 helper），经 app.rs `include!` 引入。纯文本移动零行为变更；upstream 438/490 字节级零回归。**附带 cargo fmt 修 R342c 遗留**（table.rs/table_borders.rs 手工对齐 `pub(crate)     field` → 规范化，恢复 DC-7 fmt 门绿）。

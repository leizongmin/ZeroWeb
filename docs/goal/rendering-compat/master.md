...
| border-bottom-width-006（2.86%）根因纠正=inline-block 定位 bug（非渲染管线合成） | R359/R360 | R359 误判为「渲染管线合成问题」——R360 隔离单案 + REFTEST_DEBUG + 像素复核实证纠正：**两个 fill 均正确渲染**（#test border fill 在 (8,51,96,96) black，(50,100) 像素确认黑；#reference bg fill 在 (12,16,96,96) black 也在 primitives）。真根因 = **#reference（第二个 inline-block）被定位到 (12,16) 而非紧邻 #test（应在 ~(109,51)）**——inline-block 定位 bug（IFC 行盒对连续 inline-block 的 x/y 推进错误），非渲染管线。style/layout-box/paint_borders/paint_background 全正确（探针已回退）。属 inline-block 布局定位（高风险区，R180 shrink-to-fit 谱系），单会话修有回归风险。同期还查 table-cell-width-0（table 算法簇）/flex-abspos-inset-nested（嵌套 abspos definite-size）/clear-float-003（taffy clear+margin R333）/multicol-breaking-006（嵌套 multicol R113）均结构性。**结论**：clean-win 面经第二轮（R359/R360）确认再穷尽，forward motion = 多会话 Phase A |

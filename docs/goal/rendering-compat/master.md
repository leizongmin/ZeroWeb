...
**R483 CSS2 oracle 监测（20:03，post-self-dump·read-only·无卡点·forward-look 实时复算 on-track）**：self-dump 完成 ~19:42 已确认（无 reftest-upstream 进程在跑，6106/6109）。oracle **5258/9265≈56.8%** @~48/min（R481 4781@19:52→今 5258@20:03，477 PNG/~10min 稳态，无 stall；PID 4901 capture-oracle-per-dir.mjs 活跃写 normal-flow/width-100_xht.png），仍处 **normal-flow（769/857≈90%，剩 ~88 即将完）**，page-box/pagination/positioning/selectors/tables/text 全 **0**（字母序 p 之后未抵达）。**⚠️ 纠正上轮「normal-flow 731/733」分母口径**：733=matched-pairs 估，实 test-ish=857（R472 权威），故 769/857≈90% **仍在 normal-flow 非「即将转 pagination」**——结论不变（仍待 unbiased 窗口），但勿再以 733 为 normal-flow 分母。**ETA 实时复算印证 R480/R481 预测在轨**：normal-flow 剩 88+page-box 36+pagination 103+positioning 578→**~20:20 覆盖 positioning**；+selectors 621（+小 run-in/sec5/stacking-context/syntax）→**~20:33 覆盖 positioning/selectors（dump 重叠大头 normal-flow/positioning/selectors 齐→早期 unbiased 信号可行）**；+tables 1139→**~20:57**；+text 565→**~21:09 覆盖 tables/text（完全代表性 unbiased）**；全量剩 4007→**~21:26 完成**（与 R480 ~21:19/R481 ~21:15 吻合，在轨）。**无 time-limit 风险**：oracle=独立 node capture-oracle-per-dir.mjs，**无 test-guard 包裹/无 --time-limit flag**（仅 setTimeout 80ms 间隔），不受 2hr 杀进程约束，跑到底即可。**决策不变**：未抵 unbiased 窗口（p/s/t/table 全 0），勿即刻 cross-validate（normal-flow-biased 会系统性高估真 chr<1%→误诊 R472 ~30-42% 预测为假高）；等 ~20:33+ 覆盖 positioning/selectors（早期信号）/ ~21:09 覆盖 tables/text（完全）→ unbiased cross-validate → 验 R472 ~30-42% → **10-dir 最终 DC-14 全量基线**。零代码变更（并行 agent 拥有 capture 脚本 + wpt-data）。

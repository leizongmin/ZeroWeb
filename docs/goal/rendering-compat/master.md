...
| border-bottom-width-006（2.86%）深查=渲染管线合成问题，非 clean win | R359 | 逐层探针排除（均已回退）：① style-system OK——`#test{border-bottom-width:96px}` 经完整级联（选择器匹配+级联+apply）正确设 Px(96)（+1 单测 `test_border_bottom_width_longhand_full_cascade` 守护）；② layout box OK——paint_node 探针实证 box_bb=96/h=96/content_h=0（taffy 正确返回 border-box）；③ paint_borders OK——Solid 分支探针实证 fill 正确 add 到 self.primitives：rect=(8,51,96,96) black。**但该 fill 未出现在最终渲染像素**（该区域 white）→ bug 在「fill 已入 primitives 但未渲染」之间，疑为 inline-block 图元合成/渲染管线（render_full_scene 或 inline-block 子树处理），深管线问题，非单点 clean fix。同期还查 table-cell-width-0（table 算法簇）/flex-abspos-inset-nested（嵌套 abspos definite-size）/clear-float-003（taffy clear+margin R333）/multicol-breaking-006（嵌套 multicol R113）均结构性。**结论**：clean-win 面经第二轮（R359）确认再穷尽，forward motion = 多会话 Phase A |

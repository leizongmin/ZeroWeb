...
> - **P1b（JS Bridge 原生化）**：解锁交互式网站（fetch/MutationObserver/事件循环）。独立 RFC [`p1b-rfc-2026-07-25.md`](./p1b-rfc-2026-07-25.md)。**S1（异步回调通道）+ S3（fetch）双路径 LANDED**（in-process browser + multiprocess renderer 默认路径均贯通；共享 `AsyncResolver`/`FetchBridge` 在 `zero-engine`）；⏳ 剩余 S5（setTimeout 真实延迟）/ S2（节点身份 v8::External → MutationObserver）。

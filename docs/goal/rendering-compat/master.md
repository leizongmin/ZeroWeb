...
| R355 paint 侧 line.y 补全 + ifc-009 残余根因定位 | R355/R356 | R355 paint 侧 line.y 偏移（text.rs:832）是 Gate 2（ca14d05）的 load-bearing 配套——stash 对照证 HEAD Gate-2-only ifc-008 4.17% FAIL → +fix **0.00% PASS**；self-source **439→440/490 净 +1 零回归**（commit 787677e）。R356 续查同簇 ifc-009（2.08%）残余：探针（已回退）实证 float 排除 y 重叠判定用的 line_height=**20（default）非 100px** → current_y 每行仅 +20 → float[0,100] 错误排除 line 2-5（应只排除 line 1）→ 第 3 个 X 被挤到 line 3 → 右下 100×100 应蓝实橙。根因=跨 block 边界收入父 IFC 的文本 run line_height 回落 default（Phase A font-metric 耦合第三机制，同 R125-R213 谱系），单会话修复高回归风险。详见 [`evidence/r356-ifc009-lineheight-float-overlap-pinpoint-2026-06-20.txt`](./evidence/r356-ifc009-lineheight-float-overlap-pinpoint-2026-06-20.txt)。**结论**：R355 是真 +1 clean win（已提交）；ifc-009 残余精确化为 Phase A 第三机制，未修 |

...
**R473 css/CSS2 全量导入 DONE + self-dump & oracle 双线 IN-FLIGHT（tracking，R465-R471 css-text 谱系）**：并行 agent 已 commit **40ca95b7「import CSS2 full WPT data (10th/final DC-14 dir, 13027 files)」**= 全量导入 DONE。**6315 matched pairs**（`link rel=match`）/ 9265 test-ish —— **精确确认 R472 估计**（~5500-6500 pairs / 9253 test-ish，无 truncation）。**self-dump + oracle 双线 in-flight 自 18:18**：① self-dump PID 4861 `reftest-upstream CSS2`（test-guard `--time-limit 7200`，@18:21 已 2:42，272 `-test.png`，~4.8GB RSS / 1288% CPU 健康）；② oracle PID 4901 `capture-oracle-per-dir.mjs --category css/CSS2`（chromium PID 4914 健康，~110 PNG @1.1s/PNG 早段 ≈ 55/min，ETA ~3hr ~21:00-21:30 for ~9000 test-ish）。⚠️ **time-limit 风险（监控点）**：CSS2 ~4× css-text 用例量；self-dump 按 css-text 速率（1572 pairs/1229s）→ 6315 pairs ~82min，**likely 适配 7200s(2hr) 但 borderline**——若复杂子目录（tables/generated-content/pagination）per-case 慢可能逼近/超 2hr 被 test-guard 杀→partial dump；**若 ~20:00 仍未完须重启或 bump time-limit**。零代码变更。下一步：双线多小时完成 → cross-validate（输入齐即 `cross-validate.py --dump target/reftest-dump --oracle oracle-shots`）→ 验 R472 预测 ~30-42% → **10-dir 最终 DC-14 全量基线**（CSS2 = 最大聚合 mover，落点定 25% vs 31%）。**R473 监测更新（18:23，read-only 去险 + 风险下修）**：① **cross-validate naming 已验对齐去险**——oracle `css_CSS2_<sub>_<name>_xht.png` ↔ dump `-test.png`/`-ref.png`，`cross-validate.py:65-72` `sid()` 拼接正确（abspos 簇 -004a/004b 双侧齐验过；-001 dump 未跑到=非 naming 问题）；-test/-ref 等量生成（494/494）✓。② **time-limit 风险下修**：self-dump 实测 **~100/min**（494 `-test.png`/4.5min，快于 css-text ~77/min），6315 pairs ETA **~19:21 远低于 2hr 限制**——R473「borderline」判断偏保守，实际健康，**~20:00 cutoff watch 可解除**；oracle ~50/min（215 PNG），ETA ~21:00-21:30 for ~9000 test-ish。双线无卡点。**R474 监测（18:28，read-only，无新结论·分母精确化）**：双线续健康线性推进，ETA 不变（self-dump **1040/6109 pairs** ~17% @~97/min ETA ~19:21；oracle **519/~9265 PNG** ~5.6% @~48/min ETA ~21:30；`-test`/`-ref` 1040/1040 等量 naming 对齐 ✓）。**分母精确化（数据质量注释，影响最终基线对账）**：runner 实跑 **6109 cases**（**非** R473 header 引用之 6315 link-rel-match）——**174 CSS2 missing-ref skips**，共享 ref 在 `../../reference`(157)/`/css/reference`(13)/css/CSS2 子目录(数个)，**不在 css/CSS2/ 导入子树内故未导入**（scoping artifact，非 ZW bug；潜在恢复=并行 agent 增导 `/css/reference/`，非本侧 action）；cross-validate 有效分母 ≤6109（`cross-validate.py:74` 跳过缺 `-ref.png`）。无卡点，零代码变更。下一实质节点不变：~19:21 self-dump 完成 → oracle 子集大样本 cross-validate 早期信号。
...
**R465 css-text 全量 oracle 捕获 IN-FLIGHT + 精确 self dump + cross-validate 准备（read-only 观测）**（**↑ 已解决：oracle 18:04 完成、结果见上 R471；以下为 in-flight 逐轮观测史 R465-R470，可后续归档**）：R464 修复（collectTests 递归）**已生效**——并行 agent PID 72483 `node capture-oracle-per-dir.mjs --category css/css-text`（17:25 启动，chromium headless ver 149 活跃，3 renderer）。**修复验证**：已跑 2:39 未完成（csstext_oracle2.log 0 字节）→ 旧顶层版 ~13s 完 8 个即退出，2.7min 未完 = 递归版遍历 1914 个 ✓。⚠️ capture-oracle-per-dir.mjs **仍未 commit**（working tree ` M`）——并行 agent 用 working tree 版直接跑，验证后再提交。预判 ~51min（当前 ~5%）。**⚠️ R468 实测进度再修正（17:46 观测，纠正 R467 早段乐观速率）**：R467 的 **0.77s/PNG**（@10min 早段）系简单子目录（animations/parsing）速率，进入 shaping/bidi 复杂脚本区后**速率回落到 ~1.2s/PNG（~50/min）**——R467「比 writing-modes 快一倍」仅对早段成立，未外推到整批。oracle 已捕获 **1010/1987=51%**（true css-text PNG，精确口径：`css_css-text_*` 排除 css-text-decor 246 张 R417 遗留；分母 1987 = css-text 树 test-ish，捕获全量，cross-validate 仅用 ~1572 匹配对）。@elapsed 20:53，剩余 977 张 @50/min ≈ **~20min**，预计 **~18:06 完成**（**非 R467 的 17:50，晚 ~16min**；勿以「过 17:50 仍在跑」误判 stall——R458/R459/R464 已三度误诊 stall，本校正防再触发）。chromium 主进程 PID 72495 健康（renderer 活跃，最新 PNG 17:46:21 写入）。self dump（PID 73090，~21min，13 并发 job，/proc/stat utime 持续增长）健康推进；dump log 的 `document is not defined`/`Runtime error` 为 24.5% JS 依赖用例预期噪声（R462），非卡点，结果 runner 末尾汇总输出。两者无卡点，~18:06 齐完 → cross-validate。**R469 更新（17:49）**：self dump（PID 73090）✅ DONE — **1402/1572=89.2%**（170 fail，与 R463 精确一致；duration 1229s≈20.5min），REFTEST_DUMP 产物 **1570 张 ZW-test PNG**（`target/reftest-dump/css_css-text_*-test.png`）= cross-validate input **① ready**。input ②（chromium oracle）仍在捕获 **1190/1987=60%**（rate ~51/min，最新 PNG 17:49 活跃），剩 ~797 张 @51/min ≈ ~16min，ETA **~18:05**——R468 预测 ~18:06 命中（误差 <1min，速率校正成立）。oracle 完成（input ②）即可 cross-validate（input ① 已备）。**并行 agent 同时跑精确 css-text self dump**（/tmp/csstext_dump.log 17:27，`Running 1572 upstream reftest cases`）——**修正 R463 decor 误匹配**：本轮 filter `css/css-text/` 带斜杠精确 1572（排除 decor 250），为 cross-validate 备 ZW-test 像素（self 通过率预期仍 89.2% 同 R463）。**cross-validate 流程在准备**（两输入齐即算 chr<1%）：① chromium oracle 截图（捕获中 ~51min）+ ② ZW-test 像素 dump（1572 跑中 ~22min）→ scripts/cross-validate.py 逐案 z_vs_chr → chr<1% 一致率 = DC-14 真指标验证 R461 30-38%（参考 writing-modes R457 两步流程）。零代码变更。下一步：等 oracle+self dump 完成 → cross-validate → 验证 R461 → 9 目录 oracle 聚合 → CSS2。**R470 预检（17:55，read-only，cross-validate 去险）**：oracle 进度 **~74.5%（1480/1987）**，PID 72483 仍活跃（chromium PID 72495 健康，最新 PNG 17:55:35），ETA ~18:05 不变；self dump（PID 73090）已 DONE。**input ① 已验齐**——`target/reftest-dump/` 含 **1572 `-test.png` + 1572 `-ref.png` → 1570 matched pairs**（2 safe_id 缺 -ref，被 cross-validate.py:74 跳过）。**cross-validate 已去险**：naming 对齐（oracle `{sid}.png` ↔ dump `{sid}-test.png`/`-ref.png`，`cross-validate.py:65-72` `sid()` 去尾 `.png` 后拼 `-test/-ref` 正确），oracle 完成即可单命令跑出 chr<1% 一致率验 R461 30-38% 预测：`python3 tests/wpt-runner/scripts/cross-validate.py --dump target/reftest-dump --oracle tests/wpt-runner/oracle-shots` → cross-validate → 验证 R461 → 9 目录 oracle 聚合 → CSS2。详见 [`evidence/r465-csstext-oracle-capture-inflight-2026-06-21.txt`](./evidence/r465-csstext-oracle-capture-inflight-2026-06-21.txt)。
...
**基线（R471 css-text oracle 完成后——9/9 目录 self+oracle 完成（9 目录 oracle 聚合 chr<1% 23.9%）·5 布局类+4 文字类，DC-14 Phase 2）**：
...
- **9 目录全量 chromium-Oracle 真一致 chr<1% 23.9%**（R471 css-text 折回后）：css-grid 19/48=39.6% / css-position 36/95=37.9% / **css-tables 49/112=43.8%** / **css-flexbox 251/496=50.6%（最高，R456 probe+R429）** / **css-multicol 106/451=23.5%** / **css-text-decor 70/242=28.9%** / **css-fonts 98/282=34.8%** / **css-writing-modes 44/784=5.6%（R457，迄今最低，538 near-miss 1-5% font/metric 噪声；chr<5% broad=74.4%）** / **css-text 263/1402=18.8%（R471，R461 预测 30-38% MISS；mean z_vs_chr 2.97%，66% 案 1-5% near-miss，chr<5% broad=84.7%）**（chr<1%，聚合 936/3912=23.9%；css-text 18.8%×大分母 1402 把 8-dir 26.8% 下拽至 23.9%，writing-modes 5.6% 仍为最低）。布局类 6 目录聚合 505/1986=25.4%（writing-modes+multicol 拖低）/ **文字类 3 目录 431/1926=22.4%（css-text 加入后首次低于布局类；原 2-dir 文字 168/524=32.1%）**；writing-modes chr<1% 5.6% 仍为迄今最大缺口目录（multicol 23.5% 次之，css-text 18.8% 第 3）

...
**🎯 当前最高优先级（2026-06-17，R228 welcome 剩余 17% 根因重定位后）**：**welcome 布局结构已对齐，剩余 diff 是 font-weight 未落地的字形保真度缺口**。R227 padding 双计修复把 welcome 28.34%→17.06%；R228 用 `LAYOUT_DUMP` + ink-mass 区域分析证伪「底部第二处布局 bug」——盒树几何正确（hero-accent 实心条 0.4% 差、背景沟槽近零差、带状 diff 已均匀），**17% 的真因 = `font-weight` 600/700 未选粗体变体**（FontLoader 仅按族名索引无 weight 维度 + paint 不传 font_weight）：title(700) ink 仅 CH 的 22%、eyebrow(600) 10%、card-titles(600) 66% 欠墨，CJK/normal 141-203% 过墨，欠/过严格跟随 font-weight。**下一步 R229** = paint 把 font-weight 从 ComputedStyle 透传到 fontdue 字形光栅化 + FontLoader 加 weight 维度选变体（先核实是否已加载同族多 weight 字体资源），以 welcome title/eyebrow/card-title ink-mass 回升到 ~100% CH 为成功标准（product-smoke 复测）；自源中性（不刷 438）但直接攻 DC-14（R221 188/475=39.6%，很可能覆盖 183 case 1-3% 聚类中含 font-weight 的用例）。⚠️ **并行 agent 正在 `crates/layout-engine/src/table.rs` 改 colspan/`<col>` border-collapse 列宽（R177/R178 谱系）——本方向（render-foundation font + engine paint text）不碰 table.rs 无冲突**。⚠️ R228 纠正上轮「找第二处布局双计」假设（ground truth 证伪）。⚠️ 持续 ruled out（勿再追）：advance-width（R225 死路）/ multicol paint 侧切片（R203 net-negative）/ multicol balance 两趟（R200 已正确）/ chromium 高 diff 候选 abspos-stretch·backdrop·fixed-bg（R202 特性缺口）/ DC-12（R197 全实现）。⚠️ 仍开放的多轮硬架构（非单会话，非当前杠杆）：multicol-breaking 需 **layout 侧 column-aware IFC**（R131）；DC-9 GPU 收尾仅 transform/filter/blend 三项（R220 clip=空谈，需 ping-pong 双纹理，reftest 低频非 load-bearing）；DC-14 chromium-oracle 严格容差默认接线（infra 就绪待默认化）。

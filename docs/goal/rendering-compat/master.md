...
**最后更新**: 2026-06-20（R348：fresh chromium-Oracle 复测**确证 plateau 稳定**——R324/R325/R326 三处 correctness 修复（position:fixed / img aspect / sticky，均 self-source 438/490 零回归）经独立 Oracle 复测：**零 polluted case 的 chromium diff 被移动**。用 490 张当前 ZeroWeb dump vs 503 oracle shots（475 comparable）cross-validate：污染率 **48.0%**（R311 为 48.2%，噪声内）；**top polluted case 与 R311 逐一同名同 chr%**（backdrop-inherit-rendered 47.54% / abs-pos-border-offset-002 27.94% / table-grid-item-dynamic-003 23.79% / position-abs-semi-replaced-stretch-input 23.03% / downloadable-font-scoped 20.22% / flexbox-baseline-align-self 17.65% / multicol-contained-absolute 16.33%——全为已记录结构性聚类）；逐目录污染与 R311 逐一相同（CSS2 56 / flexbox 26 / fonts 45 / grid 50 / multicol 60 / position 38 / tables 41 / text-decor 47 / writing-modes 73）。**结论**：plateau 不仅聚合稳定，**逐 case 逐 chr% 稳定**——R324/R325/R326 改善的是已接近 chromium 的 case（或测量集外），未降系统性污染；无新可修真 bug 浮现。详见 [`evidence/r348-cross-validate-fresh-post-r324-r326-2026-06-20.txt`](./evidence/r348-cross-validate-fresh-post-r324-r326-2026-06-20.txt)。**结构性结论**（不变）：rendering-compat 全面 plateau（R305–R348，41+ 轮），唯一 forward path = 多会话 Phase A IFC 统一，需用户决策。

**前轮 R347**：2000 行规则全仓库达标——`reftest.rs`（DC-1 引擎）2379→1868 拆出 `reftest/resources.rs` 子模块（资源加载集群 18 函数，`pub use` 再导出 convert_png_buffer_to_rgba），harness 自验证 make reftest 686/686 + upstream 438/490 零回归。至此**所有非 vendored `.rs`（生产 R343/R344 + 测试 R345/R346/R347）均 <2000**（vendored taffy-local/flexbox.rs 2313 不动）。再前 R343-R346 完成生产源码 + 2 测试文件拆分。

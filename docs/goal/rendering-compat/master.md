...
**R424（read-only 在途状态复核，已被 R425 真数据取代）**：css-flexbox 第 6 目录在途观察（oracle-capture 阶段）。最终结果见上方 R425 折回（self 49.6% / oracle 49.2% / 6 目录聚合 40.5%）。

**R430（read-only pre-confirm，第 7 目录备基线）**：css-multicol 并行 agent 正跑 self-source reftest（进程 42782→46005 两轮测量，in-flight 非停滞）。pre-confirm 完成：权威 **331 对**（旧标 537 含非 reftest，史上第 2 大分母仅次于 flexbox 502），主导聚类（column-height 30/balance 族 33/nested 28/fill 25/breaking 13）全结构性，R353 缺失特性核查（column-span/column-height/orphans·widows 未实现；balance 核心已实现但精度受限）仍成立，**预测 oracle chr<1% ~25-33%**（低于布局类 band 37.9-49.2%，近 text-decor/fonts；若成立把 7 目录聚合从 40.5% 下拽至 ~38-39%）。详见 [`evidence/r430-csmulticol-preconfirm-2026-06-21.txt`](./evidence/r430-csmulticol-preconfirm-2026-06-21.txt)。
...
- self-source 全量目录：**css-grid 30/48=62.5%** + **css-position 61/95=64.2%** + **css-tables 75/112=67.0%** + **css-flexbox 246/496=49.6%** + **css-multicol 195/451=43.2%**（布局类，runner 容差可直接比；multicol 最低=结构性缺口最多）+ **css-text-decor 244/246=99.2%**（@0.5% strict 185/242=76.4%）+ **css-fonts 282/284=99.3%**（@0.5% strict 249/282=88.3%）（文字类容差口径，与布局类不直接可比）（R401 grid / R404 position / R408 tables / R425 flexbox / R427 multicol / R417 text-decor / R422 fonts；子集 self 高估）。**7 目录聚合 self-source 1133/2032=55.8%**（文字容差抬升；strict 口径更低）；**混合口径（7 全量 + 2 仍 60-子集），非全量真通过率**
- **7 目录全量 chromium-Oracle 真一致 36.0%**：css-grid 19/48=39.6% / css-position 36/95=37.9% / **css-tables 49/112=43.8%** / **css-flexbox 244/496=49.2%（最高）** / **css-multicol 106/451=23.5%（最低）** / **css-text-decor 70/242=28.9%** / **css-fonts 98/282=34.8%**（chr<1%，聚合 622/1726=36.0%，multicol 下拉自 6-dir 40.5%）。布局类 5 目录聚合 454/1202=37.8%（multicol 拖低）/ 文字类 2 目录 168/524=32.1%；multicol chr<1% 23.5% 低于文字类 text-decor 28.9%=迄今最大缺口目录，全簇 self-fail 74-90% 印证 plateau「multicol 最大失败聚类」
...
- self-source 全量目录：css-grid **30/48=62.5%** + css-position **61/95=64.2%** + **css-tables 75/112=67.0%** + css-flexbox 246/496=49.6% + **css-multicol 195/451=43.2%** + **css-text-decor 244/246=99.2%**（strict 76.4%）+ **css-fonts 282/284=99.3%**（strict 88.3%）。**7 目录聚合 self-source 1133/2032=55.8%**（文字容差抬升，strict 口径更低）；**混合口径（7 全量 + 2 子集），非全量真通过率**
...
1. **【优先·7 目录已完成】DC-14 分母去子集化** — gating DC-14 硬门禁（goal line 317/843）：达标判定前必须用上游每目录**全量** reftest。最可操作的多会话增量 = Phase 2 目录全量导入（grid ✅ / position ✅ / tables ✅ / **flexbox ✅** / **multicol ✅** / text-decor ✅ / fonts ✅）→ 再扩中/大目录（writing-modes 855 / css-text）→ 最后 CSS2（~5000-7000）。每批 reftest + chromium-Oracle 复测，监控真通过率。**状态**：**7/9 目录全量完成（5 布局类 + 2 文字类）**——R401 grid（62.5%/39.6%）+ R404 position（64.2%/37.9%）+ R408 tables（67.0%/43.8%）+ R425 flexbox（49.6% self / 49.2% oracle·最高）+ **R427 multicol（43.2% self / 23.5% oracle·最低）** + R417 text-decor（99.2% self / 28.9% oracle）+ R422 fonts（99.3% self / 34.8% oracle）；7-目录全量 oracle **36.0% 真一致**（聚合 622/1726，multicol 下拉自 6-dir 40.5%）。工具链（`discover-reftests-authoritative.py` + `capture-oracle-per-dir.mjs`）可复用于余 2 目录。**下批：css-writing-modes（第 8 目录，855）** → 余 css-text/CSS2。详见 [`evidence/r427-cssmulticol-full-2026-06-22.txt`](./evidence/r427-cssmulticol-full-2026-06-22.txt) + [`evidence/r425-cssflexbox-full-2026-06-22.txt`](./evidence/r425-cssflexbox-full-2026-06-22.txt) + [`evidence/r421-cssfonts-full-2026-06-22.txt`](./evidence/r421-cssfonts-full-2026-06-22.txt) + [`evidence/r412-csstextdecor-full-2026-06-22.txt`](./evidence/r412-csstextdecor-full-2026-06-22.txt) + [`evidence/r405-dc14-three-dirs-complete-2026-06-22.txt`](./evidence/r405-dc14-three-dirs-complete-2026-06-22.txt) + [`evidence/r401-dc14-grid-full-authoritative-2026-06-22.txt`](./evidence/r401-dc14-grid-full-authoritative-2026-06-22.txt) + [`evidence/r394-dc14-denominator-gap-2026-06-22.txt`](./evidence/r394-dc14-denominator-gap-2026-06-22.txt)。
...
| Flexbox 通过率 | ❌ 未达标 | 全量 self 49.6%（布局类容差；multicol 43.2% 更低）/ strict 42.1% / **chromium-Oracle chr<1% 49.2%（7 目录最高）**（主拖 aspect-ratio transferred size 49 例 90% self-fail + flex min-size:auto 57 例 79% + shorthand 152 例 65%），未达 95% |
...
- self-source 全量目录：css-grid **30/48=62.5%** + css-position **61/95=64.2%** + **css-tables 75/112=67.0%** + css-flexbox 246/496=49.6% + css-multicol 195/451=43.2% + **css-text-decor 244/246=99.2%** + **css-fonts 282/284=99.3%**（文字容差）；**7 目录聚合 self 1133/2032=55.8%**；聚合（7 全量 + 2 子集）；**混合口径，非全量真通过率**
- self-source strict **295/490 (60.2%)** @ 锁定 0.1%/0.5%（DC-14 真通过口径）
...
- [ ] **多会话架构承诺 vs 接受 plateau**：7-目录全量 self 1133/2032=55.8% / oracle 622/1726=36.0%（grid/position/tables/flexbox/multicol/text-decor/fonts）+ 456/518 loose（含子集）/ 295/490 strict（pre-grid-expand）/ ~42% Oracle（broad, pre-grid-expand）是诚实基线。剩余提升需 DC-14 全量去子集化（R401/R404/R408/R425/R427/R417/R422 已完成 grid/position/tables/flexbox/multicol/text-decor/fonts 7 目录，2 目录待扩：writing-modes/css-text/CSS2）+ Phase A IFC 统一 / Phase 2 嵌套 multicol / baseline 合成 或 taffy 升级，均为多会话工程。R314 已飞书通知。

...
| M10 — 上游 WPT 真实 Reftest 导入 | 🔧 进行中 | 基础设施 ✅(PNG 解码+ImageCache+base_dir 路径解析+discover 脚本)；render_full_scene 全量渲染 ✅(13 种图元)；skip_indicators 模式 ✅；UA 默认样式 ✅；493 个上游 reftest 已导入（9 个目录）；总体通过率 77.3% (380/493)；css-text-decor 100.0% ✅；css-fonts 98.3% ✅(≥95%)；css-writing-modes 94.9%；css-grid 85.0%；css-multicol 70.2%；css-tables 67.9%；CSS2 69.2%；css-flexbox 56.4%；css-position 58.8%；**本轮改进**：gap 简写 handler 修复 ✅(不再覆盖 column_gap/row_gap)；table 行组位置更新 ✅(update_row_group_positions 按视觉顺序重排行组+relative 偏移传播+spacing 计算)；out-of-order-elements-collapsed-border ✅(从 46.32% 修复到通过)；剩余：border-collapse、writing-mode、column breaking、flexbox baseline |

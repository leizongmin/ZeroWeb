...
**R428–R437 flex min-size:auto probe（✅ 并行 agent 已 apply 工作树 @11:45:46，待 commit；逐轮详情归档 [`archive/rounds-r432-r437-flex-minsize-probe.md`](./archive/rounds-r432-r437-flex-minsize-probe.md)）**：probe = `default_impl.rs` `min_width/min_height: Px(0.0)→Auto`（R428，全局默认）。**R435 per-dir 真数据**：css-flexbox 246→260/496（+14，49.6→52.4%）/ css-grid 30→31/48（+1，62.5→64.6%）/ css-multicol 195/451 不变（43.2%，中性）/ **0 净回归**——~120 轮 plateau 首个 forward motion。**机制（R433 taffy-local 追踪）**：旧 `Px(0.0)`→converter `Some(0.0)` 短路 taffy `resolved_minimum_main_size.unwrap_or({min-content floor})`（flexbox.rs:774），屏蔽 §4.5(flex)/§6.6(grid) 内容下限（ZeroWeb bug：CSS initial 当 0）；Auto→`Dimension::Auto` 让 taffy 算内容最小值，block floor 冗余安全。**R444 实测**：rate-limit（11:42 reset）后并行 agent 已把 probe re-apply 到工作树（`default_impl.rs:30-31`=`LengthValue::Auto`+R428-R437 注释，commit-ready，未带回归测试），**防 context-reset 警告生效**——未丢弃、直接 re-apply；待 commit + fold-in 基线（flexbox 49.6→52.4%）。**R444 复核**：`git log 878ee8f..HEAD -- crates/layout-engine/ crates/style-system/` 空（仅 4d2e93d multicol panic fix），R435 数据仍有效。min-size 修复 partial（残余 ~31 cross-axis/交互 + aspect-ratio 34[taffy R304]）。**下个 ZeroWeb-side lead = R429 flex shorthand type-awareness**（`shorthand/mod.rs:674-688` `expand_flex` 位置式非 spec type-based：`flex:1 100px`→shrink=100px 错[spec basis=100px]、`flex:0 auto`→shrink=auto 错[spec basis=auto]、`flex:50%`→grow=50% 错[spec basis=50%]；R444 代码复核确认 type-mixing 形，纯数字形 `flex:1`/`1 0`/`2 1 100px` 已对）。**R445 影响面量化（grep css-flexbox wpt-data）= 仅 ~2-4 case 命中**（`flex:0 auto`/`flex:1 auto` 2 文件 + `flex:<len>` 2 文件），**SMALL clean win，非 152 例 shorthand 簇主体**（主体=flex-basis/grow/shrink 与 definite-size 的 taffy 交互）。**战略含义**：flex probe 是 css-flexbox 最大且近唯一的 ZeroWeb-side 杠杆；commit 后 ZeroWeb-side lever 要么 small（R429 ~4）要么 taffy-blocked（aspect-ratio 49 例 R304），前向 motion 转向 DC-14 分母去子集化（第 8 目录）+ 多会话架构。详见 [`evidence/r445-flex-shorthand-impact-scan-2026-06-21.txt`](./evidence/r445-flex-shorthand-impact-scan-2026-06-21.txt)。 ｜ **R448（read-only 独立 footprint 核查，probe 确近 commit-ready）**：hand-written 单元测试 footprint = **恰好 1 个**——`test_flex_shrink_behavior`（tests_3.rs，shrink=1,2>0 + width:200px 断言收缩到 ~150/100px，min-content floor 阻止收缩），**已被并行 agent 修复**（显式 min_width:0）。converter 路径核查（mod.rs:352 `convert_length_to_dimension`：`Auto→Dimension::Auto` / `Px→Length(Some)`）确证 R428 机制不变；**默认 flex_shrink=1.0**（default_impl.rs:117，CSS initial）故 grow-only 测试亦会收缩，但 `test_compute_flex_grow`（items 无显式宽、仅 grow 填充、弱断言 root.width>0）/ `test_flex_many_items_overflow`（shrink=0 不收缩）/ tests_5 + converter/tests/basic.rs（shrink=0 或纯值 round-trip）**均不触 floor**=零 footprint。**结论**：hand-written 侧 footprint 已闭，probe 仅余 685 内联 reftest 全量确认（编码 CSS-spec 预期应通过），确近 commit-ready；落地后 reconcile 基线（flexbox self 49.6→52.4% / oracle 待复测）+ fold-in 补 default-change footprint 说明。 **R450 复核（read-only，非停滞）**：probe「pending commit」延续多轮=**`reftest-upstream` 全量正由 test-guard 包裹运行**（PID 95938，~82 CPU-min/~5GB）做跨 7 目录最终验证（非 R448 所述「685 内联」smoke，而是更重的 7-dir 全集~1700 case），确认 R435 跨目录 delta（flexbox +14 / grid +1 / multicol 中性 / 0 回归）后即 commit；后续轮次勿将此「pending」误判为停滞。 **R451（read-only，reftest 慢于基线估测的合理解释）**：reftest-upstream 已跑 ~17 wall-min/225 CPU-min 仍未出 summary（1298% 并行持续=非卡死、CPU 时间持续增=推进）。最可能解释=**probe 自身使每 case 变慢**：min-size 默认 `Px(0)→Auto` 让 taffy 对每个 flex/grid item 计算 content-based 自动最小尺寸（CSS §4.5/§6.6，比直接用 `Length(0)` 更重，converter 路径 R448 已确证），1924 case 多数含 flex/grid item → 全局 per-case 成本上升 → 套件整体变慢（约 +30%）。**非卡点、会跑完**；属 probe 用小幅 perf 换 CSS 正确性的预期代价，后续 probe-inclusive reftest 同样会偏慢。 **R452（read-only，reftest 已完成 + probe temp-revert 进入 per-dir A/B；纠正 R450/R451「running」框架）**：full-suite probe-applied reftest **已完成**——**1321/1924=68.7% self-source**（注：与已知 490-子集 90.4% 基线**分母不同不可直接比**；尚无匹配的 1924 pre-probe 基线）。probe 已 **temp-revert**（default_impl.rs 回 `Px(0.0)`），并行 agent 现正测 **per-dir CLEAN 基线**补 R435 spot-check 未覆盖目录（CSS2@12:40 / css-writing-modes@12:41 已起测）——属 R436 式 **careful A/B、非放弃**；test-fix(tests_3.rs min_width:0)+multicol 守卫仍在工作树。commit 与否待 per-dir delta 确认（R435 仅测 flexbox/grid/multicol；全 1924 含 CSS2/writing-modes 等未测目录）。R450/R451「reftest running」状态已被本轮取代。

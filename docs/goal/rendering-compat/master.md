...
**最后更新**: 2026-06-19（R308 font-size 百分比解析修复落地：computed.rs font-size 调用点就地解析 Percentage=父 font-size 百分比（旧 bug 返回原始数值当 px，500%→500px）；anonymous-inline-inherit-001 chromium-Oracle **3.84%→0.43%**；loose 438/490 持平零回归；strict 296→295 一处 revealed false-pass=font-features-across-space-1 连字测试被 150px bug 掩盖；make test 12254/0；R307 关闭 near-pass frontier 后**逐项 probe 未调查 POLLUTED 候选仍发现真实单点 bug**）

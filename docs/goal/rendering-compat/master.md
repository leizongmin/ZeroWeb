...
**R428–R452 flex min-size:auto probe — ✅ LANDED（commit 574db50 probe+test-fix / 45faa3e multicol inf-loop 守卫；~120 轮 plateau 后首个 ZeroWeb-side clean win）**：根因（R428 read-only 定位，**非 taffy-blocked**）= style-system 默认 `min_width/min_height=Px(0.0)`（CSS §4.5/§6.6 initial 实为 `auto`）→ converter `Some(0.0)` 短路 taffy 0.7 `resolved_minimum_main_size` 的 content-based floor（flexbox.rs:774），屏蔽 flex/grid item 自动最小尺寸。修复=默认改 `Auto`→converter `Dimension::Auto`→taffy 算内容下限（block 元素 taffy 视 Auto 为 0，行为不变）。**R435 per-dir 实测（pre-commit，已随 commit 生效）**：css-flexbox self **+14**（246→260/496=49.6→52.4%）/ css-grid self **+1**（30→31/48=62.5→64.6%）/ css-multicol 中性 / **0 净回归**。full-suite probe-applied reftest=**1321/1924=68.7% self-source**（无匹配 1924 pre-probe 基线直接比；490-子集 90.4% 分母不同）；并行 agent per-dir CLEAN A/B 补 R435 未覆盖目录：**css-writing-modes self 49→53/59 (+4)**（in-flight /tmp preprobe @ HEAD probe，0 回归——confirm probe delta 扩展至第 4 目录；6 失败全落已知结构簇 abs-pos-vrl/vlr R334-R336 + clearance R114/R164 + orthogonal-float，无新 clean-win，印证 R447「0 新单会话 lever」）；**CSS2 per-dir A/B 已起测**（in-flight /tmp preprobe @ HEAD probe：self 子集 120/129=93.0%，无回归迹象，精确 pre/post delta 待补）。**writing-modes 全量导入（855 目录·第 8 目录）尚未起**（仍 121 子集文件，最新 06-07/11；preprobe 仅完成 oracle 预捕获，并行 agent 先转向实施 R429 见下），R447 pre-confirm 预测 oracle ~33-42%、待落基线。**hand-written footprint=1**（`test_flex_shrink_behavior`，已随 commit 加 `min_width:0`；R448 独立核查 converter 路径 mod.rs:352）。**代价**=probe 使 reftest 每 case 略慢（taffy 算 content-min，R451；CSS 正确性 > perf）。**残余**=partial（~31 cross-axis/交互 + aspect-ratio 49 例 taffy-blocked R304）；**第二 ZeroWeb-side lead=R429 flex shorthand type-awareness ✅ LANDED（commit 88e11d2）**——expand_flex 位置式→type-based 重写 CSS §7.1（修 `flex:50%`→basis、`flex:1 100px`/`flex:0 auto`→grow+basis 旧位置式误解析，+测试 footprint）。**实测 css-flexbox self net 0 delta**（260/496→260/496=52.4% 不变，`flex_r429.log` vs `flexbox-after-minsize.txt`；R445「~2-4 case narrow clean win」预测未兑现为 self 净通过，但 **R456b 隔离 oracle = +1 case（probe-only 250→probe+R429 251）= 真 DC-14 win**（selfsource 同源抵消 net-0，oracle 揭示真匹配 chromium，纠正 R455「非 lever」）；非 152 例 shorthand 簇主体）。**chromium-Oracle 复测 ✅ R456/R456b**：probe/R428 +6（244→250）+ R429 +1（250→251）= **总 +7 vs R425**，当前 251/496=50.6%（self 0 + oracle +1 已隔离）。R432–R437 逐轮详记见 [`archive/rounds-r432-r437-flex-minsize-probe.md`](./archive/rounds-r432-r437-flex-minsize-probe.md)；R448（footprint）/R450–R452（reftest 完成+A/B）已并入本段。
...
**基线（R457 css-writing-modes 扩到全量后——8/9 目录全量完成·5 布局类+3 文字类，DC-14 Phase 2）**：
- self-source 全量目录：**css-grid 31/48=64.6%** + **css-position 61/95=64.2%** + **css-tables 75/112=67.0%** + **css-flexbox 260/496=52.4%** + **css-multicol 195/451=43.2%**（布局类，runner 容差可直接比；multicol 最低=结构性缺口最多）+ **css-text-decor 244/246=99.2%**（@0.5% strict 185/242=76.4%）+ **css-fonts 282/284=99.3%**（@0.5% strict 249/282=88.3%）（文字类容差口径，与布局类不直接可比）+ **css-writing-modes 613/786=78.0%**（runner 布局容差；2 missing-ref 计入 fail）（R401 grid / R404 position / R408 tables / R425 flexbox / R427 multicol / R417 text-decor / R422 fonts / R457 writing-modes；子集 self 高估）。**8 目录聚合 self-source 1761/2818=62.5%**（文字容差抬升；strict 口径更低）；**混合口径（8 全量 + 1 仍 60-子集 css-text 待导入——R460 token gate 已移除，2 调用 runnable），非全量真通过率**

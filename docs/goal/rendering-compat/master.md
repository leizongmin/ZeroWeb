...
**最后更新**: 2026-06-19（R342b：继续 2000 行规则收尾——`inline/mod.rs` 2127→1916 行拆分，抽出 `inline/text_metrics.rs`（229，字符宽度估计 / AdvanceSource / font-metrics / BiDi 辅助）。**纯移动零行为变更**：reftest-upstream 438/490 字节级一致（同 52 失败同 diff%）、888 layout 测试过、clippy `-D warnings`（全 workspace --all-targets）干净；API 经 `pub use text_metrics::*` glob 再导出保持，零调用点改动。承接 R342（engine.rs 3371→1717，抽出 inline_finalization.rs 950 + float_positioning.rs 742）。至此 layout-engine 主要生产源码 engine.rs/inline/mod.rs 均 <2000；剩余违例 table.rs（2694）/ gpu/renderer/mod.rs（2359）独立项。**非 pass-rate lever**——plateau 本身不变（self-source 438/490 / strict 295/490 / Oracle ~35.6%）。详见 [`evidence/r342b-inline-text-metrics-split-2026-06-19.txt`](./evidence/r342b-inline-text-metrics-split-2026-06-19.txt)。**结构性结论**：rendering-compat 全面 plateau（R305–R342，36+ 轮）——全 9 reftest 目录 + WM-1×3 + multicol + welcome 垂直/精度 bounded lever 穷尽；column-aware-IFC Phase 1 经 R319 REFUTED（目标结构在失败集不存在）；border-001 = self-false-fail（ZeroWeb 离 chromium 仅 0.75%，DC-14）。唯一 forward path = 多会话 Phase A IFC 统一（墙② multicol + 换行精度同时解）+ 渲染器精度。需用户对「投入多会话 Phase A」vs「接受 plateau」决策）

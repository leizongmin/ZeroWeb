...
**R481 CSS2 oracle 监测（19:52，post-self-dump·read-only·无卡点·forward-look 实时复算 on-track）**：self-dump 完成 ~19:42 已确认（无 reftest-upstream 进程在跑，6106/6109）。oracle **4781/~9265≈51.6%** @~52-63/min，已进入 **normal-flow（387/733=R480 forward-look 预测的 n/p/s/t 路径首个大头子目录）**，positioning/selectors/tables/text 仍全 **0**（字母序 n 之后未抵达）。**ETA 实时复算印证 R480 预测在轨**：normal-flow 剩 346+pagination ~103+positioning 519+selectors 621+tables 376+text 565≈2530 @~52-60/min→覆盖完 'text' ≈**~20:40 代表性 unbiased**；全量剩 4484→**~21:15 完成**（与 R480 ~20:44/~21:19 吻合，oracle 进 normal-flow 时间点在轨，无 stall）。**决策不变**：未抵 unbiased 窗口（p/s/t/table 全 0），勿即刻 cross-validate（早段 borders/backgrounds-biased 会系统性高估真 chr<1%→误诊 R472 ~30-42% 预测）；等 ~20:40 oracle 覆盖 n/p/s/t → unbiased cross-validate → 验 R472 ~30-42% → **10-dir 最终 DC-14 全量基线**。零代码变更。

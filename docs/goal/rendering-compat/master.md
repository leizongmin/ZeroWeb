...
**R479 CSS2 双线 monitoring（19:24，read-only·无卡点·forward-look 经 fresh oracle 子目录覆盖实证确认；压缩 R473-R478 in-flight 监测段）**：并行 agent commit **40ca95b7**=CSS2 全量导入 DONE（6315 matched pairs / 9265 test-ish，精确确认 R472 估计无 truncation）。self-dump **5895/6109 ≈96.5%**（fresh `css_CSS2_*-test.png`，newest=border-bottom-width-034 @19:24:13 ≈now 活跃非 stall；~214 余案 ~3-5min 完成 ~19:27，远低于 test-guard 20:18 限时）/ oracle **3253/~9265 ≈35%**（newest=linebox/line-height-007，@~49/min ETA ~21:25）。**forward-look 经 fresh oracle 子目录覆盖实证确认（强于 R476-R478 预测）**：oracle 子目录覆盖 borders 762/backgrounds 626/fonts 324/generated-content 316/css1 262/floats-clear 249/…/**linebox 250 起步中（l\*）**，**未触达** dump 大头 normal-flow(n 857)/positioning(p 578)/selectors+syntax(s)/tables/text —— oracle 字母序约 a→l（~55% alphabet），抵 dump 大头 n/p/s 须 **~20:30+**。dump 自身已覆盖大头（normal-flow 733/selectors 545/positioning 519 等；处理序=大头先行、borders 在尾）。**故 self-dump 完成（~19:27）时 oracle ~3400 ≈dump 55% < 60% 门槛且 oracle 仍 a-l 早段 → 勿即刻 cross-validate**（早段 borders/backgrounds-biased 非代表性，会误诊 R472 ~30-42% 预测）；**unbiased 大样本窗口维持 ~20:30+** 当 oracle 抵 dump 大头 n/p/s。零代码变更。**durable facts（数据卫生/对账）**：runner 跑 **6109 cases**（非 6315 link-rel-match；174 CSS2 missing-ref skips 在 import 子树外，scoping artifact 非 ZW bug；潜在恢复=并行 agent 增导 `/css/reference/`）；naming 已验对齐（oracle `css_CSS2_<sub>_<name>_xht.png` ↔ dump `-test/-ref.png`，`cross-validate.py:65-72` sid() 正确，abspos -004a/004b 双侧齐验过）；dump dir `target/reftest-dump` 跨轮累积（naive `ls` 含 css-text 1572+writing-modes 786 stale，cross-validate 为 oracle-driven 仅比 `css_CSS2_*` sid 故对 CSS2 无害，但分母对账须按 fresh `css_CSS2_*`+mtime 计数）。R473-R478 逐轮 in-flight 监测原文已并入本条（结论一致：双线健康线性推进，无卡点；R473/R474 naming+分母去险、R476-R478 forward-look 预测均由本轮 fresh 覆盖实证确认）。下一步：oracle 抵 n/p/s（~20:30+）→ unbiased cross-validate → 验 R472 ~30-42% → **10-dir 最终 DC-14 全量基线**（CSS2 = 最大聚合 mover，落点定 25% vs 31%）。

...
> - **★ R2028（2026-07-25）P1b S1 incr1 LANDED**（轨 B 启动）：Sandbox trait 加 `resolve_async_callback(id, result)`（**默认 no-op**，QuickJS 降级继承，RFC v0.3 V8-first）+ V8 实现（防御性 execute `__zwResolveCallback(id,result)`，复用 `perform_microtask_checkpoint` drain `.then`）+ `js_string_literal` 转义器（防注入）+ 4 单测（端到端 Promise resolve / 未注入防御 / 注入转义 / 转义器）。方案 A 首增量：宿主回调同步返「回调 ID」，JS 建 pending Promise，Rust 异步完成后 `resolve_async_callback` 触发 resolve。**向后兼容**（`register_callback` 不变；默认 impl 零下游影响，全仓仅 V8Sandbox/QuickJSSandbox impl Sandbox）。script-sandbox 143/0（+4），clippy v8 干净，quickjs lib 编译。⏳ **S1 后续增量**：dom_bridge 注入 `__zwResolveCallback`+pending 表 JS + tab_js_worker marshal channel（跨线程 net/timer 完成 → JS worker 线程）+ 接通真实 fetch/setTimeout（S3/S5）。S2 节点身份（v8::External）独立。

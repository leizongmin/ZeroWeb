...
**🎯 当前最高优先级（2026-06-17，R109 主体完成后）**：① **line-height/字体度量对齐**（行高/ascent/descent 致卡片高度差，welcome/wintertc 文本块主因——R109 已落地，字体噪声已证伪，剩余产品 smoke 主因 = 行高度量）；② **R109 残余子问题**：clear-inline-001（inline img+span 经 converter→block 堆叠）、inline-box-002 残余（split inline 的 relative top 偏移在每个片段重复 → 需 relative-on-split 单次偏移）；③ DC-13 DONE#11「5 真实网站截图」+ wintertc fixture 图片密集验收。均用 chromium Oracle（`cross-validate.py`）验证，非同源通过率。

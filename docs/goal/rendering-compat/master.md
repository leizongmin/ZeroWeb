...
**最后更新**: 2026-06-20（R347：2000 行规则全量达标——`tests/wpt-runner/src/reftest.rs`（DC-1 关键 reftest 引擎）2379→1868 行拆分，抽出子模块 `tests/wpt-runner/src/reftest/resources.rs`（523，18 个自由函数的资源加载集群：HTML img/url/stylesheet 提取 + PNG/JPEG/SVG 图片解码 + 颜色/SVG 解析）。**纯自由函数移动零行为变更**：子模块 `use super::*` 复用 reftest 类型/导入；4 个被父模块调用的函数（extract_stylesheet_hrefs / load_linked_stylesheets / build_image_cache / extract_image_sizes）标 `pub(super)`；`convert_png_buffer_to_rgba` 保持 `pub` 经 `pub use` 再导出供 `main.rs`。集群自洽（不调用 reftest 其它函数）。**DC-1 harness 自验证**：make reftest 686/686（每个 case 经 render_to_framebuffer_with_base 调 build_image_cache/load_linked_stylesheets）、**upstream 438/490 字节级零回归**、make test 12265/0/72、clippy/fmt 干净。**🎉🎉 至此 2000 行规则全仓库达标（除 vendored taffy-local/flexbox.rs 2313）**——所有非 vendored `.rs`（生产源码 R343/R344 + 测试 R345/R346/R347）均 <2000。**非 pass-rate lever**——plateau 不变（self-source 438/490 / strict 295/490 / Oracle ~35.6%）。**结构性结论**（不变）：rendering-compat 全面 plateau（R305–R347，40+ 轮）——全 9 reftest 目录 + WM-1×3 + multicol + welcome 垂直/精度 bounded lever 穷尽；column-aware-IFC Phase 1 经 R319 REFUTED；唯一 forward path = 多会话 Phase A IFC 统一（墙② multicol + 换行精度同时解）+ 渲染器精度。需用户对「投入多会话 Phase A」vs「接受 plateau」决策）

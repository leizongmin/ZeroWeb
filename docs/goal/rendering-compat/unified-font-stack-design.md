...
> **📍 v0.2.2 addendum（2026-07-18，R1637–R1647）**：U1b-wiring + C3 font_id **dormant infra 全 LANDED**（零回归 verified），但 **R1639 战略结论 = font-stack 低 EV 端到端**，font-stack 不再作 active 推进方向，dormant infra 保留待用户授权 rebuild 时激活。★ **U1b-wiring 双路径 LANDED（dormant）**：R1637 切片 A（stored IFC 路径：`LayoutEngine.font_metric_provider` 字段 + setter + `compute_final_inline_layouts` provider 参数 + 递归 thread + 主 stored-IFC 站点 `.with_font_metric_provider`）+ R1638 切片 B（measure 路径：`measure_text_content` / `remeasure_text_with_float_exclusions` 加 provider 参数 + engine.rs 4 measure 调用点 thread）= **stored+measure 双路径 font_metric_provider threading 完整**，激活不再有 box-高 vs paint-行高 mismatch trap。★ **C3-enabling LANDED（dormant）**：R1639 `FontMetricProvider` trait 加 `font_id_of(family)->Option<u32>` + `FontLoader` impl（大小写不敏感）+ IFC `font_id_for_style` helper + 3 TextRun 构造点 populate `font_id`（替原 `None`）= 解 R223 font_id gap，advance_of 注入 AdvanceSource 时收真 font_id。三切片均 dormant（provider 默认 None → 字节等价旧路径，单测证机制 + make test 全绿 + product-smoke welcome 81512px/16.98% 字节一致）。★ **R1639 战略发现 = font-stack 三子杠杆全低 EV / blocked**：① **per-font line-height**（U1b 激活目标 1）：R1636 二次证伪（welcome 光栅噪声主导，post-R1263 Liberation Sans 字形已对齐但 1.150 metric 仍 +0.40pp 回归）；Ahem corpus per-font 度量 = 常数（无变化）→ 激活预期负向。② **C3 advance**（U1b 激活目标 2）：R225/R375 refute pre-R1263 DejaVu-only；**corpus 主体是 Ahem**（advance=font_size 精确，无 wall）→ 仅非-Ahem 子集受影响，且该子集光栅噪声 entangled；激活需 `AdvanceSource` 注入 = **FontLoader Rc-share**（reftest runner font_loader 被 painter `&mut` 占用，不可 Rc-share，data-map 方案不适用 advance 需字体字节）= 昂贵 refactor。③ **光栅化 C-dep**：R1560 real-Skia net-24 ruled out（font-wall 不在光栅化 coverage）。⇒ **font-stack rebuild 对 corpus pass-rate 是低 EV 路径**（line-height/advance/raster 三子杠杆全低 EV 或 blocked），font-wall 残余仅在非-Ahem 产品 fixture（welcome/morning/wintertc ~17%）且光栅噪声主导。★ **R1640–R1646 全 lever A/B 穷尽闭环**：per-font line-height activation nil（R1640）+ borders net-negative（R1642）+ vertical-mode 4 角度 net-negative（R1644/R1645/R1646）+ multicol frontier non-viable（R1593/R1594）+ struct-sweep 收口（R1576/R1578）+ 6 角度 plateau（R1579–R1585）= **in-environment rendering-compat DEFINITIVELY exhausted**（reftest ~57% = 2026-07-17 用户裁决接受的阶段性 plateau）。★ **裁决（v0.2.2）**：font-stack rebuild 作为「唯一理论 major-arch unlock」**需修正为低 EV**——对 corpus pass-rate 它是非 productive 路径（dormant infra 已就绪但激活路径三子杠杆全证伪/低 EV）。**dormant U1b/C3 infra 保留**（零回归 verified，待用户若授权 font-stack rebuild 时激活，复用 thread 模式 cheap）；**不主动推进 font-stack**（违 code-guidelines「不做零价值修改」+ R1639 低 EV 结论）。唯一剩余 forward = ~~待用户侧 chromium 环境修复（WSL2 SIGTRAP）解锁 legacy-html fixture oracle~~（**R1759 复核：已解除**——`scripts/install-oracle-chrome.sh` 装 chrome-for-testing 127 + headless:'new' 截图实测工作，DC-13 Tier 1 legacy-html 已有 51 fixture + 51 chrome-127 oracle PNG；R1640「chromium blocked」表述 stale）→ 实际剩余 = 待用户授权 font-stack rebuild（ruling #2 RFC-only，本 design v0.2 ready）或用户给新渲染方向；fresh corpus oracle 重抓 GUI-CDP 路径需 xvfb/display（headless 不可复现 corpus 标准 capture 方法，但新 capture 经 headless:'new' 可用）。

> **📍 v0.2.3 addendum（2026-07-24，R1946–R1950）·EV 上修 + advance 轴 fully closed**：本轮 5 轮实证（R1946 advance-in-layout / R1947 CSS2 subdir 4th hunt / R1948 border-conflict painting / R1949 @media print / R1950 linearHoriAdvance）对 v0.2.2 两处关键结论做**实证修订**：
>
> **① EV 上修（refute v0.2.2「font-wall 仅非-Ahem 产品 fixture ~17%」）—— R1947**：CSS2 layout subdir（visuren/pagination/borders）empirical 复核，`border-width-applies-to-008` 实测 **15.37% diff 全部来自顶部描述性 `<p>` "Test passes if..." 文本换行**（测试几何 inline 90px 黑方框 ZW vs chromium 像素一致）→ **font-wall 经描述指令 `<p>` 文本主导 layout dir**（几乎每个 CSS2 reftest 含此 `<p>`），**非仅 text dir / 非仅产品 fixture**。故 font-stack rebuild 的 yield 是 **universal cross-dir**（text + layout 全 dir 的描述文本 advance-wall），**EV 显著高于 v0.2.2 估计**（v0.2.2 低估为仅产品 fixture）。★ 强化 ruling #2 授权论据。
>
> **② advance 轴 fully closed（强化 v0.2.2 §C3「advance 低 EV」，post-R1263 + linearHoriAdvance 双证）—— R1946 + R1950**：v0.2.2 称 C3 advance refute 是 pre-R1263 DejaVu-only「stale-待复检」。本轮 post-R1259（Liberation Serif = chromium 同源 font）clean 复检：**R1946 advance-in-layout（scoped line-breaking-only，font_id=0 paint-coherent）= -24 回归**（css-text/line-breaking 76/127→52/127）；**R1950 FreeType linearHoriAdvance（chromium/Skia text-layout 用的 unhinted scaled advance，诊断 m=0.7778×fs 距 chromium 0.797 近 2.5× 于 gridfitted 0.75）= 0 effect**（paint Path B 用 layout-stored fragment.x，layout/paint IFC 双路径，metric 改动无效）。advance 全角度（fontdue/DejaVu/NotoSansCJK 表 R224/R1769 + FreeType gridfitted R1946 + FreeType linear R1950）**穷尽 closed**。★ **「更近 chromium 值 ≠ 更好 break」**（R1946 反证：0.75 距 0.797 近于 0.55，却 -24）→ advance 值非 break 决定因素，须 full layout/paint/wrap metric coherence（非 metric-source 单点）。
>
> **裁决（v0.2.3）**：font-stack rebuild **EV 上修**（payoff universal cross-dir，R1947）但 **path difficulty 确认**（advance metric 单点切片全 refuted，须 full coherence rebuild，R1946/R1950）。即「更值得做，但须整体做，不能切片蹭」。dormant U1b/C3 infra（R1637-R1639）仍保留待授权激活。**font-wall 根因非 advance 单点**（advance 轴全关），残余 unlock 须 full font-stack（layout/paint/wrap metric coherence + chromium-matching raster/shape）= ruling #2，仍 RFC-only 待用户授权。**border-conflict = structural painting 折叠（R1948，非 font-stack）+ @media print 机制已 done（R1949，@page paged media 另列）+ clean lever 5 轮确认穷尽** → 非 gated 自主推进只剩 structural multi-session（border-conflict painting / @page / R109）或维持 plateau。详见 master.md R1946-R1950 + 各 evidence。

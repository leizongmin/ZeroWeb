...
> **R373 方法论纠正（2026-06-20）**：welcome 经 product-smoke（阈值0，与 morning/wintertc 同口径）复测 = **16.98%**，pre-R368 = 17.01%——**R368 对 welcome 效果可忽略（-0.03pp）**。R371 报告的 14.68% 是用 cross-validate.py（chan>5 阈值）测的，比 product-smoke（阈值0）宽松，**不可与 morning/wintertc 的 product-smoke 数字直接比**；ZeroWeb 渲染本身字节相同（md5 一致），仅 diff 阈值不同。welcome 的 DC-13 真实改善来自后续 R373（morning -12.3pp），非 R368。

> **R370 复测（2026-06-20，post-R368）**：~~welcome `z_vs_chr`（ZeroWeb CPU vs chromium Oracle，DC-14 口径）= **14.68%**~~（已被 R373 纠正为 product-smoke 16.98%），较 R227 的 17.06% 改善 ~2.4pp。归因：R368（commit 8db89df，inline-block 文本内容 shrink-to-fit 经 intrinsic 测量）修正了 welcome 的两处 `display:inline-block; width:auto` 含文本元素——`.version` 徽章（`<head>` CSS line 53）与 `.shortcut kbd`（line 149）——此前被 taffy 当 Block 拉伸到容器满宽（满宽灰条），现收缩到文本宽，与 chromium 一致。welcome-zeroweb-cpu.png 已更新为当前渲染。**残余 14.68% 仍为 fontdue 字体 AA 噪声 + item-tag span→block R109 IFC（结构性，Phase A）**，非 inline-block 缺口。复测方法：临时把 welcome.html 包成 reftest（注入 match link）+ REFTEST_DUMP + cross-validate.py vs welcome-chromium.png（已清理临时文件）。

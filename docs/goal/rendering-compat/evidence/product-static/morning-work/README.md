...
> **R374 残余根因（2026-06-20，post-R373 16.41% 像素带分析）**：残余 diff 分散在多 y 带（y560-600 43.8% / y320-360 36.0% / y360-400 25.8%），非单点布局 bug。带色分析：① y560-600 chromium 有蓝色 `.item-tag` 徽章而 ZeroWeb 是背景 → **`.item-tag` 在 ZW 与 CHR 处不同 y**（上游文本高度差级联致位置偏移）；② y320-360 是代码块文本（`#3d4752`），ZW 渲染量少于 CHR（换行差异）。**根因 = 字体不匹配**（R375 DC-14 纠正 R374 的 advance-width 推断）：morning 用 system-ui 字体，chromium 用 OS 系统 UI 字体，ZeroWeb fontdue 捆绑字体≠system-ui。R375 实验：estimate_char_width 加 DejaVu 实测 advance 表 → morning DC-13 **16.41%→19.14%（+2.7pp 退步）**已回退——证明 advance-width 改善**不帮助**（crude 0.55 启发式碰巧接近 system-ui 平均字宽反而最优；per-char DejaVu 表与 system-ui 分布不同致换行偏离更多）。故 cascade 非 advance-width 计算错误，而是**字体层**（ZeroWeb 字体≠chromium 字体的 line-metric/glyph）。line-height 计算本身正确。**勿再以 morning cascade 为 advance-width 或任何 layout 单会话杠杆**——须字体加载/光栅器升级（多会话/架构）。

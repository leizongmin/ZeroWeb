...
- 2026-09-11（R4217-F 切片 3 落地）：**float walk 无 float 页快速路径**（float_positioning.rs +128/−37、engine.rs +11/−1）——§3「pass 数量 × 全树 walk」postprocess 段最大单杠杆（adjust_float_positions ~0.15-0.29ms）。**设计**：全 walk 中无 float 页面仍有行为的臂只有两个——R4140 inline 原子首子 mt 解折叠（per 容器）+ BFC auto-height 含后代重算（per flow-root，R4069 fold 语义）——**原样抽取**为 `unhoist_inline_first_atom_mt` / `recompute_bfc_auto_height` helper（全 walk 调用点语义零变化），新增 `adjust_no_float_page` 快速路径以同序（self 臂 → 递归子 → 高度重算）访问；其余全 walk 臂无 float 页均为数值 no-op（float-mt 需 float 子、is_layout_container 归零、clear 位移 max(0,bottom=0)）。engine.rs 调用点以 `page_has_float` 预扫描（styles 扫描 ~20µs）分派。**S1 converter 探针结论（本轮副产品）**：computed_style_to_taffy 实测 **~95-150ns/节点（~0.15ms）非热点**——build_subtree 其余（R109 接线/doc 查找/taffy 插入/分支密度）才是 0.87ms 主体；但本机 `Instant::now` 单次 ~0.4-0.5µs（VM 时钟源），**逐节点仪器化不可行**（读数自污染 >50%），perf/valgrind/samply 均不可用（无 perf_event_open + 无 sudo）——S1 进一步细分**工具阻塞**，待采样 profiler 可用或 CI 侧测量。**A/B（静默窗口 load 0.25 交错两轮一致）**：block **−5.6~−8.9%**（quiet 1.98ms 逼近 1.91 锚点）、flex **−9.7~−14.5%**（1.88ms 已低于回归前 1.93 锚）、incremental **−8.5~−10.4%**——三基准全部入回归前水平带，flex 已达标。**验证**：corpus 首跑即 **14681 零漂移**、make test 67 套件全绿、make reftest 687/687、product-smoke 23.49% 逐字节同值 struct PASS、legacy 0 struct FAIL、fmt、clippy -D warnings。**余**：block/incremental 仍高于 1.88/1.78 基线（残余 = S1 build_subtree 主体 + compute_final ~0.2ms + taffy pass1），S1 需工具；静默窗口定向 bench-gate 复测（本轮 load 0.4 时 3 FAIL：block 2.65/flex 2.65/incremental 2.41——bench-report 管道读数 vs 直接 criterion ~+0.4ms 系统差待查）。
- 2026-09-11（R4218-P 解阻确认轮，0 net code）：**layout 家族门禁解阻达成（RFC §4 判据）**。①本地定向 bench-gate（load 1.2-2.5）：**layout 六微基准 p50 全部入预算**（block 2.122ms/预算 2.537、flex 2.018/2.609、incremental 2.280/2.399、grid 264µs/285.6、deep 175/200.7、wide 154/164.8）——残余 2 FAIL 全为 **p95 共租突发污染**（incremental p95 6.398ms 系 20 样本中单次 6.7ms 尖峰；flex p95 2.639 同理）；上轮挂账的「bench-report 管道 vs 直接 criterion ~+0.4ms 系统差」**定案为口径差非管道缺陷**（门禁比 p95、直接交错读 mean/p50，同条件 p50 与直接测量一致 2.12 vs 2.1-2.2）。②CI 专用 runner 复核（run 34504935270，workflow_dispatch，9v74 实例）：**layout 家族 6/6 PASS**（block 4.51ms/预算 5.17 余量 13%、flex 4.34/5.03、incremental 4.22/4.89）。③该 CI 轮另有 11 指标 FAIL（dom ×3 / css-parser tokenizer / style-system ×2 / script-sandbox / wasm-sandbox / browser-shell / render-foundation / page/medium first_paint）= **跨 7 crate 均匀 +37-67% 漂移签名**——非本专项域（layout 全 PASS），候选 = 9v74 实例调度方差（基线 8/31 捕获）或并行流/共享 dom 域回归，按 run-rules §10 归因纪律留待属主流次核对，本专项不越界处理。**专项状态**：解阻达成（layout 家族本地 p50 + CI 双确认全入预算带）；完成判据（auto-tighten 收紧至回归前水平带）待 weekly auto-tighten 轮对 block/incremental 收紧（本地 quiet 1.98 vs 1.91 锚 +3.7%，接近达标；flex 已达标）。**余账**：S1 build_subtree 细分工具阻塞（VM 时钟 + 无 profiler）不变。
...
- [ ] **Position: sticky** — 滚动时正确固定在指定偏移范围内 — ✅ **R1982 静态部分已验证**：`position:sticky` 经 converter:418 映射 taffy Relative + layout-engine/src/engine/postprocess.rs:1652（resolve_relative_inset）应用 inset，scroll=0 时正确等价 relative（offset 生效，单测 `r1982_position_sticky_at_scroll0_acts_as_relative` 守 y=static+offset）。⏳ 残余 = 动态 sticking（滚动时固定在阈值范围内），属 host 层（browser scroll 输入路由）interactive 特性，非 rendering-compat reftest 范围（reftest 静态 scroll=0）。

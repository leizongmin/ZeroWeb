...
**已知限制（记录，defer）**：① `characterDataOldValue` 未支持——须 latest-wins 文本读（sel-based `__zw_get_text` 读 stale 快照），独立 slice（补 `__zw_get_text_lw` 镜像 `__zw_get_attr_lw`）；② characterData 记录 target=元素（pragmatic，spec target=文本节点——文本节点无 selector 故无法直接作 target，element target 对消费者等价可用）；③ 文本节点 nodeValue/data setter 的 characterData emission 未接（文本节点无 selector，同 ② 根因，handle-based 文本节点可后续支持）；④ innerHTML setter 的 childList emission 未接（childList 已支持 append/remove/insert，innerHTML 整体替换的 childList 记录为独立 follow-up）。

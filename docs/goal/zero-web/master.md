...
**已知限制（记录）**：① setAttribute/removeAttribute 仅改树内 attrs，不回注 host DOM（detached 容器无需持久化，DOMPurify 等读 body.innerHTML 取结果）；② 加载真实 DOMPurify 库（~150KB MIT）端到端实测为独立 follow-up slice（本文 driving test 已复刻其核心算法形态并验证）；③ NamedNodeMap setNamedItem/attributes 索引 setter 未接（DOMPurify 经 setAttribute/removeAttribute，不走 NamedNodeMap mutation）。
...
**下一步**：detached 面已闭合，pivot 回 P1a 其他缺口：① **加载真实 DOMPurify 库端到端实测**（vendor MIT purify.js 作 test fixture，跑 sanitize 验证清洗生效 + 库内 feature-detect 路径，按需补 NamedNodeMap/setNamedItem 等）；② fetch 二进制 byte-wire（host `__zw_fetch` 接 csv-decimal 字节 body，闭合 multipart/Blob 二进制保真——host-layer net 改动）；③ custom element upgrade/ctor（需 P1b RFC）；④ 事件循环 rAF 帧驱动（多会话须 RFC）。每切片 make test 零回归 + clippy/fmt 守门。

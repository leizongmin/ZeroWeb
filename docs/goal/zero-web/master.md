...
**已知限制（follow-up）**：① select.value 不缓存→同脚本内 setter 后即读见旧值（apply 后反映，同 input 的 stale 模式但 input 有缓存；select 因 value 派生自 DOM 不缓存）；② 用户交互 change（下拉 UI click option）未实现（需 UI，headless 经编程 setter + 手动 dispatch change 达成）；③ `select.options`/`selectedOptions` 集合 API 未实现（option 级访问现经 querySelector + 唯一选择器）；④ `:not()`/`:nth-last-*` 伪类（R2662 follow-up）；⑤ querySelectorAll（`__zw_query_all`）仍返 stable_selector（歧义时），仅 querySelector（单）改唯一——querySelectorAll 唯一化为独立 follow-up。

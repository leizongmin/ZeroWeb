...
**版本**: v0.5（v0.4→v0.5：d3d-r3/d3e 实用收口裁定——三域探针实证无本树化收益面[detached/main identity 全绿 + 19µs/查询]，d3d 系全部结项，M1 JS 侧实用收口——2026-08-29 R359）
...
| **d3d-r3 element/fragment 本树化重启** | `_zwMQueryAll`/fragment QSA 纯 tag+compound 以调用元素为根遍历（复用 `_queryTreeByCompound` + 守卫），产物经桥 | d3d-r1 + d3d-r2 | 全量双路径 net≥0 + Element-matches/`#id-li-duplicate` 文件级 | ⛔ **结项（R359 裁定：三域探针实证 compound identity 已由桥/归并覆盖[detached/main 全绿]、查询 19µs/次非瓶颈——本树化无收益面；探针暴露的唯一实缺口[iframe 工厂元素 own QSA compound]已补面修复；d3e 随之结项备档）** |
| **d3e 组合器本树化** | 组合器形态走 part05 `_matchComplexAgainst`（nodeInfo 从各自树构）；attr 其它运算符同步 | d3d-r3 | 同上 + traversal 文件级 | ⛔ **随 d3d-r3 结项备档**（iframe 域组合器维持 headless 近似、其余域 host JSON 权威可达[R359]） |
...
| v0.5 | 2026-08-29 | **d3d-r3/d3e 实用收口裁定（§3 表状态回填）**：R359 三域探针（detached/iframe/main × compound/identity/性能）实证 element 上下文 compound 消费面已由 R167/R308 桥 + R322 归并覆盖（identity 全绿）、查询单价 19µs/次非瓶颈（R353 同结论）——本树化无正确性/性能收益面；唯一实缺口 = iframe 工厂元素 own QSA 不支持 compound（R181 简单匹配器 + 工厂域不经 JSON 往返），已以 `_zwParseCompoundSel` 本树 walk 补面修复。**L2-d3 全切片收口，M1 JS 侧实用收口达成**（js-dom R359） |

...
- **S5 剩余后续**：① 抽 mod.rs 与 html_element.rs 共享 Element 接口安装函数（消除 DRY 重复，重构 mod.rs，中风险）；② `customElements.upgrade(root)` + whenDefined Promise 时序对齐（parser 建 my-el 在 define 后升级）；③ adoptedCallback（跨 document adopt，罕见）；④ lit/stencil 真实 CE 库集成测试。

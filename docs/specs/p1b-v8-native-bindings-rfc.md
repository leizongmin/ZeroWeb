...
**版本**：v0.3（2026-08-31 收官状态对齐——TBD-5 双引擎关闭 + S6/S7 状态终局 + 双 feature default-on）
**日期**：2026-08-08
**作者**：ZeroWeb rally（自主推进）
**状态**：v0.1 已批准 S0 PoC（2026-08-09 用户决策）；**S0 PoC 验证完成（R3095, 2026-08-09）—— TBD-1/TBD-2 阻塞性验证通过**；**S1 dom_bindings 生产化完成（R3096, 2026-08-09）—— 首组原生 getter（nodeType/tagName）+ NodeId↔对象映射 + bench（native ~15.6x）+ kill-switch**；**S2 生产接线完成（R3097, 2026-08-09）—— 原生绑定接通 webview 真实页面沙箱（`Sandbox::install_native_bindings` escape-hatch + `WebViewConfig.native_dom`，默认关 → 零回归，webview 集成测试通过）**；**S3 查询原生完成（R3099/R3100）—— selector→NodeId 全选择器引擎 + element 子树作用域**；**S4 EventTarget 原生 + 事件系统原生化完成（R3109–R3126）—— native addEventListener/dispatchEvent、host→page 原生派发、事件对象丰富化（target/currentTarget/bubbles）、dispatchEvent target+bubble 两阶段冒泡、stopPropagation/stopImmediatePropagation**；**L1 live Document 共享完成（R3106–R3108，`Rc<RefCell<Document>>`，原生写触发重渲染）**；节点导航/文本/属性族原生落地（R3103/R3104/R3110–R3115：textContent/childNodes/nodeValue/节点导航/cloneNode/contains/replaceChild/NamedNodeMap/innerHTML-outerHTML getter）；**RFC §3.2 dom_bindings 五子模块化闭合（R3116–R3120，mod.rs 1446→379）**；**S5 customElements/Web Components 完成（R3262–R3269）—— TBD-1 class 继承专项验证通过（R3262 PoC）+ S5a native HTMLElement 基类/class-extends（R3264）、S5b customElements.upgrade/createElement('my-el') 原生实例（R3265）、S5c connected/disconnected 生命周期原生路径（R3266）、S5d attributeChangedCallback + HTMLElement Element 接口（R3267/R3268）、S5e 子树 upgrade（R3269）+ upgrade 时 observed attrs 回调（R3274）**；S6–S7 状态终局（js-dom goal R361/R384 定案，2026-08-31）——S6 记 superseded-by-default-on：shim 高层 API 改调 native 的前置是「页面 document 由 native 提供」，default-on（R384/R382 双 feature）已使其达成——页面 globalThis 现由 native bindings 供给，polyfill 桥不再是生产路径，S6 的「在将被删除的路径上建一次性桥」失去对象；S7 的 polyfill 桥死代码删除已随 kill-switch 移除（R384b：`install_dom_bindings_if_enabled`/`ZW_NATIVE_DOM` env/`native_dom=false` 回退路径全删）落地，shim 残余仍被 WPT 资产与 M4 基线消费（其萎缩随上游用例面收敛持续，非一次性删除动作）**
...
- ✅ **S5 后续 ① HTMLElement Element/Node 接口完整化**（R3268 已 land）：`html_element.rs` 新 `install_element_interface(scope, tmpl)`，镜像 mod.rs generic Element 模板注册集，复用 element.rs/node.rs/event_target.rs/dom_token_list.rs/css_style_declaration.rs/dataset.rs/namednodemap.rs 的 invoke，挂全套接口（只读属性 + 反射属性含 47 ARIA + 复杂对象 getter + 内容 + 导航 + Attribute 方法 + 查询 + 树 mutation + 现代 mutation + Adjacent + 事件 + 焦点）到 HTMLElement instance_template。3 helper（set_method/set_getter/set_getset，正确 trait bound `AccessorNameGetterCallback`/`AccessorNameSetterCallback`）。custom 实例 Element API 与 generic Element 模板等价。+3 测。零回归（engine v8 lib 1994 + webview 572）。
- ✅ **S5 剩余后续 ② customElements.upgrade(root)**（R3269 已 land）：part03.js `upgrade(root)` 从 stub 落地——`_ceUpgradeNode` DFS 遍历子树，`Object.setPrototypeOf(el, ctor.prototype)` 升级既有元素（保留 NodeId，不重建）+ 已连 document 触发 connectedCallback。PoC 验证 setPrototypeOf 对 native 元素可行。whenDefined 已 R2813 完整。+2 测。零回归（engine v8 lib 1996 + webview 572）。CE 注册 API 五件套（define/get/getName/whenDefined/upgrade）+ lifecycle 四件套全闭合。
- **S5 剩余后续**：① 抽 mod.rs 与 html_element.rs 共享 Element 接口安装函数（消除 DRY，重构 mod.rs，中风险）；② adoptedCallback（跨 document adopt，罕见）；③ upgrade 时 observedAttributes 的 attributeChangedCallback；④ lit/stencil 真实 CE 库集成测试。
...
| TBD-5 | 是否保留 QuickJS 路径的原生绑定（或仅 V8） | ✅ **已解（js-dom goal v1.1 决策，2026-08-13 用户确认；R57–R78 落地）** | **更新为「双引擎 V8+QuickJS」**——v1.0「QuickJS 是扩展沙箱非页面引擎」的前提经代码核实推翻：QuickJS 是 feature-gated 的**页面引擎之一**（`webview.rs` 与 V8 对称构造页面沙箱；CI 强制 `--features quickjs` 矩阵），若不覆盖则其页面路径永远停在 polyfill 字符串桥。**QuickJS 原生绑定已全量落地**（`crates/engine/src/quickjs_dom_bindings.rs`，镜像 V8 S0–S5：S0q 骨架+weak 生命周期 / S1q 属性+复合对象 / S2q 写入+mutation / S3q 查询 / S4q EventTarget+三阶段派发 / S5q customElements 五件套+lifecycle 四件套；WPT 双路径对等差 0.02pp）；双 feature `native_dom` 已 default-on、kill-switch 已删（js-dom M5/M7，R384/R384b/R382） | ✅ 关闭（双引擎对等达成） |
...
| v0.3 | 2026-08-31 | 状态终局对齐（js-dom goal 收官）：TBD-5 由「仅 V8」更新为「双引擎 V8+QuickJS」并关闭（QuickJS 页面引擎地位核实 + quickjs_dom_bindings 全量落地 R57–R78）；S6 记 superseded-by-default-on（R361）、S7 的 kill-switch 死代码删除随 R384b 落地；双 feature `native_dom` default-on（M5 R384 + M7 R382）、kill-switch 移除（R384b） |

...
**当前架构**（截至 2026-09-09）: Cargo workspace 已扩展到 32 个 member（22 个库 crate、7 个应用入口、2 个测试工具、1 个开发工具）。核心内核（`dom` / `css-parser` / `style-system` / `layout-engine` / `engine` / `render-foundation` / `paint-convert` / `host-runtime` / `net` / `security` / `storage` / `protocol` / `canvas` / `media` / `wasm-sandbox` / `script-sandbox` / `page-runtime` / `webview`）均有实质实现；产品层（`apps/browser` 桌面入口 + headless / remote debugging、`apps/renderer` 多进程渲染、`apps/image-decoder` 图像解码进程、`apps/compositor` 合成器进程、`apps/webdriver` WebDriver 服务（34 endpoint，含 screenshot）、`apps/android-browser` Android M0 bootstrap）已打通。已完成并整树归档的专项 goal：js-dom（P1b V8 原生 DOM 绑定，2026-08-31）、媒体三 goal（webm/VP9/AV1/H.264 解码与 renderer 播放泵，2026-09-05，media-elements 终态 WPT 640P/0F = 98.01%）、storage-cache-api 与 service-workers（Cache API 真实现 + SW 真实生命周期，2026-09-06）。keyboard/editing 三 goal 与 webdriver goal 已于 2026-09-08 收口整树归档，webdriver-screenshot goal（公共转换层 `zero-paint-convert` + GET /session/{id}/screenshot）于 2026-09-09 收口整树归档；当前主线为渲染兼容性收口（corpus 14664/16813 = 87.2%，R4193 轮口径）与 M12/M14 拆分余下的子 goal（storage-opfs——M1 已落地 / android-browser，2026-09-07 新立；web-components 已于 2026-09-11 收口归档、page-wasm 已于 2026-09-12 收口归档、event-loop-spec 已于 2026-09-12 收口归档——IO/RO 基线 + MO host 触发 default-on + per-task checkpoint default-on）；渲染兼容性（WPT/CSSWG reftest 对齐 Chromium Oracle）自 2026-08-09 字体栈重建 RFC 获批后恢复主动实施。详见 [docs/architecture.md](../architecture.md) 与 [docs/goal/zero-web/master.md](../goal/zero-web/master.md)。

**代码规模**（2026-09-09 静态统计）: 约 17,750 个 `#[test]`（不含 taffy-local vendored 补丁与宏生成/doctest；v8 feature 全量全绿），wpt-runner 导入上游 WPT reftest 约 9,967 个 + WPT testharness 常驻断言集（`tests/wpt-runner`）；criterion 基准 + `make bench-gate` 性能门禁（perf-gate 基线/趋势回写 main）；`cargo clippy --workspace --all-targets -- -D warnings` 全绿（CI 强制）。

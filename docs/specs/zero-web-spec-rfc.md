...
**当前架构**（截至 2026-09-03）: Cargo workspace 已扩展到 31 个 member（21 个库 crate、7 个应用入口、2 个测试工具、1 个开发工具）。核心内核（`dom` / `css-parser` / `style-system` / `layout-engine` / `engine` / `render-foundation` / `host-runtime` / `net` / `security` / `storage` / `protocol` / `canvas` / `media` / `wasm-sandbox` / `script-sandbox` / `page-runtime` / `webview`）均有实质实现；产品层（`apps/browser` 桌面入口 + headless / remote debugging、`apps/renderer` 多进程渲染、`apps/image-decoder` 图像解码进程、`apps/compositor` 合成器进程、`apps/webdriver` WebDriver 服务、`apps/android-browser` Android M0 bootstrap）已打通。当前主线为 P1b V8 原生 DOM 绑定（js-dom goal 已收官归档，R383/R384 双引擎 `native_dom` default-on）与媒体解码管线（`crates/media`，media-playback / media-audio / media-elements 三 goal 2026-09-05 完成收口整树归档——webm/VP9/AV1/H.264 解码与 renderer 播放泵已落地，media-elements 终态 WPT 640P/0F = 98.01%）；渲染兼容性（WPT/CSSWG reftest 对齐 Chromium Oracle）自 2026-08-09 字体栈重建 RFC 获批后恢复主动实施。详见 [docs/architecture.md](../architecture.md) 与 [docs/goal/zero-web/master.md](../goal/zero-web/master.md)。

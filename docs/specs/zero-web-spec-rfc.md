...
**当前架构**（截至 2026-09-03）: Cargo workspace 已扩展到 31 个 member（21 个库 crate、7 个应用入口、2 个测试工具、1 个开发工具）。核心内核（`dom` / `css-parser` / `style-system` / `layout-engine` / `engine` / `render-foundation` / `host-runtime` / `net` / `security` / `storage` / `protocol` / `canvas` / `media` / `wasm-sandbox` / `script-sandbox` / `page-runtime` / `webview`）均有实质实现；产品层（`apps/browser` 桌面入口 + headless / remote debugging、`apps/renderer` 多进程渲染、`apps/image-decoder` 图像解码进程、`apps/compositor` 合成器进程、`apps/webdriver` WebDriver 服务、`apps/android-browser` Android M0 bootstrap）已打通。已完成并整树归档的专项 goal：js-dom（P1b V8 原生 DOM 绑定，2026-08-31）、媒体三 goal（webm/VP9/AV1/H.264 解码与 renderer 播放泵，2026-09-05，media-elements 终态 WPT 640P/0F = 98.01%）、storage-cache-api 与 service-workers（Cache API 真实现 + SW 真实生命周期，2026-09-06）。keyboard/editing 三 goal 与 webdriver goal 已于 2026-09-08 收口整树归档；当前主线为渲染兼容性收口（corpus 14626/16814 = 87.0%）与 M12/M14 拆分余下的 5 个子 goal（storage-opfs / page-wasm / android-browser / webdriver / web-components / event-loop-spec，2026-09-07 新立）；渲染兼容性（WPT/CSSWG reftest 对齐 Chromium Oracle）自 2026-08-09 字体栈重建 RFC 获批后恢复主动实施。详见 [docs/architecture.md](../architecture.md) 与 [docs/goal/zero-web/master.md](../goal/zero-web/master.md)。

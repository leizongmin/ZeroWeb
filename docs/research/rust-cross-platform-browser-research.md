...
> - **⚠️ 假设**: 本章未依赖你尚未存在的 ZeroBrowser 代码结构，因为当前仓库几乎为空。
...
> - **⚠️ 假设**: 本章没有直接测量 ZeroBrowser 的资源预算，因此“与目标不匹配”是从系统复杂度而非团队规模估算得出。
...
| 层 | 建议角色 | 备注 |
|---|---|---|
| `webview-api` | 稳定嵌入接口 | 对外暴露 `WebViewHandle`、导航、输入、脚本桥、纹理/表面输出 |
| `engine-core` | 默认内核适配器 | 首个现实主线，基于 permissive 依赖自建页面内核 |

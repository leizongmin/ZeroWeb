...
| 渲染兼容性（恢复主动实施） | 以 WPT/CSSWG reftest 对齐 Chromium 为验收标准，Chromium Oracle 像素一致率（`make reftest-oracle`）为诚实度量（同源 reftest 存在假通过，仅作自一致性参考）。上游 WPT reftest corpus **14556/16814 = 86.6%**（2026-09-08，R4121 轮口径）；transform-box 属性三 crate 贯通 + svg 参考框换算（R4098–R4110）、Ahem 真实轮廓光栅化（R4115）、省略号单 U+2026（R4116）、background-position 负偏移（R4118）、@supports 裸 general-enclosed（R4120）等系列持续收口（R3936–R4121）。详见 [路线图](ROADMAP.md) 与 [docs/goal/rendering-compat.md](docs/goal/rendering-compat.md) |
...
| HTML 编辑与键盘 | contenteditable/execCommand/Selection 编辑管线（WPT selection 套件 2994P，execCommand bold/italic/underline/delete 实应用 + queryCommandState 反射 + insertHTML/insertParagraph，CE 键入/删除/Enter 换行/innerHTML 注册落 DOM，E2 切片 13 收口）；键盘——Space/Enter/Esc/方向键/type-ahead 激活与导航、隐式表单提交、Ctrl+Home/End 与滚动键 JS 滚动默认动作（keyboard/editing 三 goal 活跃，2026-09-08） |

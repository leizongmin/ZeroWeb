...
| 渲染兼容性（恢复主动实施） | 以 WPT/CSSWG reftest 对齐 Chromium 为验收标准，Chromium Oracle 像素一致率（`make reftest-oracle`）为诚实度量（同源 reftest 存在假通过，仅作自一致性参考）。上游 WPT reftest corpus **14765/16594 = 89.0%**（2026-09-14，R4322 轮口径）；**inline 子树 flatten walk 系列**（混合 white-space 独立成 run、SVG/竖排/ruby/bidi 特例门逐域收敛后 default-on 翻转，R4299–R4313）、**counter-style 族**（ethiopic-numeric 算法、CJK 计数器万亿组系合成 range 扩到 10^16、Hebrew marker bidi 视觉序反转、隐式值域/additive 算法，R4314–R4321）、**S11 style-key 缓存不可键父链继承态碰撞修复**（direction 继承根基修，R4322）、filter region 外扩 + backdrop 集群 box_blur 滑动窗口双计修复 + inline svg 序列化 xmlns:xlink（R4291–R4293）、auto 表格列宽压缩 + 压缩后 cell 重排（R4298）、margin-trim 通用化族（R4250–R4255）、`::scroll-marker` / `::scroll-button` 函数伪元素（R4256–R4264）、CSS filter url(#id) 引用 SVG filter 输出链与 alpha 保真三切片（R4273–R4284）等系列持续收口（R3936–R4322）。布局依赖值两遍样式解析深域已立案待用户点名（R4189，三族同构：cq 单位 / stretch 父 % 高 / line-clamp 约束链）。详见 [路线图](ROADMAP.md) 与 [docs/goal/rendering-compat.md](docs/goal/rendering-compat.md) |

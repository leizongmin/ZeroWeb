...
| 渲染兼容性（当前主线） | 以 WPT/CSSWG reftest 对齐 Chromium 为验证标准；已有 WPT runner、reftest、真实静态页截图对比和真实站点 smoke，CSS 全面渲染集成（100+ 属性）、增量布局与渲染管线优化（填充批处理 + 视口剔除 + draw call 统计）已落地，CSS2 / Flexbox / Grid / 布局等渲染缺口仍在持续修复中，距生产级兼容仍有明显距离 |

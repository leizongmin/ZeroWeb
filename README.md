...
| 渲染兼容性（恢复主动实施） | 以 WPT/CSSWG reftest 对齐 Chromium 为验收标准，Chromium Oracle 像素一致率（`make reftest-oracle`）为诚实度量（同源 reftest 存在假通过，仅作自一致性参考）。上游 WPT reftest corpus **14626/16814 = 87.0%**（2026-09-08，R4157 轮口径）；transform-box 属性三 crate 贯通 + svg 参考框换算（R4098–R4110）、contain:layout/paint 捕获 fixed 后代为其 CB（R4122，csswg #10544）、box-shadow punch-out 与两相位绘制（R4139/R4143）、aspect-ratio transferred % 基准（R4147）、宽轴关键字固有贡献（R4150–R4152）、counter-style extends 回落内置表（R4154）、abspos stretch 关键字（R4155）等系列持续收口（R3936–R4157）。详见 [路线图](ROADMAP.md) 与 [docs/goal/rendering-compat.md](docs/goal/rendering-compat.md) |
...
| 浏览器自动化 | `zero-webdriver` W3C 协议 9 → 33 endpoint（session/导航/超时/元素族/execute sync+async/source/window 族），CI 双 feature 矩阵接线 + 10 个 HTTP 全链路集成测试；webdriver goal 已收口归档（2026-09-08，DC-1~4 ✅），screenshot 等余项挂账。详见 [docs/goal/archive/webdriver/](docs/goal/archive/webdriver/master.md) |

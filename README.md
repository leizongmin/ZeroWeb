...
| 浏览器应用 | `zero-browser` 固定经 `zero-renderer`、`zero-image-decoder` 与 `zero-compositor` 子进程工作；browser 仅保留窗口、浏览器 chrome、网络代理、IPC 帧导入与呈现，默认发布版不链接 WebView、脚本 sandbox 或任何 JS 引擎，页面脚本由 renderer 独占执行。headless 调试同样通过 renderer IPC 完成导航、脚本和截图。`ZeroWebView` 作为嵌入 API 保持进程内实现；Android 端 M0 bootstrap 已落地（`apps/android-browser`，Kotlin chrome + Rust JNI 桥，decoder/compositor 复用共享 role 循环）；整体仍处于实验阶段 |
...
| 渲染兼容性（恢复主动实施） | 以 WPT/CSSWG reftest 对齐 Chromium 为验收标准，Chromium Oracle 像素一致率（`make reftest-oracle`）为诚实度量（同源 reftest 存在假通过，仅作自一致性参考）。上游 WPT reftest corpus **14664/16813 = 87.2%**（2026-09-09，R4193 轮口径）；宽轴关键字固有贡献（R4149–R4152）、object/embed/applet 解码固有尺寸桥接（R4159）、ratio-only SVG × aspect-ratio 优先级族（R4160–R4163/R4165）、head 内注入文本节点渲染链（R4169，净+13）、cq 单位样式缓存与渐变解析（R4178/R4179）、float 提升钳制基准（R4176）、line-clamp:auto 家族（R4181–R4181c）、line-clamp 双写级联 no-op（R4184）、单行 flex line cross 钳制（R4185）、margin-trim 自折叠臂（R4191）、contain:inline-size 内联尺寸抑制（R4193）等系列持续收口（R3936–R4193）。布局依赖值两遍样式解析深域已立案待用户点名（R4189，三族同构：cq 单位 / stretch 父 % 高 / line-clamp 约束链）。详见 [路线图](ROADMAP.md) 与 [docs/goal/rendering-compat.md](docs/goal/rendering-compat.md) |
...
| 存储与 Service Worker | 存储 `caches`/`Cache` 全 API 接 zero-storage 真实实现 + per-origin 持久化（WPT cache-storage 39 case / 449 subtest 全绿）；Service Worker 真实生命周期 + fetch 拦截 respondWith + Cache API 集成 + registration-local CacheStorage 持久化（WPT 三 runner 121 case / 652 subtest 全绿）。storage-cache-api 与 service-workers 两 goal 已完成收口归档（2026-09-06）；**storage-opfs goal 已启动**（2026-09-07 立案）：zero-storage `opfs` 模块 Rust 底座落地（目录树/句柄/流全语义/per-origin 持久化）+ fs/ window 子集 13 用例 WPT 基线导入（内存 shim 48P/85F 为真实化对照起点，M2 修复队列已聚类）。详见 [docs/goal/storage-opfs/master.md](docs/goal/storage-opfs/master.md) |
...
| 浏览器自动化 | `zero-webdriver` W3C 协议 9 → 34 endpoint（session/导航/超时/元素族/execute sync+async/source/window 族/screenshot），CI 双 feature 矩阵接线 + 10 个 HTTP 全链路集成测试；webdriver 与 webdriver-screenshot 两 goal 均已收口归档（2026-09-08 / 2026-09-09，DC-1~4 ✅）。详见 [docs/goal/archive/webdriver/](docs/goal/archive/webdriver/master.md) 与 [docs/goal/archive/webdriver-screenshot/](docs/goal/archive/webdriver-screenshot/master.md) |

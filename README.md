...
| 媒体（解码管线起步） | `crates/media`（zero-media）已落地：webm/Matroska demux、VP9 纯 Rust 解码、AV1（`decode-av1` feature）、音频解码（mp3/ogg-vorbis/opus/webm 音轨）、`VideoPlayer` 播放驱动、混音总线与 Web Audio 振荡器最小面；media-elements 语义面 WPT 532P/0F（532/556 = 95.7%）。完整 `<video>`/`<audio>` 播放体验仍在推进，详见 [docs/goal/media-playback/master.md](docs/goal/media-playback/master.md) |

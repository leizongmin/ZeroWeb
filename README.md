...
| 浏览器应用 | `zero-browser` 固定经 `zero-renderer`、`zero-image-decoder` 与 `zero-compositor` 子进程工作；已打通桌面入口、headless 调试与跨平台打包（Linux / macOS / Windows）+ CI 发布工作流。`ZeroWebView` 作为嵌入 API 保持进程内实现；整体仍处于实验阶段 |

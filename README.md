...
| 渲染兼容性（恢复主动实施） | 以 WPT/CSSWG reftest 对齐 Chromium 为验收标准，Chromium Oracle 像素一致率（`make reftest-oracle`）为诚实度量（同源 reftest 存在假通过，仅作自一致性参考）。上游 WPT reftest corpus **14842±flake/16594 = 89.4%**（2026-09-15，R4355 轮口径）；**display:run-in 深挖大族**（css-display 簇 56 fail 全量归因后逐案收敛——breaking-001 探针、合并自盒重影抑制、分裂边框载荷、幻影高度钳零、basic-004/009 翻绿、匿名块补偿链欠计门、between 族完美收敛，R4324–R4344）、**首个全量 chromium-oracle DC-14 strict 基线**（真通过 538/4765 = 11.3%、credible 48.0%、域热力图与 css-ruby oracle 真值补齐，R4348）、**background 简写多层与滚动状态管线**（`<bg-layer>#` 多层、多图层 Vec 化、canvas 传播逐层 origin、attachment-local + 脚本化 scrollTop/scrollLeft、`<box>` 解析恢复，R4349–R4354）、**ruby-overhang 模型专项**（slice A 混合 inline 固有宽坍塌修复 + slice B 悬挂模型修正 opt-in 落树，R4355–R4357）、cjk_contiguous default-on 翻绿 line-break 族 18 案（R4345）、counter-styles inside marker 尾随空格 suffix 语义（R4346，净+19）等系列持续收口（R3936–R4357）。布局依赖值两遍样式解析深域已立案待用户点名（R4189）。详见 [路线图](ROADMAP.md) 与 [docs/goal/rendering-compat.md](docs/goal/rendering-compat.md) |

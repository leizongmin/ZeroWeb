...
| 浏览器应用 | `zero-browser` 固定经 `zero-renderer`、`zero-image-decoder` 与 `zero-compositor` 子进程工作；browser 仅保留窗口、浏览器 chrome、网络代理、IPC 帧导入与呈现，默认发布版不链接 WebView、脚本 sandbox 或任何 JS 引擎，页面脚本由 renderer 独占执行。headless 调试同样通过 renderer IPC 完成导航、脚本和截图，并已提供 CDP 协议雏形（cdp-protocol goal 推进中，Playwright `connectOverCDP` 驱动绿步 25）。`ZeroWebView` 作为嵌入 API 保持进程内实现；Android 端可用化目标已完成（`apps/android-browser`，android-browser goal 2026-09-13 DONE——多标签槽位、视口真值、键盘/IME 输入、下载/通知/SAF 导出、双语与无障碍、CI 模拟器冒烟 + chaos 断连恢复，DC-1~4 ✅）；整体仍处于实验阶段 |
...
| 渲染兼容性（恢复主动实施） | 以 WPT/CSSWG reftest 对齐 Chromium 为验收标准，Chromium Oracle 像素一致率（`make reftest-oracle`）为诚实度量（同源 reftest 存在假通过，仅作自一致性参考）。上游 WPT reftest corpus **14742/16594 = 88.8%**（2026-09-13，R4290 轮口径）；margin-trim 通用化族（multicol extends / inline 链 / flex multiline / adjacency 折叠轨，R4250–R4255）、corner-shape 特性切片（R4248）、`::scroll-marker` / `::scroll-button` 函数伪元素解析与盒合成（R4256–R4264）、scrollbar 预留语义（R4265）、`revert-layer` 回退语义与 @layer 层序（R4245/R4246）、**CSS filter url(#id) 引用 SVG filter 输出链**（常量链 + 非常量链 SourceGraphic 隔离 + feImage 内联，R4273–R4280）、**CPU 光栅 alpha 保真三切片**（isolate 离屏 dual-matte 直 alpha，opacity() 真透明，R4282–R4284）、SVG presentation CSS 注入（R4280）、svg 原子 inline 尺寸 shrink 误伤 + viewBox 自然比（R4290）等系列持续收口（R3936–R4290）。布局依赖值两遍样式解析深域已立案待用户点名（R4189，三族同构：cq 单位 / stretch 父 % 高 / line-clamp 约束链）。详见 [路线图](ROADMAP.md) 与 [docs/goal/rendering-compat.md](docs/goal/rendering-compat.md) |

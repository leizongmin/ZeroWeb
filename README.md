...
| 安全与可访问性 | CSP 完整实现、HSTS 预加载、混合内容阻止 / 升级、权限模型与站点隔离已落地并统一接入 `SecurityContext`（R3342/R3343 修复 CSP source-expr 前缀匹配与 mixed-content 大小写绕过）；security-hardening goal 推进中（M1 三 corpus WPT 导入基线 + M2-s1–s5 CSP 运行时强制接线——meta 装配、script/style/img 检查点、connect-src 阻止族、`SecurityPolicyViolationEvent` 事件上报，WPT csp corpus subtests 16.3%→28.9%）；可访问性基础（`FocusManager` Tab 导航 + ARIA）已起步 |

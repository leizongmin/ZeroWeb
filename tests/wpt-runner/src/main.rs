...
fn cmd_testharness_canvas(options: &CliOptions, filter: Option<&str>) {
    let wpt_root = options
        .wpt_data
        .as_deref()
        .map(std::path::PathBuf::from)
        .unwrap_or_else(|| std::path::PathBuf::from("tests/wpt-runner/wpt-data"));
    let cases = testharness::run_canvas_cases(&wpt_root, filter);
    let failed = cases.iter().any(|(_, results)| {
        results
            .iter()
            .any(|result| result.status != testharness::HarnessStatus::Pass)
...
/// `testharness-dom` 子命令 — 跑导入的上游 `dom/` testharness 用例（js-dom goal M4 / DC-3）。
///
...
/// `testharness-media` 子命令 — 跑导入的上游 media-elements testharness 用例
/// （media-elements goal M1 / DC-1）。
///
/// 用例由 `fetch-media-subset.sh` 按需拉到 `wpt-data/html/semantics/embedded-content/`
/// （gitignored）。退出码：有用例非 Pass 或用例集为空 → 1（与 testharness-dom 一致）。
/// 基线首跑即便大量 Fail 也只用于记录通过率（agent 经 `--format json` 捕获后写
/// evidence/），不作为 land 门禁。
fn cmd_testharness_media(options: &CliOptions, filter: Option<&str>) {
    let wpt_root = options
        .wpt_data
        .as_deref()
        .map(std::path::PathBuf::from)
...
/// `testharness-keyboard` 子命令 — 跑导入的上游键盘 testharness 用例
/// （keyboard-default-actions goal M1 / DC-1——键盘默认动作基线）。
///
/// 用例由 `fetch-keyboard-subset.sh` 按需拉到 `wpt-data/uievents/keyboard/` 与
/// `wpt-data/html/semantics/forms/form-submission-0/`（gitignored）。退出码：有用例
/// 非 Pass 或用例集为空 → 1（与 testharness-dom 一致）。基线首跑即便大量 Fail
/// 也只用于记录通过率（agent 经 `--format json` 捕获后写 evidence/），不作为
/// land 门禁。filter 按路径子串透传：make testharness-keyboard FILTER=implicit。
fn cmd_testharness_keyboard(options: &CliOptions, filter: Option<&str>) {
    let wpt_root = options
        .wpt_data
        .as_deref()
        .map(std::path::PathBuf::from)
...
fn cmd_testharness_indexeddb(options: &CliOptions, filter: Option<&str>) {
    let wpt_root = options
        .wpt_data
        .as_deref()
        .map(std::path::PathBuf::from)

...
/// R4147 勘察：intrinsic-percent-replaced-015 布局树——aspect-ratio 高度传递链。
#[test]
#[ignore]
fn debug_r4147_ipr015_tree() {
    use zero_css_parser::Parser as CssParser;
    use zero_dom::parse_html;
    use zero_layout_engine::LayoutEngine;
    use zero_style_system::StyleSystem;
    ...
    <div class="outer"><div class="inner"><div style="height:100%">
    <img src="aspect-ratio/support/200x200-green.png"></div></div></div></body></html>"#;
    let doc = parse_html(html);
    let stylesheet = CssParser::parse_stylesheet(
        ".outer { width: 200px; aspect-ratio: 2/1; } .inner { width: min-content; height: 100%; background: red; } img { height: 100%; }",
    );
    let mut sys = StyleSystem::new();
    let styles = sys.compute_styles(&doc, &[stylesheet]);
    let mut engine = LayoutEngine::new(800.0, 600.0);

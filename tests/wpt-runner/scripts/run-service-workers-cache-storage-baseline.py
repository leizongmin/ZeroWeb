...
def render_markdown(summary: dict) -> str:
    ...
    return "\n".join(
        [
            ...
            "`serviceworker/cache-storage.https.html` wrapper. It runs the upstream "
            "`script-tests/cache-storage.js` in a real Service Worker global and "
            "validates `caches.open()`, `CacheStorage.has/delete/keys()`, opened "
            "`Cache` identity, delete dooming semantics, empty cache names, "
            "required-argument TypeError behavior, worker `Cache.add()`, and "
            "DOMString cache-name preservation for unpaired surrogate code units.",
        ]
    ) + "\n"

...
pub(crate) fn configure_inline_fonts(
    ...
    if let Some(provider) = inline_fonts.metric_provider {
        context = context.with_font_metric_provider(provider.0.clone());
    }
    let use_advance = allow_vertical_advance || !context.vertical;
    if use_advance && let Some(source) = inline_fonts.advance_source {
        context = context.with_advance_source(source.0.clone());
    }
    if let Some(resolver) = inline_fonts.resolver {
...
pub(crate) fn compute_final_inline_layouts(
    ...
    let mut inline_ctx = InlineFormattingContext::new(container_width)
        ...
        // R4434：vertical 的 block_extent 恒取 content_width（物理块轴跨度，R1122
        // 语义）——旧非 caption 分支传 container_width（= content_height，行内深度），
        // RTL 列自 x=行内深度向左堆叠、越界列负 x 裁剪（vrl-015 cell 25 列仅 ~7 列
        // 可见，字形饿死左带实证）。caption 臂语义并入恒等分支。
...
pub(crate) fn measure_text_content(
    ...
    // R4034b（CSS Containment 1 §3）：contain:size → 元素按「无内容」sized——本回调是
    // 内容测量，containment 下内容（含 R4034 后被正确识别的 nbsp 文本）不贡献尺寸，
    // 返回 CIS 替代值（无 CIS = 0）。此前 nbsp 被 trim() 误判无内容而「巧合正确」
    ...
    {
        let cis = |v: &Option<zero_css_parser::values::LengthValue>| -> f32 {
            v.as_ref()
                .and_then(|l| crate::intrinsic_sizing::resolve_intrinsic_real_length(l, s))
                .unwrap_or(0.0)
        };
        return Size {
            width: cis(&s.contain_intrinsic_width),
            height: cis(&s.contain_intrinsic_height),
        };
    }
        ...
        && let NodeKind::Text(text_data) = &node.kind
    {
        ...
        let measure_shaped = |value: &str| {
            let source = advance_source.expect("shaped measurement requires an advance source");
        ...
        let measured_width: f32 = if matches!(available_space.width, AvailableSpace::MinContent) {
            text.split(char::is_whitespace)
                ...
                .map(|word| {
                    if shaped_measure_eligible && letter_spacing == 0.0 && advance_source.is_some() {
                        measure_shaped(word)
                    } else {
                        word.chars()
                ...
                .fold(0.0f32, f32::max)
        } else if shaped_measure_eligible && letter_spacing == 0.0 && advance_source.is_some() {
            measure_shaped(&text)
        } else {
            text.chars()
                .map(|ch| crate::inline::estimate_char_width(ch, font_size, is_ahem) + letter_spacing)
...
pub(crate) fn remeasure_inline_only_containers(
        ...
        })
    {
        ...
        //（converter 已写 CIS-or-0）——本 IFC 重测不得把内容行高回填

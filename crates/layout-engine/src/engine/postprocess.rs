...
fn postprocess_raw_real_length_value(value: &LengthValue) -> Option<f32> {
    match value {
        ...
        | LengthValue::Cqb(v)
        | LengthValue::Cqmin(v)
        | LengthValue::Cqmax(v) => v.is_finite().then_some((*v).max(0.0) as f32),
        LengthValue::Calc(_)
...
fn shift_siblings_after_ifc_grow_inner(
    ...
    let is_shiftable_next = |c: &LayoutBox| {
        ...
        // R1505：inline-block（非 floated）的 is_block_level=false（engine.rs:1170 仅 floated
...
pub(super) fn restretch_abspos_flex_replaced_items(
    ...
    let abspos_stretch = box_node.is_absolute
        && resolve_postprocess_real_length(&style.top, style).is_some()
...
mod runtime_flag_tests {
    ...
    #[test]
    fn r3638_abspos_flex_replaced_stretch_accepts_residual_real_insets() {
        ...
        flex_style.top = LengthValue::Em(1.0);
        flex_style.bottom = LengthValue::Ch(2.0);
        ...
        assert_eq!(
            item.height, 120.0,
            "residual real top+bottom insets should mark abspos flex cross-size definite"
        );
        assert_eq!(
            item.width, 120.0,

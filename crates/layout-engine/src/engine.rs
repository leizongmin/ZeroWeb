...
mod postprocess;
use postprocess::*;
// R2513：sizing-helpers 关联函数组（apply_intrinsic_content_sizing /
// apply_flex_aspect_ratio_item_size / apply_aspect_ratio_container_cross_size /
// apply_indefinite_percent_height_to_auto / gather_replaced_html_attr_intrinsic /
// resolve_percentage_padding / is_zeroish_len）抽出到 engine/sizing.rs（2000 行规则）。
// 关联函数（无 &self），经 `Self::method(...)` 调用，impl 块随 `mod sizing;` 激活，
// 无需 glob import。纯移动，零行为变化。
mod sizing;
...
impl LayoutEngine {
    ...
    pub fn compute_with_img_intrinsic(
        ...
        shrink_inline_blocks_to_content(
            &mut root_box,
            doc,
            styles,
            self.inline_font_context(&font_overrides),
        ...
        // R4511：multicol 收缩后的兄弟重排（taffy 兄弟位按旧堆叠高滞留，004a-ref
        // spanner 208 应 108）——须在 resync 前跑（祖先高回收以重排后的子底为准）。
        crate::multicol::restack_siblings_after_multicol_shrink(&mut root_box, styles);
        ...
        recenter_abspos_margin_auto_vertically(
            &mut root_box,
            initial_cb_height,
            self.viewport_width,
            self.viewport_height,
        ...
        crate::vertical_block_flow::apply_vertical_block_flow(
            &mut root_box,
            styles,
            self.viewport_height,
            self.viewport_width,
    ...
    fn extract_layout(
        ...
        LayoutBox {
            ...
            inline_vertical_nodes: Default::default(),
            inline_block_child_nodes: Default::default(),

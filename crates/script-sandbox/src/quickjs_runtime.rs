...
impl QuickJSSandbox {
    ...
    fn eval_in_ctx(sandbox: &QuickJSSandbox, ctx: rquickjs::Ctx, code: &str) -> Result<String, ScriptError> {
        ...
        if unsafe { qjs_sys::JS_IsException(raw) } {
            ...
            let base_msg = if caught.is_object() {
                caught
                    .as_object()
            ...
            let msg = if base_msg.is_empty() {
                "Exception generated by QuickJS".to_string()
            } else {
                format!("Exception generated by QuickJS: {base_msg}")
            };
            return Err(if msg.contains("SyntaxError") || msg.contains("syntax error") {

...
impl QuickJSSandbox {
    ...
    pub fn execute(&mut self, code: &str) -> Result<ScriptResult, ScriptError> {
        ...
        // R78：脚本源码直接经 JS_Eval(GLOBAL) 执行（见 eval_in_ctx）——不再包
        // `String(eval("..."))`（JS 层直接 eval，QuickJS 下全局声明丢失，两引擎分歧）。
        // 结果 String() coerce 由 eval_in_ctx 的 JS_ToString 承担（execute 契约不变）。
    ...
    ///
    /// R78：改用 C API `JS_Eval` + `JS_EVAL_TYPE_GLOBAL`（全局代码语义）——旧包装
    /// `String(eval("..."))` 是 JS 层**直接 eval**，QuickJS 下 `function`/`var` 声明
    /// 落在 eval 临时词法环境而非全局对象，跨 `execute`（即跨页面 `<script>`）全部
    /// 丢失（quickjs WPT dom/traversal `assert_node is not defined` 47F 整簇根因）；
    /// V8 经 `Script::compile+run` 是全局代码语义，声明落全局——两引擎行为分歧。
    /// `JS_EVAL_TYPE_GLOBAL` 是 QuickJS 中唯一让 `fd->is_global_var` 恒真的形态
    /// （quickjs.c js_parse_program：GLOBAL/MODULE 或非 strict——JS 间接 eval `(0,eval)`
    /// 对 'use strict' 脚本体仍落临时环境，实测 probe 否决），与 V8 Script::run 等价。
    /// 不加 `JS_EVAL_FLAG_STRICT`：页面 classic 脚本默认非严格（脚本体自带 'use strict'
    /// 指令时由 parser 自行进入严格模式，声明仍全局可见）。
    /// https://tc39.es/ecma262/#sec-globaldeclarationinstantiation
    fn eval_in_ctx(sandbox: &QuickJSSandbox, ctx: rquickjs::Ctx, code: &str) -> Result<String, ScriptError> {
        sandbox.install_callbacks(&ctx);
        use rquickjs::qjs as qjs_sys;

        // 全局代码执行（无强制 strict）。CString 内嵌 NUL 会截断脚本——与旧包装同限制
        //（旧 String(eval("…")) 的 Rust {:?} 转义同样无法携带 NUL），如实报错。
        let c_code =
            std::ffi::CString::new(code).map_err(|_| ScriptError::InvalidInput("script contains NUL byte".into()))?;
        let file_name = b"execute\0";
        // SAFETY: ctx 由持久 Context 的 with 借出，原生指针经 as_raw 同源取得；
        // input/file_name 为有效 C 字符串，len 与 input 一致。返回 JSValue 归本调用
        // 所有，异常路径/使用后经 JS_FreeValue 释放。
        let raw = unsafe {
            qjs_sys::JS_Eval(
                ctx.as_raw().as_ptr(),
                c_code.as_ptr(),
                ...
                qjs_sys::JS_EVAL_TYPE_GLOBAL as ::std::os::raw::c_int,
            )
        };
        if unsafe { qjs_sys::JS_IsException(raw) } {
            // 提取 JS 异常具体消息：rquickjs Error::Exception 的 Display 只有
            // "Exception generated by QuickJS"（不含 message）——用 ctx.catch() 取
            // 真实 error.message（2026-08-08 修复，保持原错误提取语义）。
            let caught = ctx.catch();
            let (error_name, base_msg) = if let Some(object) = caught.as_object() {
                (
                    object
            ...
            let msg = if error_name.is_empty() && base_msg.is_empty() {
                "Exception generated by QuickJS".to_string()
            } else {
                format!("Exception generated by QuickJS: {error_name}: {base_msg}")
            };
            return Err(if error_name == "SyntaxError" {
        ...
        // 结果 String() coerce（execute 契约：任意完成值的字符串表示）。
        // SAFETY: raw 是本调用所有的非异常 JSValue；JS_ToString 可能分配新值，
        // 两者均经 JS_FreeValue 释放；JS_ToCStringLen2 返回的 C 字符串经 JS_FreeCString 释放。
        // SAFETY: cstr 非 null 分支才解引用；from_raw_parts 的长度来自同一次
        // JS_ToCStringLen2 写回的 plen，与 C 字符串实际长度一致。
        unsafe {
            let sval = qjs_sys::JS_ToString(ctx.as_raw().as_ptr(), raw);
            let mut len = 0u64;
            let cstr = qjs_sys::JS_ToCStringLen2(ctx.as_raw().as_ptr(), &mut len, sval, 0);
            let out = if cstr.is_null() {
                Err(ScriptError::RuntimeError("failed to convert result".into()))
            } else {
                let bytes = std::slice::from_raw_parts(cstr.cast::<u8>(), len as usize);
                Ok(String::from_utf8_lossy(bytes).into_owned())
            };
            if !cstr.is_null() {
                qjs_sys::JS_FreeCString(ctx.as_raw().as_ptr(), cstr);
            }
            qjs_sys::JS_FreeValue(ctx.as_raw().as_ptr(), sval);
            qjs_sys::JS_FreeValue(ctx.as_raw().as_ptr(), raw);
            out
        }
    }

...
#[test]
#[serial_test::serial(service_worker_runtime)]
fn iframe_reload_observes_active_service_worker_controller() {
    const PAGE_URL: &str = "https://example.test/service-workers/service-worker/controller-on-reload.https.html";
    let mut webview = crate::WebView::new(WebViewConfig {
        service_worker_script_fetcher: Some(Arc::new(|_, script| {
            if script != "https://example.test/service-workers/service-worker/resources/empty-worker.js" {
                return Err(format!("unexpected script URL: {script}"));
    ...
    webview.complete_load(
        "<!doctype html><body>
           <script>
             globalThis.__iframeControllerOnReload = 'pending';
             var frame;
             var registration;
             new Promise(function(resolve) {
               ...
               if (frame.contentWindow.navigator.serviceWorker.controller !== null) {
                 globalThis.__iframeControllerOnReload = 'controlled-before-reload';
                 return;
               }
               return new Promise(function(resolve) {
                 frame.onload = function() { resolve(); };
                 ...
                 return w.navigator.serviceWorker.getRegistration('blank.html')
                   .then(function(frameRegistration) {
                     globalThis.__iframeControllerOnReload = [
                       controller instanceof w.ServiceWorker,
                       controller.scriptURL.endsWith('/resources/empty-worker.js'),
               ...
               globalThis.__iframeControllerOnReload = 'error:' + error.name + ':' + error.message;
             });
           </script>
         </body>",
        None,
    ...
    let deadline = Instant::now() + Duration::from_secs(20);
    loop {
        let value = webview.execute_script("globalThis.__iframeControllerOnReload").unwrap();
        if value != "pending" {
            assert_eq!(value, "true|true|true|true");
            break;
        }
        assert!(
            Instant::now() < deadline,
            "iframe controller-on-reload regression timed out"
        );
        std::thread::sleep(Duration::from_millis(10));
    }
}

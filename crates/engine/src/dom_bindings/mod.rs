...
use gc::{
    add_listener, cache_namednodemap, cache_native_element, cached_namednodemap, cached_native_element, decode_node_id,
    drop_cached_native_element, element_template_local, encode_node_id, listeners_local, namednodemap_template_local,
    node_exists, remove_listener, set_dom_source, set_element_template, set_namednodemap_template, with_dom,
    with_dom_mut,
};
...
pub fn install_dom_bindings(scope: &mut v8::PinScope, ctx: v8::Local<v8::Context>, dom: Rc<RefCell<Document>>) {
    ...
    // getNamedItem/setNamedItem/removeNamedItem。internal slot[0] = owner element NodeId（getter/方法
    // 经 read_node_id 取 owner）。Attr 返 plain {name,value}（documented——完整 Attr 节点后续）。

...
fn test_get_computed_style_font_shorthand_r2761() {
    ...
    assert_eq!(computed_style_property(html, "#f6", "font-stretch"), "75%");
    assert_eq!(computed_style_property(html, "#f6", "font"), "75% 14px Arial");
    assert_eq!(
        computed_style_property(html, "#f7", "font-variant-alternates"),
        "historical-forms"
    );
...
fn test_set_remove_attr_syncs_cache() {
    ...
    let mk = |html: &str| -> (V8Sandbox, Arc<Mutex<Vec<DomMutation>>>, Arc<Mutex<String>>) {
        ...
        let canvas_registry: std::sync::Arc<std::sync::Mutex<crate::js_dom_bridge::CanvasRegistry>> =
        std::sync::Arc::new(std::sync::Mutex::new(crate::js_dom_bridge::CanvasRegistry::new()));
    register_dom_callbacks(&mut sb, &mutations, &dom_html, &page_url, &canvas_registry, None);

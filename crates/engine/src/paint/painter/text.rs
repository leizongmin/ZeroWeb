                ...
                if let Some(ref mc) = multicol_info {
                    ...
                    macro_rules! render_fragment {
                        ...
                        ($frag_x:expr, $frag_y:expr, $baseline_offset:expr, $frag_fs:expr, $frag_text:expr, $frag_nid:expr, $is_ahem:expr) => {{
                            ...
                            // R2160 Phase A slice 2 part2：env `ZW_PHASEA_MULTI_INLINE`（**default-on**，
                            // `=0` kill）时，R639 per-fragment bg/border 也对 **orphan inline**
                            //（owner_h==0.0，即 part1 skip-taffy 致 inline_heights 无条目）触发——
                            // 补 part1 丢的 LayoutBox bg/border 绘制（解 R1492）。单行非 orphan
                            //（owner_h∈(0,1.5·fs]）不触发=无双绘。orphan 信号（owner_h==0）天然耦合 part1。

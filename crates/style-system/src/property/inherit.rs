...
pub fn inherit_property(parent: &ComputedStyle, child: &mut ComputedStyle, property: &str) -> bool {
    match property {
        ...
        "font-variant-alternates" => {
            child.font_variant_alternates = parent.font_variant_alternates.clone();
            child.font_variant_alternates_features = parent.font_variant_alternates_features.clone();
            true
        }
        "font-stretch" => {
...
pub fn apply_initial_value(style: &mut ComputedStyle, property: &str) -> bool {
    ...
    match property {
        ...
        "font-stretch" => {
            style.font_stretch = default_style.font_stretch;
            true
        }
        "user-select" => {
        ...
        "line-clamp" => {
            ...
            // 属元素级，同样随重置清空（R4257）。::scroll-marker per-item 伪样式同型（R4261）。
            // ::scroll-button per-direction 伪样式同型（R4262）。
            style.scroll_marker_group = default_style.scroll_marker_group;
            style.scroll_marker_pseudo = default_style.scroll_marker_pseudo;
            style.scroll_buttons = default_style.scroll_buttons;
            true
        }
        "background-image" => {

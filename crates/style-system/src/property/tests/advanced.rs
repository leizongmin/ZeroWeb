...
#[test]
fn test_apply_property_aspect_ratio_rejects_non_finite_numbers() {
    let mut style = ComputedStyle::default();
    style.aspect_ratio = Some(1.5);

    assert!(!apply_property_value(&mut style, "aspect-ratio", "inf"));
    assert_eq!(style.aspect_ratio, Some(1.5));
    assert!(!apply_property_value(&mut style, "aspect-ratio", "NaN"));
    assert_eq!(style.aspect_ratio, Some(1.5));
    assert!(!apply_property_value(&mut style, "aspect-ratio", "1 / inf"));
    assert_eq!(style.aspect_ratio, Some(1.5));
    assert!(!apply_property_value(&mut style, "aspect-ratio", "inf / inf"));
    assert_eq!(style.aspect_ratio, Some(1.5));
}
...
fn test_perspective_apply() {
    ...
    assert!(!apply_property_value(&mut style, "perspective", "50%"));
    assert_eq!(style.perspective, previous);
    assert!(!apply_property_value(&mut style, "perspective", "thin"));
    assert_eq!(style.perspective, previous);

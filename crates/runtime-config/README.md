...
- **权威清单** — `ENVIRONMENT_VARIABLES` 常量表列出全部受支持的环境变量（名称、默认值、用途说明），覆盖渲染后端（`ZEROWEB_RENDERER`）、多进程（`ZERO_BROWSER_MULTIPROCESS` / `ZERO_RENDERER_PATH`）、网络（`ZERO_HTTP2` / `ZERO_MAX_CONNECTIONS_*`）、隐私与缓存（`ZERO_PRIVATE` / `ZERO_CACHE_DIR`）、compositor（`ZW_COMPOSITOR_*` 系列）、沙箱（`ZW_RENDERER_SECCOMP` / `ZW_COMPOSITOR_*`）与图像解码进程（`ZW_IMAGE_DECODER_*`）
...
let compositor = enabled_by_default("ZW_COMPOSITOR_PROCESS");

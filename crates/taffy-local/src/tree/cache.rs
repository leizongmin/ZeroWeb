...
impl Cache {
    ...
    /// 从缓存的最终布局结果中提取 first_baselines。
    /// 用于 flex/inline-flex 容器的基线对齐计算。
    pub fn cached_baselines(&self) -> Option<crate::geometry::Point<Option<f32>>> {
        self.final_layout_entry
            .as_ref()
            .map(|entry| entry.content.first_baselines)
    }
}

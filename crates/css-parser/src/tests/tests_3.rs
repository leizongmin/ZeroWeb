...
fn test_parse_list_style_type() {
    ...
    // R2447：armenian / upper-armenian 现为 builtin 预定义（additive，≡ upper-armenian）。
    assert_eq!(parse_list_style_type("armenian"), Some(ListStyleTypeValue::Armenian));
    assert_eq!(
    ...
    // R2451：arabic-indic（阿拉伯-印度数字，numeric）。
    assert_eq!(

...
`zero-css-parser` 是 ZeroBrowser 浏览器渲染管线的第一阶段，负责将 CSS 文本转换为结构化的 AST。它包含一个基于 CSS Syntax Module Level 3 规范的词法分析器（Tokenizer）和一个递归下降语法解析器（Parser），能解析样式规则、@规则、完整的选择器语法以及常见的 CSS 属性值类型。该 crate 为下游的 `zero-style-system` 提供解析后的结构化数据。

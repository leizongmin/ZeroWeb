...
impl Document {
    ...
    // 表单状态伪类权威判定（`:disabled`/`:enabled`/`:read-write`/`:read-only`/
    // `:placeholder-shown`/`:indeterminate`/`:default`）已拆至 `form_state` 子模块
    //（R3280，rule 5 单文件 <2000 行）。下方 `element_matches_selector` 经这些 pub 方法
    // 复评延后的伪类；style-system CSS 匹配亦共享之（R3277-R3279 一致化）。
    ...
    fn element_matches_selector(&self, node: NodeId, selector: &crate::query::SimpleSelector) -> bool {
        ...
        selector.pseudos.iter().all(|p| match p {
            ...
            crate::query::PseudoClass::Enabled => {
                form_state::is_disableable_tag_of_node(self, node) && !self.is_effectively_disabled(node)
            }
